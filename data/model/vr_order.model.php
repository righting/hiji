...
//         $order_code[$i-1]['pay_price'] += $diff_amount;
     ...
     * @param string  $fields
     */
    public function getCodeList($condition = [], $fields = '*', $pagesize = '', $order = 'rec_id desc', $limit = '', $group = '')
    {
     ...
     * @param string  $fields
     */
    public function getCodeUnusedList($condition = [], $fields = '*', $pagesize = '', $order = 'rec_id desc', $limit = '')
    {
     ...
     * @return array
     */
    public function getCodeRefundList($order_list = [])
    {
        if (!empty($order_list) && is_array($order_list)) {
            ...
            $condition                = [];
            $condition['order_id']    = ['in', array_keys($order_ids)];
     ...
     * @param string  $fields
     */
    public function getOrderCodeList($condition = [], $fields = '*')
    {
        $code_list = $this->getCodeList($condition);
        //进一步处理
        if (!empty($code_list)) {
     ...
     * @param string  $fields
     */
    public function getOrderCodeInfo($condition = [], $fields = '*', $master = false)
    {
    ...
    private function _orderState($order_state)
    {
        ...
        return [$order_state, $order_state];
    }
    ...
    {
        if (!is_array($order_info) || empty($order_info)) return [];
        $step_list = [];
        // 第一步 下单完成
