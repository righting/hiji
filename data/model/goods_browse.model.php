...
class goods_browseModel extends Model{
    ...
    public function getGuessLikeGoods($member_id = 0,$shownum = 0){
        ...
        if (empty($goodsid_arr)){
            $likeGoods = Model('goods_browse')->where(['member_id'=>$_SESSION['member_id']])->limit(12)->select();//如果没有找到就直接到会员浏览记录中选取
            $goodsid_arr = array_column($likeGoods,'goods_id');
        }
     ...
     * @param int $member_id 会员ID（一般传递$_SESSION['member_id']）
     * @param int $store_id 店铺ID（一般传递$_SESSION['seller_session_info']['store_id']）
