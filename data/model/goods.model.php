...
class goodsModel extends Model
{
    ...
    // 设置分销类型
    const DISTRIBUTION_NONSUPPORT = 1;  // 不支持分销
    const DISTRIBUTION_EXCLUSIVE  = 2;   // 排他分销
    const DISTRIBUTION_WHOLE      = 3;       // 全网分销


    public function returnDistributionGroup($type = '')
    {
        $arr = [
            self::DISTRIBUTION_NONSUPPORT => '不支持分销',
            self::DISTRIBUTION_EXCLUSIVE  => '排他分销',
            self::DISTRIBUTION_WHOLE      => '全网分销',
        ];
    ...
    public function getGoodsGcStoreRandList($gcId, $storeId, $notEqualGoodsId = 0, $size = 4)
    {
        ...
        if ($notEqualGoodsId > 0) {
            $condition['goods_id'] = ['neq', (int)$notEqualGoodsId];
        }

        return $this->getGoodsOnlineList($condition, '*', 0, '', $size);
    }
    ...
    public function getGoodsListByColorDistinct($condition, $field = '*', $order = 'goods_id asc', $page = 0, $limit = 0)
    {
        ...
        if (C('dbdriver') == 'mysql') {
            ...
            $condition                 = ['goods_id' => ['in', array_keys($goods_id_list)]];
    ...
    public function getGoodsCommonWaitVerifyList($condition, $field = '*', $page = 10, $order = "goods_commonid desc", $limit = '')
    {
        if (!isset($condition['goods_verify'])) {
            $condition['goods_verify'] = ['neq', self::VERIFY1];
        }
        return $this->getGoodsCommonList($condition, $field, $page, $order, $limit);
    }
    ...
    {
        if (C('dbdriver') == 'mysql') {
            $goods_commend_list = $this->getGoodsOnlineList(['store_id' => $store_id, 'goods_commend' => 1], 'goods_id,goods_name,goods_jingle,goods_image,store_id,goods_promotion_price', 0, '', $limit, 'goods_commonid');
        } elseif (C('dbdriver') == 'oracle') {
            $goods_id_list = $this->getGoodsOnlineList(['store_id' => $store_id, 'goods_commend' => 1], 'min(goods_id) as goods_id', 0, '', $limit, 'goods_commonid');
            if (!empty($goods_id_list)) {
                $tmp = [];
                foreach ($goods_id_list as $v) {
                    $tmp[] = $v['goods_id'];
                }
                $goods_commend_list = $this->getGoodsOnlineList(['goods_id' => ['in', $tmp]], 'goods_id,goods_name,goods_jingle,goods_image,store_id,goods_promotion_price', 0, '', $limit);
            }
        }
    ...
    public function calculateStorage($goods_list)
    {
        ...
        if (!empty($goods_list)) {
            ...
            $goods_storage = $this->getGoodsList(['goods_commonid' => ['in', $goodscommonid_array]], 'goods_storage,goods_commonid,goods_id,goods_storage_alarm', '', '', false);
            $storage_array = [];
    ...
    public function editGoodsById($update, $goodsid_array, $updateXS = false)
    {
        ...
        $condition['goods_id']    = ['in', $goodsid_array];
            ...
            foreach ($val as $k => $v) {
                if ($v['is_book'] == '1') {
                    if ($v['book_down_time'] > time()) {
                        // 更新价格
                        $this->editGoodsById(['goods_promotion_price' => ($v['book_down_payment'] + $v['book_final_payment']), 'goods_promotion_type' => 0], $k);
                    } else {
                        $this->editGoodsById(['is_book' => 0, 'book_down_payment' => 0, 'book_final_payment' => 0, 'book_down_time' => 0], $k);
                    }
                }
            }
            // 查询团购是否进行
            $groupbuy = $model_groupbuy->getGroupbuyOnlineInfo(['goods_commonid' => $key]);
            if (!empty($groupbuy)) {
                // 更新价格
                $this->editGoods(['goods_promotion_price' => $groupbuy['groupbuy_price'], 'goods_promotion_type' => 1], ['goods_commonid' => $key, 'is_book' => 0]);
                continue;
            }
            foreach ($val as $k => $v) {
                if ($v['is_book'] == '1') {
                ...
                // 查询限时折扣是否进行
                $xianshigoods = $model_xianshigoods->getXianshiGoodsInfo(['goods_id' => $k, 'start_time' => ['lt', TIMESTAMP], 'end_time' => ['gt', TIMESTAMP]]);
                if (!empty($xianshigoods)) {
                    // 更新价格
                    $this->editGoodsById(['goods_promotion_price' => $xianshigoods['xianshi_price'], 'goods_promotion_type' => 2], $k);
                    continue;
                }

                // 没有促销使用原价
                $this->editGoodsById(['goods_promotion_price' => ['exp', 'goods_price'], 'goods_promotion_type' => 0], $k);
            }
        }
        return true;
    }
    ...
    public function editGoodsCommonById($update, $commonid_array)
    {
        ...
        $condition['goods_commonid'] = ['in', $commonid_array];
     ...
     * @return boolean
     */
    public function editProduces($condition, $update1, $update2 = [], $updateXS = false)
    {
        ...
        $return2 = $this->editGoods($update2, ['goods_commonid' => ['in', $commonid_array]], $updateXS);
        if ($return1 && $return2) {
            return true;
        } else {
            return false;
     ...
     * @return boolean
     */
    public function editProducesVerifyFail($condition, $update1, $update2 = [])
    {
     ...
     * @return boolean
     */
    public function editProducesNoLock($condition, $update1, $update2 = [], $updateXS = false)
    {
    ...
    public function editProducesOffline($condition)
    {
        ...
        return $this->editProducesNoLock($condition, $update, [], true);
    }
    ...
    {
        $update = ['goods_state' => self::STATE1];
        // 禁售商品、审核失败商品不能上架。
        $condition['goods_state']  = self::STATE0;
        $condition['goods_verify'] = ['neq', self::VERIFY0];
     ...
     * @return boolean
     */
    public function editProducesJx($update, $condition)
    {
        $update_param['is_select'] = 1;
        // 获取商品精选状态
        $goods_info = $this->getGoodsInfo($condition, 'is_select');
        if ($goods_info['is_select'] == 1) {
            $update_param['is_select'] = 2;
        }
     ...
     * @param $goods_id        // 商品id
     * @param $store_id        // 店铺id
     * @param $goods_member_id // 发布商品的人的id
     * @param $user_id         // 生成推广链接的人
     * @return string
     */
    public function generateGoodsReferralKey($goods_id, $store_id, $goods_member_id, $user_id)
    {
        //2018/04/27
        $encrypt_arr['user_id']   = $user_id;
        $encrypt_arr['goods_id']  = $goods_id ? $goods_id : '';
        $encrypt_arr['store_id']  = $store_id ? $store_id : '';
        $encrypt_arr['member_id'] = $goods_member_id ? $goods_member_id : '';
        $encrypt_str              = hj_encrypt(implode('+', $encrypt_arr));
        return $encrypt_str;
    }

    // 解码商品推广链接邀请码
    public function getGoodsReferralKey($referral_key)
    {
        $decrypt_str     = hj_decrypt($referral_key);
        $goods_id        = '';
        $store_id        = '';
        $goods_member_id = '';
        $user_id         = '';
        //2018/04/27
        list($user_id, $goods_id, $store_id, $goods_member_id) = explode('+', $decrypt_str);
        $decrypt_arr['goods_id']  = $goods_id;
        $decrypt_arr['store_id']  = $store_id;
        $decrypt_arr['member_id'] = $goods_member_id;
        $decrypt_arr['user_id']   = $user_id;
        return $decrypt_arr;
    }
    ...
    public function getGoodsCommonInfoByID($goods_commonid, $fields = '*')
    {
        ...
        if (empty($common_info)) {
            $common_info = $this->getGoodsCommonInfo(['goods_commonid' => $goods_commonid]);
    ...
    public function delGoods($condition)
    {
        ...
        if (!empty($goods_list)) {
            ...
            // 删除属性关联表数据
            $this->table('goods_attr_index')->where(['goods_id' => ['in', $goodsid_array]])->delete();
            // 删除优惠套装商品
            Model('p_bundling')->delBundlingGoods(['goods_id' => ['in', $goodsid_array]]);
            // 优惠套餐活动下架
            Model('p_bundling')->editBundlingCloseByGoodsIds(['goods_id' => ['in', $goodsid_array]]);
            // 推荐展位商品
            Model('p_booth')->delBoothGoods(['goods_id' => ['in', $goodsid_array]]);
            // 限时折扣
            Model('p_xianshi_goods')->delXianshiGoods(['goods_id' => ['in', $goodsid_array]]);
            //删除商品浏览记录
            Model('goods_browse')->delGoodsbrowse(['goods_id' => ['in', $goodsid_array]]);
            // 删除买家收藏表数据
            Model('favorites')->delFavorites(['fav_id' => ['in', $goodsid_array], 'fav_type' => 'goods']);
            // 删除商品赠品
            Model('goods_gift')->delGoodsGift(['goods_id' => ['in', $goodsid_array], 'gift_goodsid' => ['in', $goodsid_array], '_op' => 'or']);
            // 删除推荐组合
            Model('p_combo_goods')->delComboGoods(['goods_id' => ['in', $goodsid_array], 'combo_goodsid' => ['in', $goodsid_array], '_op' => 'or']);
            // 删除商品F码
            Model('goods_fcode')->delGoodsFCode(['goods_id' => ['in', $goodsid_array]]);
            // 删除门店商品关联
            Model('chain_stock')->delChainStock(['goods_id' => ['in', $goodsid_array]]);
        }
    ...
    public function delGoodsAll($condition)
    {
        ...
        // 删除商品表数据
        $this->delGoods(['goods_commonid' => ['in', $commonid_array]]);
        // 删除商品公共表数据
        $this->table('goods_common')->where(['goods_commonid' => ['in', $commonid_array]])->delete();
        // 删除商品图片表数据
        $this->delGoodsImages(['goods_commonid' => ['in', $commonid_array]]);
        return true;
    }
    ...
    public function delGoodsNoLock($condition)
    {
        ...
        return $this->delGoodsAll(['goods_commonid' => ['in', $commonid_array]]);
    }
    ...
    {
        QueueClient::push('sendStoreMsg', ['code' => $code, 'store_id' => $store_id, 'param' => $param]);
    }
    ...
    private function _getRecursiveClass($condition)
    {
        if (isset($condition['gc_id']) && !is_array($condition['gc_id'])) {
            ...
            if (!empty($gc_list[$condition['gc_id']])) {
                ...
                $gcchild_id         = empty($gc_list[$condition['gc_id']]['child']) ? [] : explode(',', $gc_list[$condition['gc_id']]['child']);
                $gcchildchild_id    = empty($gc_list[$condition['gc_id']]['childchild']) ? [] : explode(',', $gc_list[$condition['gc_id']]['childchild']);
    ...
    {
        $goods_info = $this->getGoodsInfoByID($goods_id, '*');
        return $goods_info['is_virtual'] == 1 && $goods_info['virtual_indate'] >= TIMESTAMP ? $goods_info : [];
    }
    ...
    public function getGoodsInfoByID($goods_id, $fields = '*')
    {
        ...
        if (empty($goods_info)) {
            $goods_info = $this->getGoodsInfo(['goods_id' => $goods_id]);
    ...
    public function getGoodsSpecListByCommonId($common_id)
    {
        ...
        if (empty($spec_list)) {
            $spec_array        = $this->getGoodsList(['goods_commonid' => $common_id], 'goods_spec,goods_id,store_id,goods_image,color_id');
    ...
    public function getGoodsImageByKey($key)
    {
        ...
        if (empty($image_list)) {
            ...
            $image_more = $this->getGoodsImageList(['goods_commonid' => $common_id], 'goods_image');
            if (empty($image_more)) {
                $goods_model = Model('goods');
                $image_more  = $this->table('goods')->where(['goods_commonid' => $common_id])->field('goods_image')->select();
            }
//            $image_more = $this->getGoodsImageList(array('goods_commonid' => $common_id, 'color_id' => $color_id), 'goods_image');
    ...
    public function getGoodsDetail($goods_id)
    {
        ...
        $spec_list        = [];       // 各规格商品地址，js使用
        $spec_list_mobile = [];       // 各规格商品地址，js使用
        $spec_image       = [];      // 各规格商品主图，规格颜色图片使用
        foreach ($spec_array as $key => $value) {
            ...
            $tpl_spec['url']                = urlShop('goods', 'index', ['goods_id' => $value['goods_id']]);
            $spec_list[]                    = $tpl_spec;
            $spec_list_mobile[$spec_sign]   = $value['goods_id'];
            $spec_image[$value['color_id']] = cthumb($value['goods_image'], 60);
        }
        ...
        if (!empty($image_more)) {
            foreach ($image_more as $val) {
                $goods_image[]        = [
                    cthumb($val['goods_image'], 60, $goods_info['store_id']),
                    cthumb($val['goods_image'], 360, $goods_info['store_id']),
                    cthumb($val['goods_image'], 1280, $goods_info['store_id'])
                ];
                $goods_image_mobile[] = cthumb($val['goods_image'], 360, $goods_info['store_id']);
            }
        } else {
            $goods_image[]        = "{ title : '', levelA : '" . cthumb($goods_info['goods_image'], 60) . "', levelB : '" . cthumb($goods_info['goods_image'], 360) . "', levelC : '" . cthumb($goods_info['goods_image'], 360) . "', levelD : '" . cthumb($goods_info['goods_image'], 1280) . "'}";
            $goods_image_mobile[] = cthumb($goods_info['goods_image'], 360);
        }
        // 新版结束
        ...
        if ($goods_info['is_book'] != '1') {
            ...
            //满即送
            $mansong_info = ($goods_info['is_virtual'] == 1) ? [] : Model('p_mansong')->getMansongInfoByStoreID($goods_info['store_id']);
        }
        ...
        if (C('cache_open')) {
            $this->_wGoodsCache($goods_id, ['goods_click' => $goods_info['goods_click']]);
            wcache('updateRedisDate', [$goods_id => $goods_info['goods_click']], 'goodsClick');
        } else {
            $this->editGoodsById(['goods_click' => ['exp', 'goods_click + 1']], $goods_id);
        }
     ...
     * 处理商品消费者保障服务信息
     */
    public function getGoodsContract($goods_list, $contract_item = [])
    {
        if (!$goods_list) {
            return [];
        }
