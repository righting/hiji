...
class pageModel extends Model {
     ...
     */
    public function getPageList($condition,$page=''){
        $condition_str = $this->_condition($condition);
        $param = array();
        $param['table'] = 'page';
        $param['where'] = $condition_str;
        $param['limit'] = $condition['limit'];
        $param['order'] = (empty($condition['order'])?'page_id asc':$condition['order']);
    ...
    private function _condition($condition){
        ...
        if ($condition['page_type'] != ''){
            $condition_str .= " and page.page_type = '". $condition['page_type'] ."'";
        }
        if ($condition['page_key'] != ''){
            $condition_str .= " and page.page_key = '". $condition['page_key'] ."'";
        }
        if ($condition['ac_id'] != ''){
        ...
        if ($condition['page_types'] != ''){
            $condition_str .= " and page.page_type in(". $condition['page_types'] .")";
        }
     ...
     * @return array 数组类型的返回结果
     */
    public function getOneArticleKey($key){
        if ($key){
            $param = array();
            $param['table'] = 'page';
            $param['field'] = 'page_key';
            $param['value'] = $key;
            $result = Db::getRow($param);
            return $result;
        }else {
    ...
    public function updates($param){
        ...
        if (is_array($param)){
            ...
            $result = Db::update('page',$tmp,$where);
            return $result;
        }else {
            return false;
        }
    ...
    public function getCount($condition = array()) {
        return $this->table('page')->where($condition)->count();
    }
}
