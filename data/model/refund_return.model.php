...
class refund_returnModel extends Model{
    ...
    // 设置退货退款类型
    const TYPE_REFUND = 1;  // 1为补单退款
    const TYPE_RETURN = 2;  // 2为补单退货
    const TYPE_REFUND_NO = 3;   // 3为不补单退款
    const TYPE_RETURN_NO = 4;   // 4为不补单退货

    // 设置状态
    const STATE_ONE = 1;    // 1.处理中
    const STATE_TWO = 2;    // 2.待管理员处理
    const STATE_THREE = 3;  // 3.已完成
    const STATE_FOUR = 4;   // 4.开始补单
    const STATE_FIVE = 5;   // 5.补单完成
    const STATE_SIX = 6;    // 6.确认补单完成

    // 根据类型获取类型信息
    public function getRefundReturnTypeInfo($type = ''){
        $arr = [
            self::TYPE_REFUND => '补单退款',
            self::TYPE_RETURN => '补单退货',
            self::TYPE_REFUND_NO => '不补单退款',
            self::TYPE_RETURN_NO => '不补单退货',
        ];
        return isset($arr[$type]) ? $arr[$type] : $arr;
    }


    public function getButtonForState($refund_state = 1){
        switch ($refund_state){
            case self::STATE_ONE :
                break;
            case self::STATE_TWO :
                break;
            case self::STATE_THREE :
                break;
            case self::STATE_FOUR :
                break;
            case self::STATE_FIVE :
                break;
            case self::STATE_SIX :
                break;
        }
    }
    public function getButtonAndMessageForType($refund_type = 1,$refund_state = 1){
        $refund_return_common_model = Model('refund_return_common');
        $where['refund_type'] = $refund_type;
        $where['refund_state'] = $refund_state;
        $info = $refund_return_common_model->where($where)->find();
        return $info;
    }
    ...
    /*
     * 平台确认退款，如果买家选择了补单，那么就进入补单模式
     */
    public function editOrderBeginRefund($refund,$user = '') {
    ...
    public function getRefundList($condition = array(), $page = '', $order = 'refund_id desc', $limit = '') {
        $condition['refund_type'] = ['in',[1,3]];//类型:1为补单退款,2为补单退货，3为不补单退款,4为不补单退货
    ...
    public function getReturnList($condition = array(), $page = '', $order = 'refund_id desc', $limit = '') {
        $condition['refund_type'] = ['in',[2,4]];//类型:1为补单退款,2为补单退货，3为不补单退款,4为不补单退货
        $result = $this->getRefundReturnList($condition, $page,'*',$limit,$order);
        return $result;
    }
