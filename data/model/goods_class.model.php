...
{
    // 设置商品类型状态
    const GOODS_CLASS_STATUS_SHOW   = 1;      // 显示
    const GOODS_CLASS_STATUS_HIDE   = 2;      // 隐藏
    const GOODS_CLASS_STATUS_DELETE = 99;   // 删除

    /**
     * 设置海吉壹佰、全球跨境、海豚主场、农耕优品、农资农具、三品一标、农旅民宿、消费扶贫的一级分类id
     */
    const HJ_CATEGORY_TYPE = 5481; // 海吉壹佰
    const KJ_CATEGORY_TYPE = 5505; // 全球跨境
    const HT_CATEGORY_TYPE = 1068; // 海豚主场

    const NG_CATEGORY_TYPE   = 5504; // 农耕优品
    const NZNJ_CATEGORY_TYPE = 5509; // 农资农具
    const SPYB_CATEGORY_TYPE = 5508; // 三品一标
    const NLMS_CATEGORY_TYPE = 5510; // 农旅民宿
    const XFFP_CATEGORY_TYPE = 5511; // 消费扶贫
    ...
    public function getCache()
    {
        ...
        if (!$data) {
            $data = [];
            foreach ((array)$this->getGoodsClassList([]) as $v) {
                $id                       = $v['gc_id'];
                $pid                      = $v['gc_parent_id'];
                $data['data'][$id]        = $v;
                $data['parent'][$id]      = $pid;
    ...
    public function getIndexCache()
    {
        ...
        $data = rkcache('gc_index_class');
        if (!$data) {
            $data            = [];
            $where['status'] = self::GOODS_CLASS_STATUS_SHOW;
            foreach ((array)$this->getGoodsClassList($where) as $v) {
                $id                       = $v['gc_id'];
                $pid                      = $v['gc_parent_id'];
                $data['data'][$id]        = $v;
                $data['parent'][$id]      = $pid;
            ...
            wkcache('gc_index_class', $data);
        }
        return $this->cachedIndexData = $data;
    }
     ...
     * 从缓存获取前台头部全部商品分类
     */
    public function getIndexGoodsClassListAll()
    {
     ...
     */
    public function editGoodsClass($data = [], $condition = [])
    {
        // 删除缓存
        $this->dropCache();
    ...
    public function getGoodsClass($store_id, $pid = 0, $deep = 1, $group_id = null, $gc_limits = 1, $is_all_class = null)
    {
        ...
        if (!$is_all_class) {
            ...
            $gcid_array           = $model_storebindclass->getStoreBindClassList([
                'store_id' => $store_id,
                'state'    => ['in', [1, 2]],
        ...
        if (!$gc_limits && $group_id) {
            $gc_list_group = Model('seller_group_bclass')->getSellerGroupBclasList(['group_id' => $group_id], '', '', 'distinct class_' . $deep, 'class_' . $deep);
            $temp          = [];
    ...
    public function delGoodsClassByGcIdString($gcids)
    {
        ...
        foreach ($gcids as $gc_id) {
            ...
            $gcid_array = array_merge($gcid_array, [$gc_id], $child, $childchild);
        }
        // 删除商品分类
        $this->delGoodsClass(['gc_id' => ['in', $gcid_array]]);
        // 删除常用商品分类
        Model('goods_class_staple')->delStaple(['gc_id_1|gc_id_2|gc_id_3' => ['in', $gcid_array]]);
        // 删除分类tag表
        Model('goods_class_tag')->delGoodsClassTag(['gc_id_1|gc_id_2|gc_id_3' => ['in', $gcid_array]]);
        // 删除店铺绑定分类
        Model('store_bind_class')->delStoreBindClass(['class_1|class_2|class_3' => ['in', $gcid_array]]);
        //删除商家权限组绑定的分类
        Model('seller_group_bclass')->delSellerGroupBclass(['class_1|class_2|class_3' => ['in', $gcid_array]]);
        // 商品下架
        Model('goods')->editProducesLockUp(['goods_stateremark' => '商品分类被删除，需要重新选择分类'], ['gc_id' => ['in', $gcid_array]]);
        return true;
    }
    ...
    public function get_all_category($update_all = 0)
    {
        ...
        if ($update_all == 1 || !($gc_list = rkcache('all_categories'))) {
            $class_list  = $this->getIndexGoodsClassListAll();
            $gc_list     = [];
            $class1_deep = [];//第1级关联第3级数组
            $class2_ids  = [];//第2级关联第1级ID数组
    ...
    public function _getGoodsClassNavById($gc_id)
    {
        ...
        if ($nav_info['cn_brandids'] != '') {
            $nav_info['cn_brands'] = $model_brand->getBrandList(['brand_id' => ['in', $nav_info['cn_brandids']]]);
            unset($nav_info['cn_brandids']);
        }
        if ($nav_info['cn_classids'] != '') {
            $nav_info['cn_classs'] = $this->getGoodsClassList(['gc_id' => ['in', $nav_info['cn_classids']]]);
     ...
     */
    public function getTreeClassList($show_deep = '3', $condition = [])
    {
    ...
    {
        static $show_class = [];//树状的平行数组
        if (is_array($class_list) && !empty($class_list)) {
            $size = count($class_list);
            if ($i == 0) $show_class = [];//从0开始时清空数组，防止多次调用后出现重复
    ...
    public function getGoodsClassNav($id = 0, $sign = 1)
    {
        if (intval($id) > 0) {
            ...
            if ($sign == 1) {
                ...
                $nav_link [] = [
                    'title' => $data[$id]['gc_name'],
                    'link'  => urlShop('cate', 'index', ['cate_id' => $data[$id]['gc_id']])
                ];
            }
            ...
            // 加上 首页 商品分类导航
            $nav_link[] = ['title' => Language::get('goods_class_index_search_results')];
        }
        // 首页导航
        $nav_link[] = ['title' => Language::get('homepage'), 'link' => SHOP_SITE_URL];
    ...
    public function getGoodsClassArrayFirstId($id = 0, $sign = 1)
    {
        if (intval($id) > 0) {
            $data = $this->getGoodsClassIndexedListAll();

            if ($sign == 1) {
                $nav_link [$data[$id]['gc_id']] = $data[$id]['gc_id'];
            } else {
                $nav_link [$data[$id]['gc_id']] = $data[$id]['gc_id'];
            }
    ...
    public function getGoodsClassLineForTag($id = 0)
    {
        if (intval($id) > 0) {
            ...
            if ($class['gc_parent_id'] != 0) {
                ...
                if ($parent_1['gc_parent_id'] != 0) {
                    ...
                    if ($parent_2['gc_parent_id'] != 0) {
                        $parent_3                = $this->getGoodsClassInfoById($parent_2['gc_parent_id']);
                        $gc_line['gc_id_1']      = $parent_3['gc_id'];
                        $gc_line['gc_tag_name']  = trim($parent_3['gc_name']) . ' >';
                        $gc_line['gc_tag_value'] = trim($parent_3['gc_name']) . ',';
                    }
                    if (!isset($gc_line['gc_id_1'])) {
                        $gc_line['gc_id_1'] = $parent_2['gc_id'];
                    } else {
                        $gc_line['gc_id_2'] = $parent_2['gc_id'];
                    }
                ...
                if (!isset($gc_line['gc_id_1'])) {
                    ...
                    $gc_line['gc_id_3'] = $parent_1['gc_id'];
                }
            ...
            } else if (!isset($gc_line['gc_id_3'])) {
                $gc_line['gc_id_3'] = $class['gc_id'];
            } else {
                $gc_line['gc_id_4'] = $class['gc_id'];
            }
    ...
    public function getGoodsClassLineForTag备份($id = 0)
    {
        if (intval($id) > 0) {
            $gc_line = [];
            /**
    ...
    public function getKeyWords($gc_id = null)
    {
        ...
        return [1 => $seo_title, 2 => trim($seo_key, ','), 3 => trim($seo_desc, ',')];
    }
    ...
    public function getGoodsclassCache($choose_gcid, $show_depth = 3)
    {
        ...
        return ['showclass' => $show_gc_list, 'choose_gcid' => $choose_gcidarr];
    }
}
