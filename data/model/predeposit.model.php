    ...
    public function changePd($change_type, $data = array())
    {
        ...
        switch ($change_type) {
            ...
            case 'order_invite':
                ...
                $data_msg['desc'] = $data_log['lg_desc'];
                break;
            case 'order_key_invite':
                $data_log['lg_av_amount'] = +$data['amount'];
                $data_log['lg_desc'] = '邀请购买，获得提成，订单号: ' . $data['order_sn'];
                $data_pd['available_predeposit'] = array('exp', 'available_predeposit+' . $data['amount']);
                ...
                $data_msg['desc'] = $data_log['lg_desc'];
                break;
            case 'personal_consumption_bonus':
                $data_log['lg_av_amount'] = +$data['amount'];
                $data_log['lg_desc'] = '个人消费分红，订单号: ' . $data['order_sn'];
                $data_pd['available_predeposit'] = array('exp', 'available_predeposit+' . $data['amount']);
            ...
            case 'order_team_invite':
                $data_log['lg_av_amount'] = +$data['amount'];
                $data_log['lg_desc'] = '直推店铺销售利润提成，订单号: ' . $data['order_sn'];
                $data_pd['available_predeposit'] = array('exp', 'available_predeposit+' . $data['amount']);
            ...
            case 'shipping_fee':
                $msg = $data['msg'];
                $data_log['lg_freeze_amount'] = +$data['amount'];
                $data_log['lg_desc'] = '订单运费' . $msg . ',订单号: ' . $data['pdr_sn'];
                $data_pd['freeze_predeposit'] = array('exp', 'freeze_predeposit+' . $data['amount']);
                $data_msg['av_amount'] = 0;
                $data_msg['freeze_amount'] = +$data['amount'];
                $data_msg['desc'] = $data_log['lg_desc'];
            ...
            case 'sys_unfreeze_money':
                ...
                $data_msg['desc'] = $data_log['lg_desc'];
                break;
            case 'seller_money':                // @TODO 13.调节预存款
                $msg = $data['msg'];
                $data_log['lg_freeze_amount'] = +$data['amount'];
            ...
            case 'seller_refund':
                ...
                $data_msg['desc'] = $data_log['lg_desc'];
                break;
            case 'user_bonus_money':
                $msg = $data['msg'];
                $data_log['lg_freeze_amount'] = +$data['amount'];
                $data_log['lg_desc'] = $msg;
                $data_pd['freeze_predeposit'] = array('exp', 'freeze_predeposit+' . $data['amount']);
                $data_msg['av_amount'] = 0;
                $data_msg['freeze_amount'] = +$data['amount'];
                $data_msg['desc'] = $data_log['lg_desc'];
                break;
            case 'hi_to_bonus'; //分红HI值转奖金
                $msg = $data['msg'];
                $data_log['lg_av_amount'] = $data['amount'];
                $data_log['lg_desc'] = '用户HI值转现,新增预存款' . $msg ;
                $data_pd['available_predeposit'] = array('exp', 'available_predeposit+' . $data['amount']);
                $data_msg['av_amount'] = $data['amount'];
                $data_msg['available_predeposit'] = +$data['amount'];
                $data_msg['desc'] = $data_log['lg_desc'];
                break;
            case 'bonus_to_hi'; //奖金转分红HI值
                $msg = $data['msg'];
                $data_log['lg_av_amount'] = -$data['amount'];
                $data_log['lg_desc'] = '用户转入HI值,减少预存款' . $msg ;
                $data_pd['available_predeposit'] = array('exp', 'available_predeposit-' . $data['amount']);
                $data_msg['av_amount'] = -$data['amount'];
                $data_msg['available_predeposit'] = -$data['amount'];
                $data_msg['desc'] = $data_log['lg_desc'];
        ...
        p('member_id=>'.$data['member_id'].'member_name=>'.$data['member_name'].'consume_amount=>'.$data['amount'].'consume_time=>'.time());
        QueueClient::push('addConsume', array('member_id' => $data['member_id'], 'member_name' => $data['member_name'],
            'consume_amount' => $data['amount'], 'consume_time' => time(), 'consume_remark' => $data_log['lg_desc']));
        QueueClient::push('sendMemberMsg', $param);
        p('sendMemberMsg'. json_encode($param));
        return $insert;
    }
