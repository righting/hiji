...
class pointorderModel extends Model{
    ...
    public function getPointOrderState($order_state){
        ...
        $pointorderstate_arr[20] = array('waitship','待支付');
		$pointorderstate_arr[21] = array('waitship','待发货');
    ...
    public function cancelPointOrder($point_orderid, $member_id = 0) {
        ...
        //获取积分日志
        $pl_info = Model('points')->getPointsLogList(array('points_ordersn'=>$order_info['point_ordersn']));
        ...
        if ($result){
            ...
            if (!empty($pl_info)){
                foreach ($pl_info as $v){
                    $insert_arr['pl_points']        = abs($v['pl_points']) ;
                    if ($v['pl_stage']=='pointorder'){
                        $insert_arr['pl_desc']  = L('member_pointorder_cancel_tip1').$order_info['point_ordersn'].L('member_pointorder_cancel_tip2');
                    }elseif ($v['pl_stage']=='pointorder_other'){
                        $insert_arr['pl_desc']  = L('member_pointorder_cancel_tip1').$order_info['point_ordersn'].L('member_pointorder_cancel_tip3');
                    }
                    Model('points')->savePointsLog($v['pl_stage'],$insert_arr,true);
                }
            }
            //更改兑换礼品库存
    ...
    public function createOrder($post_arr, $pointprod_arr, $member_info){
        ...
        $order_array['point_shipping_fee']  =$pointprod_arr['pgoods_shipping_fee'];
        $order_array['point_service_charge']  =$pointprod_arr['pgoods_service_charge'];
        ...
        $insert_arr = array();
            ...
            if ($member_info['member_h_points']>=$pointprod_arr['pgoods_pointall']) {//H积分够就直接扣
                $insert_arr['pl_points'] = -$pointprod_arr['pgoods_pointall'];
            }else{
                $insert_arr['pl_points'] = -($pointprod_arr['pgoods_pointall']-$member_info['member_h_points']);
                Model('points')->savePointsLog('pointorder',$insert_arr,true);
                $insert_arr['pl_points'] = -$member_info['member_h_points'];
            }

        Model('points')->savePointsLog('pointorder_other',$insert_arr,true);
