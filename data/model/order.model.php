...
class orderModel extends Model
{
     ...
     * @return unknown
     */
    public function getOrderInfo($condition = [], $extend = [], $fields = '*', $order = '', $group = '')
    {
        ...
        if (in_array('order_common', $extend)) {
            $order_info['extend_order_common']                 = $this->getOrderCommonInfo(['order_id' => $order_info['order_id']]);
        ...
        if (in_array('store', $extend)) {
            $order_info['extend_store'] = Model('store')->getStoreInfo(['store_id' => $order_info['store_id']]);
        }
        ...
        if (in_array('order_goods', $extend)) {
            //取商品列表
            $order_goods_list                 = $this->getOrderGoodsList(['order_id' => $order_info['order_id']]);
    ...
    public function getOrderCommonInfo($condition = [], $field = '*')
    {
        return $this->table('order_common')->where($condition)->find();
    }

    public function getOrderPayInfo($condition = [], $master = false)
    {
     ...
     */
    public function getStoreOrderList($store_id, $order_sn, $buyer_name, $state_type, $query_start_date, $query_end_date, $skip_off, $fields = '*', $extend = [], $chain_id = null)
    {
        $condition             = [];
        ...
        $allow_state_array = ['state_new', 'state_pay', 'state_send', 'state_success', 'state_cancel'];
        if (in_array($state_type, $allow_state_array)) {
            $condition['order_state'] = str_replace($allow_state_array,
                [ORDER_STATE_NEW, ORDER_STATE_PAY, ORDER_STATE_SEND, ORDER_STATE_SUCCESS, ORDER_STATE_CANCEL], $state_type);
        } else {
     ...
     */
    public function getNormalOrderList($condition, $pagesize = '', $field = '*', $order = 'order_id desc', $limit = '', $extend = [])
    {
     ...
     * @return Ambigous <multitype:boolean Ambigous <string, mixed> , unknown>
     */
    public function getOrderList($condition, $pagesize = '', $field = '*', $order = 'order_id desc', $limit = '', $extend = [], $master = false)
    {
        ...
        //追加返回订单扩展表信息
        if (in_array('order_common', $extend)) {
            $order_common_list = $this->getOrderCommonList(['order_id' => ['in', array_keys($order_list)]]);
        ...
        if (in_array('store', $extend)) {
            ...
            $store_list     = Model('store')->getStoreList(['store_id' => ['in', $store_id_array]]);
            $store_new_list = [];
        ...
        if (in_array('order_goods', $extend)) {
            ...
            if (!empty($order_goods_list)) {
                ...
                $order_list[$value['order_id']]['extend_order_goods'] = [];
            }
        }
     ...
     * @param unknown $condition
     */
    public function getOrderStateTakesCount($condition = [])
    {
        $condition['order_state'] = ['in', [ORDER_STATE_NEW, ORDER_STATE_PAY]];
        $condition['chain_code']  = ['gt', 0];
        return $this->getOrderCount($condition);
     ...
     * @param unknown $condition
     */
    public function getOrderStateTradeCount($condition = [])
    {
        $condition['order_state'] = [['neq', ORDER_STATE_CANCEL], ['neq', ORDER_STATE_SUCCESS], 'and'];
        return $this->getOrderCount($condition);
    }
     ...
     * @param string  $key
     */
    public function getOrderGoodsList($condition = [], $fields = '*', $limit = null, $page = null, $order = 'rec_id desc', $group = null, $key = null)
    {
     ...
     * @param string  $limit
     */
    public function getOrderCommonList($condition = [], $fields = '*', $order = '', $limit = null)
    {
    ...
    public function addOrder($data)
    {
        ...
        if ($insert) {
            //更新缓存
            if (C('cache_open')) {
                QueueClient::push('delOrderCountCache', ['buyer_id' => $data['buyer_id'], 'store_id' => $data['store_id']]);
            }
        }
        return $insert;
    }
    ...
    /**
     * @TODO 13-2 向order_log表中添加日志
     * 添加订单日志
     */
    public function addOrderLog($data)
    {
        $data['log_role'] = str_replace(['buyer', 'seller', 'system', 'admin'], ['买家', '商家', '系统', '管理员'], $data['log_role']);
        $data['log_time'] = TIMESTAMP;
        return $this->table('order_log')->insert($data);
    }

    /**
     * @TODO 13-1 更改orders表中的订单信息
     * 更改订单信息
     *
     ...
     * @param string  $order
     */
    public function getOrderLogInfo($condition = [], $order = '')
    {
    ...
    public function getOrderOperateState($operate, $order_info)
    {
        ...
        switch ($operate) {
            ...
            case 'store_cancel':
                $state = ($order_info['order_state'] == ORDER_STATE_NEW && $order_info['payment_code'] != 'chain') ||
                    ($order_info['payment_code'] == 'offline' &&
                        in_array($order_info['order_state'], [ORDER_STATE_PAY, ORDER_STATE_SEND]));
                break;
            ...
            //买家投诉
            case 'complain':
                $state = in_array($order_info['order_state'], [ORDER_STATE_PAY, ORDER_STATE_SEND]) ||
            ...
            //订单延迟
            case 'delay':
                $state = !$order_info['lock_state'] && $order_info['delays_time'] == 0 && $order_info['order_state'] == ORDER_STATE_SEND;
                break;

            //解冻
            case 'thaw':
                $state = !$order_info['lock_state'] && $order_info['order_state'] == 40;
                break;

            //门店自提完成
            case 'chain_receive':
                $state = !$order_info['lock_state'] && in_array($order_info['order_state'], [ORDER_STATE_NEW, ORDER_STATE_PAY]) &&
            ...
            //快递跟踪
            case 'deliver':
                $state = !empty($order_info['shipping_code']) && in_array($order_info['order_state'], [ORDER_STATE_SEND, ORDER_STATE_SUCCESS]);
                break;

            //放入回收站
            case 'delete':
                $state = in_array($order_info['order_state'], [ORDER_STATE_CANCEL, ORDER_STATE_SUCCESS, 50]) && $order_info['delete_state'] == 0;
                break;

            //永久删除、从回收站还原
            case 'drop':
            case 'restore':
                $state = in_array($order_info['order_state'], [ORDER_STATE_CANCEL, ORDER_STATE_SUCCESS]) && $order_info['delete_state'] == 1;
                break;
...
//                $state = true;
    ...
    {
        $condition['order_state'] = ['in', [ORDER_STATE_PAY, ORDER_STATE_SEND, ORDER_STATE_SUCCESS]];
