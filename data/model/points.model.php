...
class pointsModel extends Model
{
    ...
    function savePointsLog($stage, $insertarr, $if_repeat = true)
    {
        ...
        switch ($stage) {
            ...
            case 'order':
                if (!$insertarr['pl_desc']) {
                    $insertarr['pl_desc'] = '订单' . $insertarr['order_sn'] . '海豚主场购物消费';
                }
                ...
                $data                      = [];
                $data['order_pointscount'] = ['exp', 'order_pointscount+' . $insertarr['pl_points']];
                $obj_order->editOrderCommon($data, ['order_id' => $insertarr['order_id']]);
                break;
            case 'order_for_other':
                if (!$insertarr['pl_desc']) {
                    $insertarr['pl_desc'] = '订单' . $insertarr['order_sn'] . '非海豚主场购物消费';
                }
                $insertarr['pl_points'] = 0;
                if ($insertarr['orderprice']) {
                    $insertarr['pl_points'] = @intval($insertarr['orderprice'] / C('points_orderrate'));
            ...
            case 'pointorder_other':
                if (!$insertarr['pl_desc']) {
                    $insertarr['pl_desc'] = '兑换礼品信息' . $insertarr['point_ordersn'] . '消耗非海豚主场积分';
                }
                break;
            case 'app':
                if (!$insertarr['pl_desc']) {
            ...
            case 'pointmember':
                if (!$insertarr['pl_desc']) {
                    $insertarr['pl_desc'] = '兑换会员' . $insertarr['grade_name'] . '消耗积分';
                }
                break;
            case 'other':
                break;
        ...
        $value_array['points_ordersn'] = $insertarr['point_ordersn'];
        ...
        if ($result) {
            ...
            $upmember_array = [];
            /*if ($stage != 'pointorder_other'){
                $upmember_array['member_points'] = array('exp','member_points+'.$insertarr['pl_points']);
            }*/

            if ($stage == 'order_for_other' || $stage == 'pointorder_other') {
                $upmember_array['member_h_points'] = ['exp', 'member_h_points+' . $insertarr['pl_points']];
            } else {
                $upmember_array['member_points'] = ['exp', 'member_points+' . $insertarr['pl_points']];
            }
            $obj_member->editMember(['member_id' => $insertarr['pl_memberid']], $upmember_array);
            return true;
        } else {
            return false;
        }
    ...
    private function getCondition($condition_array)
    {
        ...
        //类型(支出、收入)
        if (isset($condition_array['types'])) {
            switch ($condition_array['types']) {
                case 2:
                    $condition_sql .= " and points_log.pl_points > 0";
                    break;
                case 3:
                    $condition_sql .= " and points_log.pl_points < 0";
                    break;
     ...
     * @return mixed
     */
    public function getPointsSumByStage($user_id, $stage)
    {
        return Model()->table('points_log')->where(['pl_memberid' => $user_id, 'pl_stage' => $stage])->sum('pl_points');
    }
}
