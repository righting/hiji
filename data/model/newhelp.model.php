...
class newhelpModel extends Model {
    ...
    public function getArticleList($condition,$page=''){
        ...
        $result = Db::select($param,$page);//echo '<pre>';print_r($param);exit;
        return $result;
    }
    ...
    public function getJoinList($condition,$page=''){
        ...
        $condition_str  = $this->_condition($condition);
        $param  = array();
        $param['table'] = 'newhelp,help_class';
    ...
    private function _condition($condition){
        ...
        if ($condition['article_type'] != ''){
            $condition_str .= " and newhelp.article_type = '". $condition['article_type'] ."'";
        }
        if ($condition['common'] != ''){
            $condition_str .= " and newhelp.common = '". $condition['common'] ."'";
        }
        if ($condition['ac_id'] != ''){
            $condition_str .= " and newhelp.ac_id = '". $condition['ac_id'] ."'";
        }
        if ($condition['ac_ids'] != ''){
            $condition_str .= " and newhelp.ac_id in(". $condition['ac_ids'] .")";
        }
        if ($condition['article_position_in'] != ''){
            $condition_str .= " and newhelp.article_position in(". $condition['article_position_in'] .")";
        }
        if ($condition['like_title'] != ''){
            $condition_str .= " and newhelp.article_title like '%". $condition['like_title'] ."%'";
        }
    ...
    public function updates($param){
        ...
        if (is_array($param)){
            ...
            $result = Db::update('newhelp',$tmp,$where);
            return $result;
        }else {
            return false;
        }
    ...
    public function del($id){
        if (intval($id) > 0){
            ...
            $result = Db::delete('newhelp',$where);
            return $result;
        }else {
            return false;
        }
    ...
    public function getCount($condition = array()) {
        return $this->table('newhelp')->where($condition)->count();
    }
}
