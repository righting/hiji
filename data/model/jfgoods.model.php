...
class jfgoodsModel extends Model
{
    public function __construct()
    {
        parent::__construct('jfgoods');
    }
    ...
    {   //echo '<pre>';print_r($insert);
        $result = $this->table('jfgoods')->insert($insert);
    ...
    {
        return $this->table('jfgoods_common')->insert($insert);
    }
    ...
    public function getGoodsList($condition, $field = '*', $group = '', $order = '', $limit = 0, $page = 0, $count = 0)
    {
        ...
        return $this->table('jfgoods')->field($field)->where($condition)->group($group)->order($order)->limit($limit)->page($page, $count)->select();
    }
    ...
    public function getGoodsListByColorDistinct($condition, $field = '*', $order = 'goods_id asc', $page = 0, $limit = 0)
    {
        ...
        if (C('dbdriver') == 'mysql') {
            ...
            $condition['goods_state'] = self::STATE1;
            $condition['goods_verify'] = self::VERIFY1;
            $goods_id_list = $this->table('jfgoods')->where($condition)->field('min(goods_id) as goods_id,' . $_field)->group($_field)->key('goods_id')->select();
    ...
    {	
        $condition = $this->_getRecursiveClass($condition);
        return $this->table('jfgoods')->field($field)->where($condition)->order($order)->limit($limit)->page($page)->select();
    }
    ...
    {
        $condition = $this->_getRecursiveClass($condition);
        return $this->table('jfgoods,store')->field($field)->join('inner')->on('goods.store_id = store.store_id')->where($condition)->select();
    }
    ...
    {
        if (C('dbdriver') == 'mysql') {
            $goods_commend_list = $this->getGoodsOnlineList(array('store_id' => $store_id, 'goods_commend' => 1), 'goods_id,goods_name,goods_jingle,goods_image,store_id,goods_promotion_price', 0, '', $limit, 'goods_commonid');
        } elseif (C('dbdriver') == 'oracle') {
            $goods_id_list = $this->getGoodsOnlineList(array('store_id' => $store_id, 'goods_commend' => 1), 'min(goods_id) as goods_id', 0, '', $limit, 'goods_commonid');
            if (!empty($goods_id_list)) {
                $tmp = array();
                foreach ($goods_id_list as $v) {
                    $tmp[] = $v['goods_id'];
                }
                $goods_commend_list = $this->getGoodsOnlineList(array('goods_id' => array('in', $tmp)), 'goods_id,goods_name,goods_jingle,goods_image,store_id,goods_promotion_price', 0, '', $limit);
            }
        }
    ...
    public function editGoodsById($update, $goodsid_array, $updateXS = false)
    {
        ...
        $result = $this->table('jfgoods')->where($condition)->update($update);
        if ($result) {
    ...
    public function editGoodsCommonById($update, $commonid_array)
    {
        ...
        $result = $this->table('jfgoods_common')->where($condition)->update($update);
        if ($result) {
    ...
    {
        return $this->table('jfgoods')->field($field)->where($condition)->find();
    }
    ...
    public function getGoodsCommonInfo($condition, $field = '*')
    {
        ...
        if($condition){
            ...
            foreach ($condition as $k=>$v){
                $new_condition['jfgoods_common.'.$k]=$v;
            }
        }
        //var_dump($new_condition);exit;
        return $this->table('jfgoods,jfgoods_common')->field($field)->join('inner')->on('jfgoods.goods_commonid = jfgoods_common.goods_commonid')->where($new_condition)->find();
        return $this->table('jfgoods_common')->field($field)->where($condition)->find();
    }

    /**
     * $new_condition（优先查询缓存）
     * 如果未找到，则缓存所有字段
    ...
    {
        return $this->table('jfgoods')->where($condition)->sum($field);
    }
    ...
    {
        return $this->table('jfgoods')->where($condition)->count();
    }
    ...
    public function getGoodsOnlineCount($condition, $field = '*')
    {
        $condition['goods_state'] = self::STATE1;
        $condition['goods_verify'] = self::VERIFY1;
        return $this->table('jfgoods')->where($condition)->group('')->count1($field);
    }
    ...
    {
        return $this->table('jfgoods_common')->where($condition)->count();
    }
    ...
    public function delGoods($condition)
    {
        ...
        return $this->table('jfgoods')->where($condition)->delete();
    }
    ...
    public function delGoodsAll($condition)
    {
        ...
        // 删除商品公共表数据
        $this->table('jfgoods_common')->where(array('goods_commonid' => array('in', $commonid_array)))->delete();
        // 删除商品图片表数据
    ...
    public function getGoodsImageByKey($key)
    {
        ...
        if (empty($image_list)) {
            ...
            $image_more = $this->getGoodsImageList(array('goods_commonid' => $common_id), 'goods_image');
            if (empty($image_more)) {
                $goods_model = Model('jfgoods');
                $image_more = $this->table('jfgoods')->where(['goods_commonid' => $common_id])->field('goods_image')->select();
            }
//            $image_more = $this->getGoodsImageList(array('goods_commonid' => $common_id, 'color_id' => $color_id), 'goods_image');
    ...
    {
        return rcache($goods_id, 'jfgoods', $fields);
    }
    ...
    {
        return wcache($goods_id, $goods_info, 'jfgoods');
    }
    ...
    {
        return dcache($goods_id, 'jfgoods');
    }
    ...
    {
        return rcache($goods_commonid, 'jfgoods_common', $fields);
    }
    ...
    {
        return wcache($goods_commonid, $common_info, 'jfgoods_common');
    }
    ...
    {
        return dcache($goods_commonid, 'jfgoods_common');
    }
    ...
    public function getGoodsDetail($goods_id)
    {
        ...
        foreach ($spec_array as $key => $value) {
            ...
            $tpl_spec['url'] = urlShop('jfgoods', 'index', array('goods_id' => $value['goods_id']));
            $spec_list[] = $tpl_spec;
