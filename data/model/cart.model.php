...
class cartModel extends Model {
    ...
    private function _addCartDb($goods_info = array(),$quantity) {
        ...
        if (!empty($check_cart)){
            // 如果购物车已经加了这个商品，那么将推广邀请码修改为现在的（可以是空）
            return $this->where(['cart_id'=>$check_cart['cart_id']])->update(['referral_key'=>$goods_info['referral_key'],'updated_at'=>$now_time]);
        }
        ...
        $array['referral_key'] = $goods_info['referral_key'];
        $array['created_at'] = $now_time;
