...
class ALIOSS{
	...
	public function auth($options){
		...
		if($this->enable_domain_style){
			...
			$hostname = (isset($options[self::OSS_BUCKET]) && ''!==$options[self::OSS_BUCKET]) ? $this->hostname : $this->hostname;
		}
...
//		$extension = array_pop ( $extension );
        // 当上传的文件是临时文件时使用$object的后缀来设置header头里的文件类型
        $extension = pathinfo($object, PATHINFO_EXTENSION);

        if (!isset($options[self::OSS_CONTENT_TYPE])) {
            $options[self::OSS_CONTENT_TYPE] = MimeTypes::get_mimetype(strtolower($extension));
        }
		$options[self::OSS_METHOD] = self::OSS_HTTP_PUT;
		$options[self::OSS_BUCKET] = $bucket;
		$options[self::OSS_OBJECT] = $object;
