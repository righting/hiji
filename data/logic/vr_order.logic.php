...
class vr_orderLogic
{
    ...
    public function changeOrderStateCancel($order_info, $role, $msg, $if_queue = true)
    {
        ...
        try {
            ...
            //库存、销量变更
            //为保证数据准确，不使用队列
            $result = Logic('queue')->cancelOrderUpdateStorage([$order_info['goods_id'] => $order_info['goods_num']]);
            if (!$result['state']) {
    ...
    public function changeOrderStatePay($order_info, $role, $post)
    {
        try {
            ...
            $update                       = $model_vr_order->editOrder($update_order, ['order_id' => $order_info['order_id'], 'order_state' => ORDER_STATE_NEW]);
            if (!$update) {
            ...
            //发送兑换码到手机
            $param = ['order_id' => $order_info['order_id'], 'buyer_id' => $order_info['buyer_id'], 'buyer_phone' => $order_info['buyer_phone']];
    ...
    public function changeOrderStateSuccess($order_id)
    {
        ...
        $condition['vr_indate']   = ['gt', TIMESTAMP];
        $order_code_info          = $model_vr_order->getOrderCodeInfo($condition, '*', true);
        if (empty($order_code_info)) {
            $update = $model_vr_order->editOrder(['order_state' => ORDER_STATE_SUCCESS, 'finnshed_time' => TIMESTAMP], ['order_id' => $order_id]);
            if (!$update) {
                callback(false, '更新失败');
            }
            ...
            //添加会员积分
            if (C('points_isuse') == 1) {
                Model('points')->savePointsLog('order', ['pl_memberid' => $order_info['buyer_id'], 'pl_membername' => $order_info['buyer_name'], 'orderprice' => $order_info['order_amount'], 'order_sn' => $order_info['order_sn'], 'order_id' => $order_info['order_id']], true);
            }

            //添加会员经验值
            Model('exppoints')->saveExppointsLog('order', ['exp_memberid' => $order_info['buyer_id'], 'exp_membername' => $order_info['buyer_name'], 'orderprice' => $order_info['order_amount'], 'order_sn' => $order_info['order_sn'], 'order_id' => $order_info['order_id']], true);
        }
