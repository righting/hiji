...
class buy_1Logic
{
    ...
    {
        if (empty($store_cart_list) || !is_array($store_cart_list)) return [$store_cart_list, [], 0];
        ...
        return [$store_cart_list, $store_goods_total];
    }
     ...
     */
    public function parseZhekou(& $goods_list, $store_order_discount = [])
    {
    ...
    {
        if (!C('promotion_allow') || empty($store_goods_total) || !is_array($store_goods_total)) return [[], []];
        ...
        return [$premiums_list, $mansong_rule_list];
    }
    ...
    {
        if (empty($input_rpt_info)) return [];
        $condition                     = [];
        $condition['rpacket_owner_id'] = $member_id;
     ...
     */
    public function parseOrderRpt($store_order_total = [], $rpt_total = 0)
    {
        if (empty($store_order_total) || $rpt_total <= 0) return [$store_order_total, []];
        ...
        return [$store_order_total, $store_rpt_total];
    }
     ...
     * @return array
     */
    public function reParseVoucherList($input_voucher_list = [], $store_goods_total = [], $member_id)
    {
        if (empty($input_voucher_list) || !is_array($input_voucher_list)) return [];
    ...
    public function getOfflineGoodsPay($buy_list)
    {
        //以支付方式为下标，存放购买商品
        $buy_goods_list = [];
        $offline_pay    = Model('payment')->getPaymentOpenInfo(['payment_code' => 'offline']);
        if ($offline_pay) {
    ...
    {
        if (!is_array($store_goods_total) || !is_array($store_freight_total) || !is_array($store_final_goods_total)) return [];
        $store_promotion_total = [];
        foreach ($store_goods_total as $store_id => $goods_total) {
    ...
    {
        if (empty($store_goods_total) || !is_array($store_goods_total)) return [[], []];
        ...
        $condition  = ['store_id' => ['in', array_keys($store_goods_total)]];
     ...
     */
    public function getStoreFreightList($buy_list = [], $free_freight_sid_list)
    {
        //定义返回数组
        $return = [];
    ...
    public function calcStoreFreight($freight_list, $city_id)
    {
        ...
        return [$return_list, $no_send_tpl_ids];
    }
     ...
     */
    public function appendPremiumsToCartList($store_cart_list, $store_premiums_list = [], $store_mansong_rule_list = [], $member_id)
    {
        ...
        foreach ($store_cart_list as $store_id => $cart_list) {
            foreach ($cart_list as $cart_info) {
                ...
                if (!is_array($store_premiums_list)) $store_premiums_list = [];
                if (!array_key_exists($store_id, $store_premiums_list)) $store_premiums_list[$store_id] = [];
                $zenpin_info = [];
        ...
        return [$store_cart_list, $goods_buy_quantity, $store_mansong_rule_list];
    }
    ...
    public function rcbPay($order_list, $input, $buyer_info)
    {
        ...
        foreach ($order_list as $key => $order_info) {
            ...
            if ($available_rcb_amount >= $order_amount) {
                ...
                $result                           = $model_order->editOrder($data_order, ['order_id' => $order_info['order_id']]);
                if (!$result) {
    ...
    public function editGoodsNum($buy_items)
    {
        foreach ($buy_items as $goods_id => $buy_num) {
            $data   = ['goods_storage' => ['exp', 'goods_storage-' . $buy_num], 'goods_salenum' => ['exp', 'goods_salenum+' . $buy_num]];
            $result = Model('goods')->editGoods($data, ['goods_id' => $goods_id]);
    ...
    {
        if (!C('promotion_allow') || empty($store_id_array) || !is_array($store_id_array)) return [];
        $model_mansong     = Model('p_mansong');
        $mansong_rule_list = [];
    ...
    {
        if (empty($store_id_array) || !is_array($store_id_array)) return [];

        //定义返回数组
        $store_free_freight_active = [];

        //如果商品金额未达到免运费设置下线，则需要计算运费
        $condition  = ['store_id' => ['in', $store_id_array]];
        $store_list = Model('store')->getStoreOnlineList($condition, null, '', 'store_id,store_free_price');
        foreach ($store_list as $store_info) {
            $limit_price = floatval($store_info['store_free_price']);
            if ($limit_price > 0) {
     ...
     * @return array
     */
    public function getReciverAddr($address_info = [])
    {
        ...
        return [$reciver_str, $reciver_name, $reciver_info['mob_phone']];
    }
    ...
    public function createInvoiceData($invoice_info)
    {
        ...
        return !empty($inv) ? serialize($inv) : serialize([]);
    }
    ...
    public function extendInPay($order_list)
    {
        ...
        if ($order_list[0]['order_type'] == 2) {
            ...
            if ($order_info['order_state'] == ORDER_STATE_PAY) {
                ...
                //更新预定人数
                $order_goods_info = Model('order')->getOrderGoodsInfo(['order_id' => $order_info['order_id']], 'goods_id', 'rec_id asc');
                $update           = Model('goods')->editGoods(['book_buyers' => ['exp', 'book_buyers+1']], ['goods_id' => $order_goods_info['goods_id']]);
                if (!$update) {
     ...
     * @param array $goods_info
     */
    public function getGroupbuyInfo(& $goods_info = [])
    {
     ...
     */
    public function getMbSoleInfo(& $goods_info = [])
    {
     ...
     * @param unknown $goods_info
     */
    public function getBookInfo(& $goods_info = [])
    {
    ...
    private function _getBundlingCartList(& $cart_list)
    {
        ...
        foreach ($cart_list as $key => $cart_info) {
            ...
            $cart_list[$key]['state']         = true;
            $cart_list[$key]['storage_state'] = true;
            $bl_info                          = $model_bl->getBundlingInfo(['bl_id' => $cart_info['bl_id']]);
            ...
            //取得优惠套装商品列表
            $cart_list[$key]['bl_goods_list'] = $model_bl->getBundlingGoodsList(['bl_id' => $cart_info['bl_id']]);
     ...
     */
    private function _getEachGoodsStorageQuantity($store_cart_list, $store_premiums_list = [])
    {
        if (empty($store_cart_list) || !is_array($store_cart_list)) return [];
        $goods_storage_quangity = [];
    ...
    private function _parseMansongRuleDesc($rule_info)
    {
        ...
        if (!empty($rule_info['mansong_goods_name']) && !empty($rule_info['goods_storage'])) {
            if (APP_ID == 'mobile') {
                ...
                $goods_desc = " 送<a href='" . urlShop('goods', 'index', ['goods_id' => $rule_info['goods_id']]) . "' title='{$rule_info['mansong_goods_name']}' target='_blank'>[赠品]</a>";
                $goods_desc .= '<a class="ncc-store-gift" title="' . $rule_info['mansong_goods_name'] . '" target="_blank" href="' . urlShop('goods', 'index', ['goods_id' => $rule_info['goods_id']]) . '"><img alt="' . $rule_info['mansong_goods_name'] . '" src="' . cthumb($rule_info['goods_image'], 60) . '"></a>';
            }
        } else {
            $goods_desc = '';
        }
