...
class connect_apiLogic
{
    ...
    public function smsRegister($phone, $captcha, $password, $client)
    {
        if ($this->check_captcha($phone, $captcha)) {
            if (C('sms_register') != 1) {
                ...
                return ['state' => 0, 'msg' => '系统没有开启手机注册功能'];
            }
            ...
            //output_error($member);
            if ($result) {
                $member = $model_member->getMemberInfo(['member_mobile' => $phone]);
                $key    = '';// $model_member->_get_token($member['member_id'],$member['member_name'],$client);
                return ['state' => 1, 'username' => $member_name, 'key' => $key];
            } else {
                //output_error('注册失败');
                return ['state' => 0, 'msg' => '注册失败', $member];
            }
        }
    ...
    public function smsPassword($phone, $captcha, $password, $client)
    {
        if ($this->check_captcha($phone, $captcha)) {
            if (C('sms_password') != 1) {
                ...
                return ['state' => 0, 'msg' => '系统没有开启手机找回密码功能'];
            }
            $condition                = [];
            ...
            if (empty($sms_log) || ($sms_log['add_time'] < TIMESTAMP - 1800)) {//半小时内进行验证为有效
                ...
                return ['state' => 0, 'msg' => '动态码错误或已过期，重新输入'];
            }
            $model_member = Model('member');
            $member       = $model_member->getMemberInfo(['member_mobile' => $phone]);//检查手机号是否已被注册
            if (!empty($member)) {
    ...
    {
        if (C('sms_register') == 1) {
            return ['connect_sms_reg' => 1];
        } else {
            return ['connect_sms_reg' => 0];
        }
    }
