...
class paymentLogic
{
     ...
     *
     */
    public function getPointOrderInfo($pay_sn, $member_id = null)
    {
        $model_order                = Model('pointorder');
        $condition                  = [];
        $condition['point_ordersn'] = $pay_sn;
        $order_pay_info             = $model_order->table(points_order)->where($condition)->find();
        if (empty($order_pay_info)) {
            return callback(false, '该支付单不存在');
        }
    ...
    {
        if (in_array($payment_code, ['offline', 'predeposit']) || empty($payment_code)) {
