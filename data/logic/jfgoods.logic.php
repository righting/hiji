...
class jfgoodsLogic
{
    ...
    public function saveGoods($param, $store_id, $store_name, $store_state, $seller_id, $seller_name, $bind_all_gc)
    {
        ...
        // 验证是否设置了分销属性
        $distribution_type       = intval($param['distribution']);
        $distribution_type_group = $model_goods->returnDistributionGroup();
        if (!in_array($distribution_type, array_keys($distribution_type_group))) {
            //return callback(false, '请设置正确的分销模式。');
        }

        // 三方店铺验证是否绑定了该分类
        if (!checkPlatformStoreBindingAllGoodsClass($store_id, $bind_all_gc)) {
            $where                                    = [];
            $where['class_1|class_2|class_3|class_4'] = $gc_id;
        ...
        // 佣金比例和佣金金额的计算
        // 佣金比例
        $commission_rate   = intval($param['commission_rate']);
        $commission_amount = 0;
        if ($commission_rate >= 100 || $commission_rate < 0) {
            return callback(false, '请输入正确的佣金比例。只能为0-99之间的整数');
        }
        $base_price = floatval($param["g_price"]);
        // 如果当前商品可以分销，那么佣金比例必须大于0
        if (in_array($distribution_type, [$model_goods::DISTRIBUTION_EXCLUSIVE, $model_goods::DISTRIBUTION_WHOLE])) {
            if ($commission_rate == 0) {
                //return callback(false, '请至少提供 1% 佣金作为分销报酬');
            }
            // 佣金金额
            if ($commission_rate > 0) {
                $commission_amount = round(bcdiv(bcmul($commission_rate, floatval($param["g_price"])), 100, 2), 2);
                // 保底价（扣除佣金后结算给发布商品的用户的金额）
                $base_price = bcsub(floatval($param["g_price"]), $commission_amount, 2);
            }
        }
        // 是否是分销的其他店铺的商品
        $is_distribution_goods          = 2;
        $param['commission_rate']       = $commission_rate;
        $param['commission_amount']     = $commission_amount;
        $param['base_price']            = $base_price;
        $param['is_distribution_goods'] = $is_distribution_goods;


        // 根据参数初始化通用商品数据
        $common_array = $this->_initCommonGoodsByParam($param, $store_id, $store_name, $store_state);
        // 生成通用商品返回通用商品编号（将商品存入 goods_common 表中）
        Db::beginTransaction();
        try {
            ...
            // 生成商品返回商品ID(SKU)数组(将商品存入 goods 表中)
...
//            QueueClient::push('createGoodsQRCode', array('store_id' => $store_id, 'goodsid_array' => $goodsid_array));
        ...
        if (isset($param['starttime'])) {
            ...
            if ($selltime > TIMESTAMP) {
                Model('cron')->addCron(['exetime' => $selltime, 'exeid' => $common_id, 'type' => 1], true);
            }
        }

        //商品加入消费者保障服务更新队列
        Model('cron')->addCron(['exetime' => TIMESTAMP, 'exeid' => $common_id, 'type' => 9], true);
    ...
    public function updateGoods($param, $store_id, $store_name, $store_state, $seller_id, $seller_name, $bind_all_gc)
    {
        ...
        // 清理商品数据
        $model_goods->delGoods(['goods_id' => ['not in', $goodsid_array], 'goods_commonid' => $common_id, 'store_id' => $store_id]);
        // 清理商品图片表
        $model_goods->delGoodsImages(['goods_commonid' => $common_id, 'color_id' => ['not in', $colorid_array]]);
        // 更新商品默认主图
        $default_image_list = $model_goods->getGoodsImageList(['goods_commonid' => $common_id, 'is_default' => 1], 'color_id ,goods_image');
        if (!empty($default_image_list)) {
            foreach ($default_image_list as $val) {
                $model_goods->editGoods(['goods_image' => $val['goods_image']], ['goods_commonid' => $common_id, 'color_id' => $val['color_id']]);
            }
        }
        ...
        if ($common_array['is_virtual'] == 1) {
            ...
            QueueClient::push('clearSpecialGoodsPromotion', ['goods_commonid' => $common_id, 'goodsid_array' => $goodsid_array]);
        } else {
            // 更新商品促销价格
            QueueClient::push('updateGoodsPromotionPriceByGoodsCommonId', $common_id);
        }

        $return = $model_goods->editGoodsCommon($common_array, ['goods_commonid' => $common_id, 'store_id' => $store_id]);
        if (!$return) {
            return callback(false, '商品编辑失败');
        }
        ...
        // 生成商品二维码
        if (!empty($goodsid_array)) {
            QueueClient::push('createGoodsQRCode', ['store_id' => $store_id, 'goodsid_array' => $goodsid_array]);
        }
    ...
    private function _initCommonGoodsByParam($param, $store_id, $store_name, $store_state)
    {
        ...
        $common_array['gc_id_4']                = intval($goods_class['gc_id_4']) == 0 ? '' : intval($goods_class['gc_id_4']);
        ...
        $common_array['sup_id']                 = 1;
        $common_array['is_own_shop']            = in_array($store_id, Model('store')->getOwnShopIds()) ? 1 : 0;
        // 是否支持分销
        $common_array['distribution_type'] = intval($param['distribution']) > 0 ? intval($param['distribution']) : 1;
        // 是否新品
        $is_new_arrival = 2;
        if (isset($param['is_new_arrival']) && in_array(intval($param['is_new_arrival']), [1, 2])) {
            $is_new_arrival = 1;
        }
        $common_array['is_new_arrival'] = $is_new_arrival;
        // 佣金比例
        $common_array['commission_rate'] = $param['commission_rate'];
        // 佣金金额
        $common_array['commission_amount'] = $param['commission_amount'];
        // 是否是分销的其他店铺的商品
        $common_array['is_distribution_goods'] = $param['is_distribution_goods'];
        // 保底价
        $common_array['base_price'] = $param['base_price'];
    ...
    private function _getStoreClassArray($sgcate_id, $store_id)
    {
        ...
        if (!empty($sgcate_id)) {
            ...
            $store_goods_class = Model('store_goods_class')->getStoreGoodsClassList(['store_id' => $store_id, 'stc_id' => ['in', $sgcate_id_arr], 'stc_state' => '1']);
    ...
    private function _addGoods($param, $common_id, $common_array)
    {
        ...
        if (is_array($param['spec'])) {
            foreach ($param['spec'] as $value) {
                ...
                $goods['goods_costprice']       = $value['goods_costprice'] == 0 ? $common_array['goods_costprice'] : $value['goods_costprice'];
            ...
            $goods['color_id']              = 0;
            // 使用海吉币数量
            $goods['goods_hjb'] = $param['g_hjb'];
            //使用积分数量
            $goods['goods_integral'] = $param['g_integral'];

            $goods['goods_body'] = $param['g_body'];
        }
        Db::beginTransaction();
        try {
            $goods_id = $model_goods->addGoods($goods);
            $model_type->addGoodsType($goods_id, $common_id, ['cate_id' => $param['cate_id'], 'type_id' => $param['type_id'], 'attr' => $param['attr']]);
            Db::commit();
        } catch (Exception $e) {
    ...
    private function _editGoods($param, $common_id, $common_array, $store_id)
    {
        ...
        $model_goods = Model('jfgoods');
        $model_type  = Model('type');
        $model_type->delGoodsAttr(['goods_commonid' => $common_id]);
        if (is_array($param['spec'])) {
            foreach ($param['spec'] as $value) {
                $goods      = $this->_initGoodsByCommonGoods($common_id, $common_array);
                $goods_info = $model_goods->getGoodsInfo(['goods_id' => $value['goods_id'], 'goods_commonid' => $common_id, 'store_id' => $store_id], 'goods_id');
                if (!empty($goods_info)) {
                    ...
                    if ($common_array['is_virtual'] == 1) {
                        $goods['have_gift'] = 0;
                        Model('goods_gift')->delGoodsGift(['goods_id' => $goods_id]);
                        Model('goods_fcode')->delGoodsFCode(['goods_id' => $goods_id]);
                    }
            ...
            $goods      = $this->_initGoodsByCommonGoods($common_id, $common_array);
            $goods_info = $model_goods->getGoodsInfo([$goods_spec_field_name => serialize(null), 'goods_commonid' => $common_id, 'store_id' => $store_id], 'goods_id');
            if (!empty($goods_info)) {
                $goods_id                     = $goods_info['goods_id'];
                $goods['goods_name']          = $common_array['goods_name'];
                $goods['goods_price']         = $common_array['goods_price'];
        ...
        return ['goodsid_array' => $goodsid_array, 'colorid_array' => array_unique($colorid_array)];
    }
    ...
    private function _initGoodsByCommonGoods($common_id, $common_array)
    {
        ...
        $goods['gc_id_4']                = intval($common_array['gc_id_4']) == 0 ? '' : intval($common_array['gc_id_4']);
        ...
        $goods['distribution_type']      = $common_array['distribution_type'];
        // 是否新品
        $is_new_arrival = 2;
        if (isset($common_array['is_new_arrival']) && in_array(intval($common_array['is_new_arrival']), [1, 2])) {
            $is_new_arrival = 1;
        }
        $goods['is_new_arrival'] = $is_new_arrival;
        // 佣金比例
        $goods['commission_rate'] = $common_array['commission_rate'];
        // 佣金金额
        $goods['commission_amount'] = $common_array['commission_amount'];
        // 是否是分销的其他店铺的商品
        $goods['is_distribution_goods'] = $common_array['is_distribution_goods'];
        // 保底价
        $goods['base_price'] = $common_array['base_price'];
        return $goods;
    }
    ...
    public function uploadGoodsImageToAliOss($image_name, $store_id, $album_limit)
    {
        // 判断图片数量是否超限
        $model_album = Model('album');
        if ($album_limit > 0) {
            $album_count = $model_album->getCount(['store_id' => $store_id]);
            if ($album_count >= $album_limit) {
                return callback(false, L('store_goods_album_climit'));
            }
        }

        $class_info = $model_album->getOne(['store_id' => $store_id, 'is_default' => 1], 'album_class');
        // 上传图片
        ...
        $data                = [];
        $data ['thumb_name'] = $this->get_img_cthumb($img_path, 240);
        $data ['name']       = $img_path;

        return callback(true, '', $data);
    }

    public function get_img_cthumb($img_path, $thumb)
    {
        $uri = $img_path . '?x-oss-process=image/resize,m_pad,h_' . $thumb . ',w_' . $thumb . ',color_FFFFFF';
        return $uri;
    }
    ...
    public function editSaveImage($img, $common_id, $store_id, $seller_id, $seller_name)
    {
        ...
        $model_goods = Model('jfgoods');
        // 删除原有图片信息
        $model_goods->delGoodsImages(['goods_commonid' => $common_id, 'store_id' => $store_id]);
        // 保存
        $insert_array = [];
    ...
    {
        $return = Model('jfgoods')->editProducesOnline(['goods_commonid' => ['in', $commonid_array], 'store_id' => $store_id]);
        if ($return) {
            // 添加操作日志
    ...
    public function goodsUnShow($commonid_array, $store_id, $seller_id, $seller_name)
    {
        $model_goods             = Model('jfgoods');
        $where                   = [];
        $where['goods_commonid'] = ['in', $commonid_array];
        $where['store_id']       = $store_id;
        $return                  = Model('jfgoods')->editProducesOffline($where);
        if ($return) {
            // 更新优惠套餐状态关闭
    ...
    {
        $return = Model('jfgoods')->delGoodsNoLock(['goods_commonid' => ['in', $commonid_array], 'store_id' => $store_id]);
        if ($return) {
            // 添加操作日志
