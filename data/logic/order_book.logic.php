...
class order_bookLogic
{
     ...
     * @throws Exception
     */
    public function buy($order_data = [], $post_data = [])
    {
    ...
    {
        $book_list = Model('order_book')->getOrderBookList(['book_order_id' => $order_info['order_id']]);
            ...
            if (empty($book_list[0]['book_pay_time'])) {
                ...
                if ($book_list[0]['book_end_time'] > TIMESTAMP) {
                    ...
                    if ($order_info['order_state'] == ORDER_STATE_CANCEL) {
                        ...
                        $book_list[0]['book_operate'] = '<a class="ncm-btn ncm-btn-orange fr mr15" href="index.php?controller=buy&action=pay&pay_sn=' . $pay_sn . '">付款</a>';
                ...
                if (empty($book_list[1]['book_pay_time'])) {
                    ...
                    if ($book_list[1]['book_end_time'] > TIMESTAMP) {
                        ...
                        if ($order_info['order_state'] == ORDER_STATE_CANCEL) {
                            ...
                            $book_list[1]['book_operate'] = '<a class="ncm-btn ncm-btn-orange fr mr15" href="index.php?controller=buy&action=pay&pay_sn=' . $pay_sn . '">付款</a>';
    ...
    public function changeBookOrderReceivePay($order_info, $post)
    {
        ...
        if ($book_list[0]['book_step']) {
            ...
            if (empty($book_list[0]['book_pay_time'])) {
                ...
                Model('goods')->editGoods(['book_buyers' => ['exp', 'book_buyers+1']], ['goods_id' => $order_goods_info['goods_id']]);
            } else {
    ...
    public function changeOrderStateCancel($order_info, $role, $user = '', $msg = '')
    {
        try {
            ...
            if (!empty($order_info['real_pay'])) {
                ...
                $update = $model_order->editOrder(['shipping_fee' => 0], ['order_id' => $order_info['order_id']]);
                if (!$update) {
                    throw new Exception('保存失败');
                }
            }
     ...
     * 取得订单订金金额
     */
    public function getDepositAmount($order_info = [])
    {
        if ($order_info['order_type'] != 2) return 0;
        $condition                  = [];
        $condition['book_order_id'] = $order_info['order_id'];
        $condition['book_step']     = ['in', [0, 1]];
        $book_info                  = Model('order_book')->getOrderBookInfo(['book_order_id' => $order_info['order_id']], '', 'book_id asc');
