...
class buyLogic
{
    ...
    public function buyStep1($cart_id, $ifcart, $member_id, $store_id, $jjg = null, $orderdiscount = 0, $member_level = 0, $ifchain = null)
    {
        ...
        //处理海豚主场积分可用升级
        $points_sum = Model('user_level')->upMaxGradeTotalPoints($_SESSION['member_id']);
        if ($points_sum) {
            $total                = $this->_countOrderPointsByHs($result['data']['store_cart_list']);
            $result['data']['Hs'] = ['status' => 1, 'points' => $total, 'points_sum' => $points_sum];
        }
     ...
     */
    public function getCartList($cart_id, $member_id, $jjg = null, $orderdiscounts = [])
    {
        ...
        $goods_id_arr = [-1];
        if (!empty($buy_items)) {
            foreach ($buy_items as $goods_id_v) {
                $goods_id_arr[] = $goods_id_v[0];
            }
        }
        //购物车列表
        $condition = ['cart_id' => ['in', $goods_id_arr], 'buyer_id' => $member_id];
     ...
     */
    public function getGoodsList($cart_id, $member_id, $store_id, $orderdiscounts = [])
    {
        ...
        $goods_id     = $buy_items[0][0];
        $quantity     = $buy_items[0][1];
        $referral_key = $buy_items[0][2];
        //商品信息[得到最新商品属性及促销信息]
        ...
        return callback(true, '', ['goods_list' => $goods_list, 'referral_key' => $referral_key, 'store_cart_list' => $store_cart_list, 'is_book' => $goods_info['is_book']]);
    }
    ...
    public function getBuyStep1Data($member_id, $data, $orderdiscount = 0)
    {
        ...
        if (!$data['is_book']) {
            ...
            $result['store_premiums_list'] = $result['store_mansong_rule_list'] = $result['store_voucher_list'] = $result['rpt_list'] = [];
        }
        ...
        //输出用户默认收货地址
        $result['address_info'] = Model('address')->getDefaultAddressInfo(['member_id' => $member_id]);
        ...
        $result['referral_key'] = $data['referral_key'];
    ...
    public function delCart($ifcart, $member_id, $cart_ids)
    {
        ...
        if (preg_match('/^[\d,]+$/', $cart_id_str)) {
            Logic('queue')->delCart(['buyer_id' => $member_id, 'cart_ids' => $cart_ids]);
        }
    }
    ...
    public function changeChain($chain_id = 0, $product = '')
    {
        ...
        $condition             = [];
        $condition['goods_id'] = ['in', array_keys($product)];
    ...
    {
        $fcode_info = Model('goods_fcode')->getGoodsFCode(['goods_id' => $goods_id, 'fc_code' => $fcode, 'fc_state' => 0]);
        if ($fcode_info) {
    ...
    public function bdCheckout($post)
    {
        $is_bd = intval($post['is_bd']);
        // 如果选择补单
        if ($is_bd == 1) {
            $refund_id_arr = $post['refund_sn'];
            if (empty($refund_id_arr)) {
                throw new Exception('补单信息错误');
            }
            $user_id             = $_SESSION['member_id'];
            $refund_return_model = Model('refund_return');
            // 获取要补的订单的退货/退款信息
            $refund_return_list_where['buyer_id']  = $user_id;
            $refund_return_list_where['refund_id'] = ['in', $refund_id_arr];
            $refund_return_list                    = $refund_return_model->where($refund_return_list_where)->field('refund_id,order_id,order_sn,goods_id,order_goods_id')->select();
            $this->_return_refund_data             = $refund_return_list; // 补单信息
            // 获取要补的订单产生的总利润
            ...
            if (empty($money_arr)) {
                throw new Exception('当前没有需要补单的订单');
            }
            // 要补的订单产生的税后总利润
            $total_money = array_sum($money_arr);
            // 根据提交的数据获取当前订单可以产生的总利润

            // 获取当前购买的商品id和购买的数量
            $now_goods_id_arr = $this->getPostGoodsIdForOrder($post);
            // 获取这些商品的成本价和出售价，两者的差减去平台当前的税率就是每件商品可产生的利润
            $goods_model    = Model('goods');
            $now_goods_list = $goods_model->where(['goods_id' => ['in', array_column($now_goods_id_arr, 0)]])->field('goods_id,goods_price,goods_costprice')->select();
            if (empty($now_goods_list)) {
            ...
            // 获取每个商品id对应的数量
            $goods_id_and_count = array_combine(array_column($now_goods_id_arr, 0), array_column($now_goods_id_arr, 1));
            foreach ($now_goods_list as $new_goods_info) {
                $this_goods_count                                  = isset($goods_id_and_count[$new_goods_info['goods_id']]) ? $goods_id_and_count[$new_goods_info['goods_id']] : 0;
                $this_goods_profit                                 = bcmul(bcsub($new_goods_info['goods_price'], $new_goods_info['goods_costprice'], 2), $this_goods_count, 2);
                $this_goods_tax                                    = bcmul(bcdiv(bcmul($this_goods_profit, $tax_rate, 2), 100, 2), $this_goods_count, 2);
                $new_goods_profit_arr[$new_goods_info['goods_id']] = bcmul(bcsub($this_goods_profit, $this_goods_tax), $this_goods_count, 2);
            }
            // 当前订单可产生的税后利润
            $new_goods_total_money = array_sum($new_goods_profit_arr);
            // 如果当前订单可以产生的总利润没有达到需要补单的订单的总利润，那么提示订单利润不足
            $diff_money = bcsub($new_goods_total_money, $total_money, 2);
            if ($diff_money < 0) {
                throw new Exception('当前订单产生的利润未达到补单利润！满足条件还需要产生 ' . abs($diff_money) . ' 元的商品');
            }
        }
    }
    ...
    private function _createOrderStep1()
    {
        ...
        if (!$post['chain']['id']) {
            ...
            $chain_info = Model('chain')->getChainInfo(['chain_id' => intval($post['chain']['id'])]);
            if ($chain_info) {
                $input_address_info              = [];
...
//        $input_if_vat = $this->buyDecrypt($post['vat_hash'], $this->_member_info['member_id']);
        ...
        if (!$post['chain']['id']) {
            ...
            $input_if_offpay_batch = [];
        }

        //付款方式:在线支付/货到付款(online/offline)
        if (!in_array($post['pay_name'], ['online', 'offline', 'chain'])) {
    ...
    private function _createOrderStep2()
    {
        ...
        $cart_id_arr     = array_column($input_buy_items, 0);
        $input_is_book   = $this->_order_data['input_is_book'];

        if ($post['ifcart']) {
            //购物车列表
            $model_cart = Model('cart');
            $condition  = ['cart_id' => ['in', $cart_id_arr], 'buyer_id' => $this->_member_info['member_id']];
            $cart_list  = $model_cart->listCart('db', $condition);
...
//            $goods_id = key($input_buy_items);
            $goods_id = $input_buy_items[0][0];
            $quantity = $input_buy_items[0][1];
            ...
            $goods_info['referral_key'] = $post['referral_key'];
            //预定不享受任何优惠
        ...
        // 邀请码
        $this->_order_data['referral_key'] = $post['referral_key'];
        //验证门店自提
    ...
    private function _createOrderStep3()
    {
        ...
        if (!$input_is_book) {
            ...
            $store_promotion_total   = $store_mansong_rule_list = $input_voucher_list = $input_rpt_info = $store_rpt_total = [];
            $goods_buy_quantity      = [$goods_list[0]['goods_id'] => $goods_list[0]['goods_num']];
        ...
        //验证升级积分是否足够
        $Hs = $this->_countOrderPointsByHs($store_cart_list);
        if (array_sum($Hs) < intval($_POST['deduction_money'])) {
            throw new Exception('抱歉，你的积分数据错误，请重新购买');
        }
        //保存数据
    ...
    private function _createOrderStep4()
    {
        ...
        if ($input_pay_name == 'chain' && $input_chain_id) {
            $store_pay_type_list = [key($store_cart_list) => 'chain'];
        } else {
...
//             $should_goods_total = $store_final_order_total[$store_id]-$store_freight_total[$store_id]+$promotion_total;
			...
			foreach ($goods_list as $goods_info) {
				if ($goods_info['referral_key']) {
					$tmp = $goods_model->getGoodsReferralKey($goods_info['referral_key']);
					$referral_member_id = $tmp['member_id'];
				}
			}
            ...
            $order['deduction_money'] = $_POST['deduction_money'];
            $order['referral_member_id'] = $referral_member_id;
            ...
            if (is_array($store_mansong_rule_list[$store_id])) {
                $order_common['promotion_info'][] = ['满即送', $store_mansong_rule_list[$store_id]['desc']];
            }

            //平台红包值
            if ($store_rpt_total[$store_id]) {
                $order_common['promotion_info'][] = ['平台红包', sprintf('使用%s元红包 编码：%s', $store_rpt_total[$store_id], $input_rpt_info['rpacket_code'])];
            }
...
//             }
            ...
            //代金券
            if (isset($input_voucher_list[$store_id])) {
                $order_common['promotion_info'][] = ['店铺代金券', sprintf('使用%s元代金券 编码：%s', $input_voucher_list[$store_id]['voucher_price'], $input_voucher_list[$store_id]['voucher_code'])];
            }
            ...
            foreach ($goods_list as $goods_info) {
                ...
                $goods_invit = Model('goods')->getGoodsInfo(['goods_id' => $goods_info['goods_id']]);
                if (!intval($goods_info['bl_id'])) {
                    ...
                    $order_goods[$i]['referral_key'] = $goods_info['referral_key'];
                    if ($_POST['deduction_money'] > 0 && in_array($goods_info['goods_id'], explode(',', $_POST['htzc_goods']))) {
                        $order_goods[$i]['is_deduction'] = 1;//订单商品被抵扣用于升级
                    } else {
                        $order_goods[$i]['is_deduction'] = 0;
                    }
                    ...
                    //记录消费者保障服务
                    $contract_itemid_arr                 = $goods_info['contractlist'] ? array_keys($goods_info['contractlist']) : [];
                    ...
                    foreach ($goods_info['bl_goods_list'] as $bl_goods_info) {
                        $bl_goods_invit                   = Model('goods')->getGoodsInfo(['goods_id' => $bl_goods_info['goods_id']]);
                        $order_goods[$i]['order_id']      = $order_id;
                        ...
                        $order_goods[$i]['commis_rate']   = 0;        // @TODO 15.这里的佣金比例需要修改，现在默认全部是0
                        $order_goods[$i]['invite_rates']  = $bl_goods_invit['invite_rate'];
                        $order_goods[$i]['gc_id']         = $bl_goods_info['gc_id'];

                        //记录消费者保障服务
                        $contract_itemid_arr                 = $bl_goods_info['contractlist'] ? array_keys($bl_goods_info['contractlist']) : [];
                        $order_goods[$i]['goods_contractid'] = $contract_itemid_arr ? implode(',', $contract_itemid_arr) : '';
            ...
            foreach ((array)$jjgValidStoreSkus[$store_id] as $goods_info) {
                ...
                $ml_goods_invit = Model('goods')->getGoodsInfo(['goods_id' => $goods_info['id']]);
                ...
                $order_goods[$i]['commis_rate'] = $goods_info['commission_rate'];;
            ...
            if ($store_pay_type_list[$store_id] == 'offline') {
                $notice_list['new_order'][$order['store_id']] = ['order_sn' => $order['order_sn']];
            }
        }
        ...
        // 获取本次下次生成的所有订单
        $this_order_id_arr = array_keys($order_list);
        // 检查本次下单是否是补单
        $_return_refund_data = $this->_return_refund_data;

        if (!empty($_return_refund_data)) {
            $setting_model          = Model('setting');
            $set_field_arr          = $setting_model->where(['name' => ['in', ['annual_fee_gc_id', 'USER_CONSUMPTION_BONUS_RATIO', 'PLATFORM_TAX_RATE']]])->select();
            $set_field_arr_for_name = array_combine(array_column($set_field_arr, 'name'), array_column($set_field_arr, 'value'));
            // 获取平台税率
            $tax_rate = $set_field_arr_for_name['PLATFORM_TAX_RATE'];
            // 每个退货退款id可以对应多个订单id
            $_return_refund_log_arr = [];
            $created_at             = date('Y-m-d H:i:s');
            foreach ($this_order_id_arr as $this_order_id) {
                foreach ($_return_refund_data as $_return_refund_data_value) {
                    $this_key                                                = $this_order_id . '_' . $_return_refund_data_value['refund_id'];
                    $_return_refund_log_arr[$this_key]['refund_id']          = $_return_refund_data_value['refund_id'];
                    $_return_refund_log_arr[$this_key]['old_order_id']       = $_return_refund_data_value['order_id'];
                    $_return_refund_log_arr[$this_key]['old_order_sn']       = $_return_refund_data_value['order_sn'];
                    $_return_refund_log_arr[$this_key]['old_goods_id']       = $_return_refund_data_value['goods_id'];
                    $_return_refund_log_arr[$this_key]['old_order_goods_id'] = $_return_refund_data_value['order_goods_id'];
                    $_return_refund_log_arr[$this_key]['order_id']           = $this_order_id;
                    $_return_refund_log_arr[$this_key]['created_at']         = $created_at;
                    $_return_refund_log_arr[$this_key]['tax_rate']           = $tax_rate;
                }
            }
            $refund_return_bd_log_model = Model('refund_return_bd_log');
            $refund_return_bd_log_model->insertAll(array_values($_return_refund_log_arr));
        }
    }
    ...
    private function _createOrderStep6()
    {
        ...
        //删除购物车中的商品
        $cart_ids = array_column($input_buy_items, 0);
        $this->delCart($ifcart, $this->_member_info['member_id'], $cart_ids);
        ...
        if ($input_chain_id) {
            ...
            if ($order_info['payment_code'] == 'chain') {
                ...
                $result = Model('order')->editOrder(['chain_code' => $_code], ['order_id' => $order_info['order_id']]);
                if (!$result) {
    ...
    private function _parseItems($cart_id)
    {
        ...
        if (is_array($cart_id)) {
            foreach ($cart_id as $value) {
                $buy_items[] = explode('|', $value);
                /*if (preg_match_all('/^(\d{1,10})\|(\d{1,6})$/', '100026|1', $match)) {
     ...
     * @return int
     */
    private function _countOrderPointsByHs($data = [])
    {
        foreach ($data as $val) {
            $goods_ids  = array_column($val, 'goods_id');
            $cart_goods = array_column($val, 'goods_total', 'goods_id');
        }
        $model_gc = Model('goods_class');
        if (!empty($goods_ids)) {
            $goods_arrs = Model('goods')->field('goods_id')->where(['goods_id' => ['in', implode(',', $goods_ids)], 'gc_id_1' => $model_gc::HT_CATEGORY_TYPE])->select();
            $goods_arrs = array_column($goods_arrs, 'goods_id');
        }
        $htzc_goods = [];
        foreach ($goods_arrs as $v) {
            $htzc_goods[$v] = $cart_goods[$v];
        }
        return $htzc_goods;
    }
}
