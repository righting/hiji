...
class queueLogic
{
    ...
    public function addPoint($member_info)
    {
        ...
        $points_model->savePointsLog('login', ['pl_memberid' => $member_info['member_id'], 'pl_membername' => $member_info['member_name']], true);
    ...
    public function addExppoint($member_info)
    {
        ...
        $exppoints_model->saveExppointsLog('login', ['exp_memberid' => $member_info['member_id'], 'exp_membername' => $member_info['member_name']], true);
        return callback(true);
    }
    ...
    public function editGroupbuySaleCount($groupbuy_info)
    {
        ...
        $data                 = [];
        $data['buyer_count']  = ['exp', 'buyer_count+1'];
        $data['buy_quantity'] = ['exp', 'buy_quantity+' . $groupbuy_info['quantity']];
        $update               = $model_groupbuy->editGroupbuy($data, ['groupbuy_id' => $groupbuy_info['groupbuy_id']]);
        if (!$update) {
    ...
    public function editVoucherState($voucher_list)
    {
        ...
        foreach ($voucher_list as $store_id => $voucher_info) {
            ...
            if ($update) {
                $update = $model_voucher->editVoucherTemplate(['voucher_t_id' => $voucher_info['voucher_t_id']], ['voucher_t_used' => ['exp', 'voucher_t_used+1']]);
                if ($update) {
                    // 发送用户店铺消息
    ...
    public function editRptState($input_rpt_info, $pay_sn)
    {
        ...
        $update    = $model_rpt->editRedpacket(['rpacket_id' => $input_rpt_info['rpacket_id']], ['rpacket_state' => 2, 'rpacket_order_id' => $pay_sn], $input_rpt_info['rpacket_owner_id']);
        if ($update) {
            $update = $model_rpt->editRptTemplate(['rpacket_t_id' => $input_rpt_info['rpacket_t_id']], ['rpacket_t_used' => ['exp', 'rpacket_t_used+1']]);
            if ($update) {
                $send = new sendMemberMsg();
                // 发送用户店铺消息
    ...
    public function createOrderUpdateStorage($goods_buy_quantity)
    {
        ...
        foreach ($goods_buy_quantity as $goods_id => $quantity) {
            $data                  = [];
            $data['goods_storage'] = ['exp', 'goods_storage-' . $quantity];
            $data['goods_salenum'] = ['exp', 'goods_salenum+' . $quantity];
    ...
    {
        $update = Model('goods_fcode')->editGoodsFCode(['fc_state' => 1], ['fc_id' => $fc_id]);
        if (!$update) {
    ...
    public function delCart($cart)
    {
        ...
        $del = Model('cart')->delCart('db', ['buyer_id' => $cart['buyer_id'], 'cart_id' => ['in', $cart['cart_ids']]]);
        if (!$del) {
    ...
    {
        $update = Model('goods')->editGoodsPromotionPrice(['goods_id' => ['in', $goods_id]]);
        if (!$update) {
    ...
    {
        $update = Model('goods')->editGoodsPromotionPrice(['goods_commonid' => ['in', $goods_commonid]]);
        if (!$update) {
    ...
    {
        // 团购
        Model('groupbuy')->delGroupbuy(['goods_commonid' => $param['goods_commonid']]);
        // 显示折扣
        Model('p_xianshi_goods')->delXianshiGoods(['goods_id' => ['in', $param['goodsid_array']]]);
        // 优惠套装
        Model('p_bundling')->delBundlingGoods(['goods_id' => ['in', $param['goodsid_array']]]);
        // 更新促销价格
        Model('goods')->editGoods(['goods_promotion_price' => ['exp', 'goods_price'], 'goods_promotion_type' => 0], ['goods_commonid' => $param['goods_commonid']]);
        return callback(true);
    }
    ...
    public function delOrderCountCache($order_info)
    {
        ...
        if ($order_info['order_id']) {
            $order_info = $model_order->getOrderInfo(['order_id' => $order_info['order_id']], [], 'buyer_id,store_id');
        }
    ...
    public function sendVrCode($param)
    {
        ...
        $tpl_info          = Model('mail_templates')->getTplInfo(['code' => 'send_vr_code']);
        $data              = [];
    ...
    {
        $dorder_info         = Model('delivery_order')->getDeliveryOrderInfo(['order_id' => $param['order_id']], 'reciver_mobphone');
        $tpl_info            = Model('mail_templates')->getTplInfo(['code' => 'send_pickup_code']);
        $data                = [];
        $data['site_name']   = C('site_name');
    ...
    {
        $tpl_info           = Model('mail_templates')->getTplInfo(['code' => 'send_chain_code']);
        $data               = [];
        $data['site_name']  = C('site_name');
    ...
    public function build_pwdvoucher($t_id)
    {
        ...
        //更新代金券模板
        if ($is_succ) {
            $model_voucher->editVoucherTemplate(['voucher_t_id' => $t_info['voucher_t_id']], ['voucher_t_isbuild' => 1]);
            return callback(true);
        } else {
    ...
    public function build_pwdRedpacket($t_id)
    {
        ...
        //更新红包模板
        if ($is_succ) {
            $model_redpacket->editRptTemplate(['rpacket_t_id' => $t_info['rpacket_t_id']], ['rpacket_t_isbuild' => 1]);
            return callback(true);
        } else {
            return callback(false);
        }
    ...
    public function updateStoreGoodsContract($param)
    {
        ...
        $model_goods = Model('goods');
        //查询店铺商品总数
        $goods_count = $model_goods->getGoodsCount(['store_id' => $store_id]);
        if ($goods_count <= 0) {
            return callback(true);
        }
        $eachnum = 1000;
        for ($i = 0; $i < $goods_count; $i += $eachnum) {//每次查询$eachnum条
            //查询店铺商品
            $goods_list = $model_goods->getGoodsList(['store_id' => $store_id], 'goods_id', '', 'goods_id asc', "$i,$eachnum");
            if (empty($goods_list)) {
                break;
            }
            $goodsid_array = [];
     ...
     * 生成交易快照
     */
    public function createSphot($order_id_list = [])
    {
        $goods_list = Model('order')->getOrderGoodsList(['order_id' => ['in', $order_id_list]], 'rec_id,goods_id');
        if ($goods_list) {
     ...
     * 生成交易快照
     */
    public function createVrSphot($order_info = [])
    {
