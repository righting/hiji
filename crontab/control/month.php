...
class monthControl extends BaseCronControl
{
    ...
    public function indexOp()
    {
        ...
        $is_crontab = $model->getUserBonusStagesCrontabInfo(['frequency' => 1]);
        if ($is_crontab > 0) {
            $this->_dividend_monthly_statistics();// 分红月统计
            $this->_monthly_bonus();// 月分红
            p(date('Y-m-d H:i:s') . '月计划任务执行成功');
            $this->_create_bill();
        }
    }
     ...
     * 分红月统计
     */
    private function _dividend_monthly_statistics()
    {
        $user_bonus_logic = new user_bonusLogic();
        $user_bonus_logic->salesBonusForDay();// 统计销售日分红
        $user_bonus_logic->statisticsSellingStarDayBonus();// 统计销售明星日分红
        $user_bonus_logic->statisticalShareDayBonus();// 统计共享日分红（按月统计） 5%
        $user_bonus_logic->statisticsSupplierReferralBonus();// 统计供应商推荐奖金
        $user_bonus_logic->statisticsBlackDiamondSalesBonus();// 至尊消费月分红
        $user_bonus_logic->statisticsEliteMonthlyBonus();// 统计销售精英月分红
        $user_bonus_logic->statisticsTopSellingMonthlyBonus();// 统计高层销售月分红
        $user_bonus_logic->statisticsConsumptionPension();// 消费资本-消费养老（1,2,3,4,5,6,9,10）
        $user_bonus_logic->statisticsGaragesDream();// 消费资本-车房梦想
        p(date('Y-m-d') . '当月分红统计成功');
    }

    /**
     * 月分红
     */
    private function _monthly_bonus()
    {
        $user_bonus_logic = new user_bonusLogic();
        $user_bonus_logic->supplierReferralBonus();// 供应商推荐奖金
        $user_bonus_logic->blackDiamondSalesBonus();// 至尊消费月分红
        $user_bonus_logic->eliteMonthlyBonus();// 销售精英月分红
        $user_bonus_logic->consumptionPension();// 消费资本-消费养老（1,2,3,4,5,6,9,10）
        $user_bonus_logic->garagesDream();// 消费资本-车房梦想
        p(date('Y-m-d') . '当月分红成功');
    }
    ...
    private function _real_order()
    {
        ...
        $model_bill            = new billModel();
        $order_statis_max_info = $model_bill->getOrderStatisInfo([], 'os_end_date', 'os_month desc');
        //计算起始时间点，自动生成以月份为单位的空结算记录
        if (!$order_statis_max_info) {
            $order_min_info = $model_order->getOrderInfo([], [], 'min(add_time) as add_time');
        ...
        while (($time = strtotime('-' . $i . ' month', $current_time)) >= $start_unixtime) {
            if (date('Ym', $start_unixtime) == date('Ym', $time)) {
                //如果两个月份相等检查库是里否存在
                $order_statis = $model_bill->getOrderStatisInfo(['os_month' => date('Ym', $start_unixtime)]);
                if ($order_statis) {
                    break;
                }
            }
            ...
            $order_bill_info                       = $model_bill->getOrderBillInfo(['os_month' => $os_month], $fileds);
    ...
    private function _create_real_order_bill($data)
    {
        ...
        $store_count = $model_store->getStoreCount([]);
        ...
        for ($i = 0; $i <= $store_count; $i = $i + 300) {
            ...
            $store_list = $model_store->getStoreList([], null, '', 'store_id', "{$i},300");
            if ($store_list) {
                //自动生成以月份为单位的空结算记录
                $data_bill = [];
                foreach ($store_list as $store_info) {
                    ...
                    if (!$model_bill->getOrderBillInfo(['ob_no' => $data_bill['ob_no']])) {
    ...
    private function _calc_real_order_bill($data_bill)
    {
        ...
        $order_info                = $model_order->getOrderInfo($order_condition, [], $fields);
        ...
        //佣金金额
        $order_info               = $model_order->getOrderInfo($order_condition, [], 'count(DISTINCT order_id) as count');
        ...
        //店铺活动费用
        $model_store_cost                = new store_costModel();
        $cost_condition                  = [];
        ...
        $update['ob_state']         = 1;
        return $model_bill->editOrderBill($update, ['ob_no' => $data_bill['ob_no']]);
    }
    ...
    private function _create_vr_order_bill($data)
    {
        ...
        $condition['payment_time'] = [['egt', $data['os_start_date']], ['elt', $data['os_end_date']], 'and'];
        //取出有最终成交订单的店铺ID数量（ID不重复）
