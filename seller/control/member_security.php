...
class member_securityControl extends BaseShopMemberControl
{
    ...
    public function indexOp()
    {
        ...
        Tpl::showpage('member_security/member_security.index');
    }
    ...
    public function send_bind_emailOp()
    {
        ...
        $condition['member_id'] = array('neq', $_SESSION['seller_session_info']['member_id']);
        ...
        $update = $model_member->editMember(array('member_id' => $_SESSION['seller_session_info']['member_id']), $data);
        if (!$update) {
        ...
        $update = $model_member->editMemberCommon($data, array('member_id' => $_SESSION['seller_session_info']['member_id']));
        if (!$update) {
            showDialog('系统发生错误，如有疑问请与管理员联系');
        }
        $uid = base64_encode(encrypt($_SESSION['seller_session_info']['member_id'] . ' ' . $_POST["email"]));
        ...
        $param['user_name'] = $_SESSION['seller_session_info']['seller_name'];
    ...
    public function authOp()
    {
        ...
        if (chksubmit(false, true)) {
            ...
            $member_common_info = $model_member->getMemberCommonInfo(array('member_id' => $_SESSION['seller_session_info']['member_id']));
            ...
            if ($_POST['type'] == 'pd_cash') {
                Tpl::showpage('member_security/predeposit.pd_cash_add');
            } else {
                Tpl::showpage('member_security/member_security.' . $_POST['type']);
            }
            ...
            $member_info = $this->member_info;
            if (!$member_info) {
                $member_info = $model_member->getMemberInfo(array('member_id' => $_SESSION['seller_session_info']['member_id']), 'member_email,member_email_bind,member_mobile,member_mobile_bind');
            }
            ...
            if (($_GET['type'] == 'modify_email' && $member_info['member_email_bind'] == '0') ||
                ...
                Tpl::showpage('member_security/member_security.' . $_GET['type']);
                exit;
            }
            ...
            Tpl::showpage('member_security/member_security.auth');
        }
    ...
    public function send_auth_codeOp()
    {
        ...
        $model_member = Model('member');
        $member_info = $model_member->getMemberInfoByID($_SESSION['seller_session_info']['member_id'], 'member_email,member_mobile');
        ...
        if ($result) {
            ...
            $update = $model_member->editMemberCommon($update_data, array('member_id' => $_SESSION['seller_session_info']['member_id']));
            if (!$update) {
    ...
    public function modify_pwdOp()
    {
        ...
        $update = $model_member->editMember(array('member_id' => $_SESSION['seller_session_info']['member_id']), array('member_passwd' => hj_password_hash($_POST['password'])));
    ...
    public function modify_paypwdOp()
    {
        ...
        $update = $model_member->editMember(array('member_id' => $_SESSION['seller_session_info']['member_id']), array('member_paypwd' => md5($_POST['password'])));
    ...
    {
        $model_member = Model('member');
        $member_info = $model_member->getMemberInfoByID($_SESSION['seller_session_info']['member_id'], 'member_mobile_bind');
        if (chksubmit()) {
            $obj_validate = new Validate();
            $obj_validate->validateparam = array(
            ...
            $condition = array();
            $condition['member_id'] = $_SESSION['seller_session_info']['member_id'];
            ...
            $update = $model_member->editMember(array('member_id' => $_SESSION['seller_session_info']['member_id']), array('member_mobile_bind' => 1));
            if (!$update) {
                showDialog('系统发生错误，如有疑问请与管理员联系');
            }
    ...
    private function profile_menu($menu_type, $menu_key = '')
    {
        ...
        switch ($menu_type) {
            case 'index':
                $menu_array = array(
                    array('menu_key' => 'index', 'menu_name' => '账户安全', 'menu_url' => urlSellerMember('member_security', 'index')),
                );
                break;
            case 'modify_pwd':
                $menu_array = array(
                    array('menu_key' => 'index', 'menu_name' => '账户安全', 'menu_url' => urlSellerMember('member_security', 'index')),
                    array('menu_key' => 'modify_pwd', 'menu_name' => '修改登录密码', 'menu_url' => urlSellerMember('member_security', 'auth', ['type' => 'modify_pwd'])),
                );
                break;
            case 'modify_email':
                $menu_array = array(
                    array('menu_key' => 'index', 'menu_name' => '账户安全', 'menu_url' => urlSellerMember('member_security', 'index')),
                    array('menu_key' => 'modify_email', 'menu_name' => '邮箱验证', 'menu_url' => urlSellerMember('member_security', 'auth', ['type' => 'modify_email'])),
                );
                break;
            case 'modify_mobile':
                $menu_array = array(
                    array('menu_key' => 'index', 'menu_name' => '账户安全', 'menu_url' => urlSellerMember('member_security', 'index')),
                    array('menu_key' => 'modify_mobile', 'menu_name' => '手机验证', 'menu_url' => urlSellerMember('member_security', 'auth', ['type' => 'modify_mobile'])),
                );
                break;
            case 'modify_paypwd':
                $menu_array = array(
                    array('menu_key' => 'index', 'menu_name' => '账户安全', 'menu_url' => urlSellerMember('member_security', 'index')),
                    array('menu_key' => 'modify_paypwd', 'menu_name' => '设置支付密码', 'menu_url' => urlSellerMember('member_security', 'auth', ['type' => 'modify_paypwd'])),
                );
                break;
            case 'pd_cash':
                $menu_array = array(
                    array('menu_key' => 'loglist', 'menu_name' => '账户余额', 'menu_url' => urlSellerMember('predeposit', 'pd_log_list')),
                    // array('menu_key'=>'recharge_list','menu_name'=>'充值明细', 'menu_url'=>'index.php?controller=predeposit&action=index'),
                    array('menu_key' => 'cashlist', 'menu_name' => '余额提现', 'menu_url' => urlSellerMember('predeposit', 'pd_cash_list')),
                    array('menu_key' => 'pd_cash', 'menu_name' => '提现申请', 'menu_url' => urlSellerMember('member_security', 'auth', ['type' => 'pd_cash'])),
                );
                break;
        }
