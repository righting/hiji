...
class Control{
    ...
    protected function checkMessage() {
        if($_SESSION['seller_session_info']['member_id'] == '') return ;
        //判断cookie是否存在
        $cookie_name = 'msgnewnum'.$_SESSION['seller_session_info']['member_id'];
        if (cookie($cookie_name) != null){
            $countnum = intval(cookie($cookie_name));
        }else {
            $message_model = Model('message');
            $countnum = $message_model->countNewMessage($_SESSION['seller_session_info']['member_id']);
    ...
    protected function showCartCount() {
        if (cookie('cart_goods_num') != null){
            ...
            if($_SESSION['seller_session_info']['member_id']) {
                $save_type = 'db';
            } else {
                $save_type = 'cookie';
            }
            $cart_num = Model('cart')->getCartNum($save_type,array('buyer_id'=>$_SESSION['seller_session_info']['member_id']));//查询购物车商品种类
        }
    ...
    protected function getMemberAndGradeInfo($is_return = false){
        ...
        if($_SESSION['seller_session_info']['member_id']) {
            ...
            if ($member_info){
                ...
                $member_info['voucher_count'] = Model('voucher')->getCurrentAvailableVoucherCount($_SESSION['seller_session_info']['member_id']);
                $member_info['redpacket_count'] = Model('redpacket')->getCurrentAvailableRedpacketCount($_SESSION['seller_session_info']['member_id']);
    ...
    protected function checkLogin(){
        if ($_SESSION['seller_session_info']['is_login'] !== '1'){
            $ref_url = request_uri();
            if ($_GET['inajax']){
                showDialog('','','js',"login_dialog();",200);
            }else {
                @header("location: " . urlShop('seller_login', 'show_login'));
            }
            exit;
        }
    }
    ...
    protected function auto_login() {
        ...
        $model_member = Model('member');
        if ($_SESSION['seller_session_info']['is_login']) {
...
class BaseMemberControl extends Control {
    ...
    public function __construct(){
        ...
        $left = leftSellerMenuList();
        $right = rightSellerMenuList();
