  ...
  <div class="wrapper">
    <div class="ncsc-admin">
      <dl class="ncsc-admin-info">
        <dt class="admin-avatar"><img src="<?php echo getMemberAvatarForID($_SESSION['seller_session_info']['member_id']);?>" width="32" class="pngFix" alt=""/></dt>
        <dd class="admin-permission">当前用户</dd>
        <dd class="admin-name"><?php echo $_SESSION['seller_session_info']['seller_name'];?></dd>
      </dl>
      <div class="ncsc-admin-function"><a style="display: none" href="<?php echo urlShop('show_store', 'index', array('store_id'=>$_SESSION['seller_session_info']['store_id']), $output['store_info']['store_domain']);?>" title="前往店铺" ><i class="icon-home"></i></a><a href="<?php echo urlMember('member_security', 'auth',array('type'=>'modify_pwd'));?>" title="修改密码" target="_blank"><i class="icon-wrench"></i></a><a href="<?php echo urlShop('seller_logout', 'logout');;?>" title="安全退出"><i class="icon-signout"></i></a></div>
    </div>
        ...
        <div class="sitemap-menu">
          ...
          <div id="quicklink_list" class="content">
            ...
            <dl <?php echo ($key == 'promotion' ? 'class="double"' : '');?>>
              ...
              <dd <?php if(!empty($output['seller_quicklink'])) {echo in_array($submenu_value['controller'], $output['seller_quicklink'])?'class="selected"':'';}?>><i nctype="btn_add_quicklink" data-quicklink-act="<?php echo $submenu_value['controller'];?>" class="icon-check" title="添加为常用功能菜单"></i><a href="index.php?controller=<?php echo $submenu_value['controller'];?>&action=<?php echo $submenu_value['action'];?>"> <?php echo $submenu_value['name'];?> </a></dd>
              <?php } ?>
              <?php } ?>
            </dl>
            <?php } ?>
    ...
    <nav class="ncsc-nav">
      <dl class="<?php echo $output['current_menu']['model'] == 'index'?'current':'';?>">
        <dt><a href="index.php?controller=seller_center&action=index">首页</a></dt>
        <dd class="arrow"></dd>
      </dl>
      <?php if(!empty($output['menu']) && is_array($output['menu'])) {?>
      <?php foreach($output['menu'] as $key => $menu_value) {?>
      <dl class="<?php echo $output['current_menu']['model'] == $key?'current':'';?>">
        <dt><a href="index.php?controller=<?php echo $menu_value['child'][key($menu_value['child'])]['controller'];?>&action=<?php echo $menu_value['child'][key($menu_value['child'])]['action'];?>"><?php echo $menu_value['name'];?></a></dt>
        <dd>
          <ul>
            <?php if(!empty($menu_value['child']) && is_array($menu_value['child'])) {?>
            <?php foreach($menu_value['child'] as $submenu_value) {?>
            <li> <a href="index.php?controller=<?php echo $submenu_value['controller'];?>&action=<?php echo $submenu_value['action'];?>"> <?php echo $submenu_value['name'];?> </a> </li>
            <?php } ?>
            <?php } ?>
          </ul>
        </dd>
    ...
    <div id="sidebar" class="sidebar">
      ...
      <div class="column-menu">
        <ul id="seller_center_left_menu">
          ...
          <li <?php echo $_GET['controller'] == 'seller_center'?"id='quicklink_".$submenu_value['controller']."'":'';?>class="<?php echo $submenu_value['controller'] == $_GET['controller']?'current':'';?>"> <a href="index.php?controller=<?php echo $submenu_value['controller'];?>&action=<?php echo $submenu_value['action'];?>"> <?php echo $submenu_value['name'];?> </a> </li>
          <?php } ?>
          <?php } else { ?>
          <?php if ($_GET['controller'] == 'seller_center') { ?>
