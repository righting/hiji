...
<div class="essential">
  ...
  <div class="alert alert-success">
    ...
    <ul>
      ...
      <li>2. 如果您的邮箱已失效，可以 <a href="index.php?controller=member_security&action=auth&type=modify_mobile">绑定手机</a> 后通过接收手机短信完成验证。</li>
      <li>3. 如果您的手机已失效，可以 <a href="index.php?controller=member_security&action=auth&type=modify_email">绑定邮箱</a> 后通过接收邮件完成验证。</li>
      <li>4. 请正确输入下方图形验证码，如看不清可点击图片进行更换，输入完成后进行下一步操作。</li>
      <li>5. 收到安全验证码后，请在30分钟内完成验证。</li>
    </ul>
  </div>
  <div class="ncm-default-form">
    <form method="post" id="auth_form" action="index.php?controller=member_security&action=auth">
      ...
      <dl>
        ...
        <dd>
          <input type="text" name="captcha" class="text" id="other-captcha" maxlength="4" size="10" autocomplete="off" />
         <img src="index.php?controller=seccode&action=makecode&nchash=<?php echo getNchash();?>" name="codeimage" border="0" id="get-codeimage" class="ml5 vm"><a href="javascript:void(0)" class="ml5 blue" onclick="javascript:document.getElementById('get-codeimage').src='index.php?controller=seccode&action=makecode&nchash=<?php echo getNchash();?>&t=' + Math.random();">看不清？换张图</a>
...
$(function(){
	$('.submit').on('click',function(){
		if (!$('#auth_form').valid()){
			document.getElementById('get-codeimage').src='index.php?controller=seccode&action=makecode&nchash=<?php echo getNchash();?>&t=' + Math.random();
		} else {
			$('#auth_form').submit();
		}
	});
	...
	$('#send_auth_code').on('click',function(){
		...
		ALLOW_SEND = !ALLOW_SEND;
		$('#sending').show();
		$.getJSON('index.php?controller=member_security&action=send_auth_code',{type:$('#auth_type').val()},function(data){
			if (data.state == 'true') {
				$('#sending').hide();
				$('#show_times').html(60);
