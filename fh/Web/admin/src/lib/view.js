...
layui.define(['laytpl','table' ,'layer','table','tree'], function(exports){
    var $ = layui.jquery
        ,laytpl = layui.laytpl
        ,layer = layui.layer
        ,table = layui.table
    ...
    view.req = function(options){
        ...
        if(request.token && options.type == 'post'){
            //自动给参数传入默认 token
            options.data[request.token] = request.token in options.data
                ?  options.data[request.token]
                : (layui.data(setter.tableName)[request.token] || '');
            //自动给 Request Headers 传入 token
            // options.headers[request.token] = request.token in options.headers
            //   ?  options.headers[request.token]
            // : (layui.data(setter.tableName)[request.token] || '');
        }
        ...
        return $.ajax($.extend({
            type: 'post'
            // ,data:{
            //     token: layui.data('Security').token
            // }
            ,dataType: 'json'
    ...
    view.error = function(content, options){
        ...
        return layer.msg(content,{icon:2,time:2000})
