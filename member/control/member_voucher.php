...
class member_voucherControl extends BaseMemberControl
{
    ...
    public function voucher_bindingOp()
    {
        if (chksubmit(false, true)) {
            ...
            if (intval($_SESSION['seller_session_info']['store_id']) == $voucher_info['voucher_store_id']) {
            ...
            if ($result) {
                ...
                showDialog('代金券领取成功', 'index.php?controller=member_voucher&action=voucher_list', 'succ');
            } else {
                showDialog('代金券领取失败');
            }
        }
    ...
    private function profile_menu($menu_key = '')
    {
        $menu_array = array(
            1 => array(
                ...
                'menu_url' => 'index.php?controller=member_voucher&action=voucher_list'
            ),
            2 => array(
                'menu_key' => 'voucher_binding',
                'menu_name' => '领取代金券',
                'menu_url' => 'index.php?controller=member_voucher&action=voucher_binding'
            )
        );
