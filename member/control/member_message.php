...
class member_messageControl extends BaseShopMemberControl {
    public function __construct() {
        parent::__construct();
        //读取语言包
        Language::read('member_home_message');
        Tpl::output('webTitle',' - 我的消息');
    }
    ...
    public function savemsgOp() {
        ...
        if (chksubmit()) {
            ...
            $membername_arr = explode(',', trim($_POST['to_member_name'],','));
            ...
            if($_GET['type'] == 'sns_board'){
                ...
                showDialog(Language::get('home_message_send_success'),'index.php?controller=member_message&action=privatemsg','succ');
            }
        }
    }
    /**
    ...
    public function savereplyOp() {
        ...
        if (!$isallowsend){
            if($_GET['inajax'] == 1){
                ...
                showMessage(Language::get('home_message_noallowsend'),'index.php?controller=member_message&action=message','html','error');
            }
        }
        if ($_POST['form_submit'] == 'ok') {
            $message_id = intval($_POST["message_id"]);
            if ($message_id <=0){
                showMessage(Language::get('wrong_argument'),'index.php?controller=member_message&action=message','html','error');
            }
            $obj_validate = new Validate();
            $obj_validate->validateparam = array(
            ...
            if($_GET['inajax'] == 1){
                ...
                showMessage(Language::get('home_message_send_success'),'index.php?controller=member_message&action=privatemsg');
            }
        }else {
            if($_GET['inajax'] == 1){
...
//                array( 'mmt_code' => 'vr_code_will_expire', 'member_id' => $_SESSION['member_id'], 'is_receive' => intval($_POST['vr_code_will_expire'])),
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array(
            array('menu_key'=>'message', 'menu_name'=>Language::get('home_message_received_message'),'menu_url'=>'index.php?controller=member_message&action=message'),
            array('menu_key'=>'private', 'menu_name'=>Language::get('home_message_private_message'), 'menu_url'=>'index.php?controller=member_message&action=privatemsg'),
            array('menu_key'=>'system',  'menu_name'=>Language::get('home_message_system_message'),  'menu_url'=>'index.php?controller=member_message&action=systemmsg'),
            array('menu_key'=>'ggmsg',  'menu_name'=>'系统公告',  'menu_url'=>'index.php?controller=member_message&action=ggmsg'),
            array('menu_key'=>'close',   'menu_name'=>Language::get('home_message_close'),   'menu_url'=>'index.php?controller=member_message&action=personalmsg'),
            array('menu_key'=>'setting', 'menu_name'=>'接收设置', 'menu_url'=>urlMember('member_message', 'setting'))
        );
        if($menu_key == 'sendmsg') {
            $menu_array[] = array('menu_key'=>'sendmsg','menu_name'=>Language::get('home_message_send_message'),'menu_url'=>'index.php?controller=member_message&action=sendmsg');
