    ...
    public function registerOp(){
        ...
        if (chksubmit() && strlen($phone) == 11 && strlen($captcha) == 6){
            ...
            if (!empty($_POST['invite_member_id'])) {
                ...
                $invite_member = $model_member->getMemberInfo(['member_number' => $rec_id],'member_id,member_name,member_avatar,member_state,level_id');
                if(empty($invite_member)){
                    showDialog('邀请码已失效，如有需要请联系您的邀请人重新发送邀请码', '', 'error');
                }
                // 如果用户等级不是免费会员以上，则不能邀请别人注册
                if($invite_member['level_id'] == 7){
            ...
            if($result) {
                ...
                if(!empty($invite_member)){
                    $register_invite_data['from_user_id'] = $invite_member['member_id'];
                    $register_invite_data['to_user_id'] = $result;
                    ...
                    //写入实名信息
                    Model('member_detail')->editMemberDetail(['member_id'=>$result],['real_name'=>$_POST['real_name'],'member_id_number'=>$_POST['member_id_number'],'id_card_photo'=>$_POST['id_card_photo']]);
                }
