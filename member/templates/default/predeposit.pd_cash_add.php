...
<div class="essential">
  ...
  <div class="ncm-default-form">
      <?php if ($_SESSION['isauth']!=1){ ?>
        请先完成实名认证，点击<a href="<?php echo urlMember('member_information','detail') ?>" target="_blank">【立即完成】</a>
      <?php }else{ ?>
    <form method="post" id="cash_form" action="<?php echo MEMBER_SITE_URL;?>/index.php?controller=predeposit&action=pd_cash_add">
...
<i class="icon-renminbi"></i></em> （当前可用金额：<strong class="orange"><?php echo floatval($output['member_info']['available_predeposit']); ?></strong>&nbsp;&nbsp;元）<span></span>
      ...
      <dl>
        ...
        <dd><input name="pdc_bank_name" type="text" class="text w200" value="<?php echo $output['bankInfo']['bank_name']?>" id="pdc_bank_name" maxlength="40"/><span></span>
...
请填写详细的开户银行分行名称，虚拟账户如支付宝、财付通填写“支付宝”、“财付通”即可。</p>
      ...
      <dl>
        ...
        <dd><input name="pdc_bank_no" type="text" class="text w200" id="pdc_bank_no" value="<?php echo $output['bankInfo']['bank_card_number']?>" maxlength="30"/><span></span>
          <p class="hint">银行账号或虚拟账号(支付宝、财付通等账号)</p>
        </dd>
      </dl>
      <dl>
        <dt><i class="required">*</i>开户人姓名：</dt>
        <dd><input name="pdc_bank_user" type="text" class="text w100" id="pdc_bank_user" value="<?php echo $output['bankInfo']['bank_user_name']?>" maxlength="10"/><span></span>
