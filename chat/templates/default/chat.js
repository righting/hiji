$.getScript('/chat/templates/default/js/swfobject.js');
$.getScript('/chat/templates/default/js/web_socket.js');
...
$('.begin-chat').click(function () {
    var _uid = $(this).attr('u_id');
    name = $(this).find('h5').html();
    var _avatar = $(this).find('.user-avatar img').attr('src');
    var _chat_box = $('#new_msg_dialog');   // 聊天窗口
    _chat_box.find('dt.user-name').html(name);
    _chat_box.find('dd.user-avatar img').attr('src', _avatar);
    _chat_box.show();
    // connect();
});
