...
class member_refundControl extends BaseMemberControl {
    public function __construct(){
        ...
        Tpl::output('current_active_name','member_refund');
    ...
    public function add_refundOp(){
        ...
        if ($order_id < 1 || $goods_id < 1) {//参数验证
            showDialog(Language::get('wrong_argument'),'index.php?controller=member_order&action=index','error');
        }
        $condition = array();
        ...
        if (chksubmit() && $goods_id > 0){
            ...
            $return_type = intval($_POST['refund_type']);//类型:1为退款,2为退货
            // 如果退款类型不属于这四种类型，那么抛出异常
            if(!in_array($return_type,[1,2,3,4])){
                showDialog('退款类型异常，请刷新后重新选择','index.php?controller=member_order&action=index','error');
            }
            $refund_array['refund_type'] = $return_type;//类型:1为退款,2为退货
            $show_url = 'index.php?controller=member_return&action=index';
            $refund_array['return_type'] = '2';//退货类型:1为不用退货,2为需要退货
            if (in_array($return_type,[1,3])) {
                $show_url = 'index.php?controller=member_refund&action=index';
            }
    ...
    public function add_refund_allOp(){
        ...
        if (chksubmit()) {
            ...
            $order_goods_gc = Model('order_goods')->field('goods_id')->where(['order_id'=>$order_id])->select();
            $order_goods_gc= array_column($order_goods_gc,'goods_id');
            $order_goods_gc = implode(',',$order_goods_gc);
            $total = Model()->table('goods')->where(['goods_id'=>['in',$order_goods_gc],'gc_id_1'=>$annual_fee_gc_id])->count();
            if (intval($total)>0){
                   showDialog('专项消费商品不能申请退款','','error');
            }
            $refund_array = array();
            $refund_array['refund_type'] = '3';//类型:3为退款,2为退货
            $refund_array['seller_state'] = '1';//状态:1为待审核,2为同意,3为不同意
            $refund_array['order_lock'] = '2';//锁定类型:1为不用锁定,2为需要锁定
            ...
            if ($state) {
                $model_refund->editOrderLock($order_id);
                showDialog(Language::get('nc_common_save_succ'),'index.php?controller=member_refund&action=index','succ');
            } else {
                showDialog(Language::get('nc_common_save_fail'),'reload','error');
            }
        }
    ...
    public function indexOp(){
        ...
        $member_info = $member_model->where(['member_id'=>$user_id])->field('member_id,member_number,member_name,member_truename,level_id,member_points,member_h_points,member_time,positions_id,member_nickname,member_avatar,available_predeposit,freeze_predeposit,member_contribution')->find();
        ...
        switch ($type){// 0=所有订单  1=退款订单  2= 退货订单 3=已退货退款的订单
            case 1:$condition['refund_type']=['in',[1,3]];$condition['refund_state']=['neq',3];break;
            case 2:$condition['refund_type']=['in',[2,4]];$condition['refund_state']=['neq',3];break;
            case 3:$condition['refund_state']=3;break;
        }

        $refund_list = $model_refund->getRefundReturnList($condition,10);
        $orderGoodsModel = Model('order_goods');
        //获取图片
        foreach($refund_list as $k=>$v){
            if(empty($v['goods_image'])){
                $refund_list[$k]['goods_image'] = $orderGoodsModel->where(['order_id'=>$v['order_id']])->field('goods_image')->order('rec_id asc')->find()['goods_image'];
            }
        }

        //统计所有订单
        $countAll = $model_refund->where(['buyer_id'=>$_SESSION['member_id']])->count();
        //统计退款中订单
        $countRefundAll = $model_refund->where(['buyer_id'=>$_SESSION['member_id'],'refund_type'=>['in','1,3'],'refund_state'=>['neq','3']])->count();
        //统计退货中订单
        $countReturnAll = $model_refund->where(['buyer_id'=>$_SESSION['member_id'],'refund_type'=>['in','2,4'],'refund_state'=>['neq','3']])->count();
        //统计退款退货已完成订单
        $countCompleteAll = $model_refund->where(['buyer_id'=>$_SESSION['member_id'],'refund_state'=>'3'])->count();
        ...
        Tpl::output('countAll',$countAll);
        Tpl::output('countRefundAll',$countRefundAll);
        Tpl::output('countReturnAll',$countReturnAll);
        Tpl::output('countCompleteAll',$countCompleteAll);
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'member_order':
                $menu_array = array(
                array('menu_key'=>'buyer_refund','menu_name'=>Language::get('nc_member_path_buyer_refund'), 'menu_url'=>'index.php?controller=member_refund'),
                array('menu_key'=>'buyer_return','menu_name'=>Language::get('nc_member_path_buyer_return'), 'menu_url'=>'index.php?controller=member_return'));
                break;
        }
