...
class store_promotion_bundlingControl extends BaseSellerControl {
    ...
    public function bundling_listOp() {
        ...
        if (checkPlatformStore()) {
            ...
            if (!empty($bundling_quota)) {
                // 计算已经发布活动、剩余活动数量
                $bundling_published  = $model_bundling->getBundlingCount(array('store_id' => $_SESSION['seller_session_info']['store_id']));
        ...
        if ($hasList) {
            ...
            if (!empty($bundling_list)) {
                ...
                foreach ($bundling_list as $key => $val) {
                    ...
                    $bundling_list[$key]['img'] = cthumb($bgoods_array[$val['bl_id']]['goods_image'], 60, $_SESSION['seller_session_info']['store_id']);
                }
            }
    ...
    public function bundling_quota_addOp() {
        if (chksubmit()) {
            ...
            $data['store_name']         = $_SESSION['seller_session_info']['store_name'];
            ...
            if ($return) {
                ...
                Model('cron')->addCron(array('exetime' => $end_time, 'exeid' => $_SESSION['seller_session_info']['store_id'], 'type' => 3), true);
    ...
    public function bundling_renewOp() {
        if (chksubmit()) {
            ...
            if ($return) {
                ...
                // 添加任务队列
                Model('cron')->addCron(array('exetime' => $update['bl_quota_endtime'], 'exeid' => $_SESSION['seller_session_info']['store_id'], 'type' => 3), true);
    ...
    public function bundling_addOp() {
        ...
        if (intval(C('promotion_bundling_sum')) != 0 && !isset($_REQUEST['bundling_id'])) {
            $count = $model_bundling->getBundlingCount(array('store_id' => $_SESSION['seller_session_info']['store_id']));
        ...
        if (intval($_GET['bundling_id']) > 0) {
            $bundling_info = $model_bundling->getBundlingInfo(array('bl_id' => intval($_GET['bundling_id']), 'store_id' => $_SESSION['seller_session_info']['store_id']));
    ...
    public function drop_bundlingOp(){
        ...
        $return = Model('p_bundling')->delBundling($blids, $_SESSION['seller_session_info']['store_id']);
        if ($return) {
