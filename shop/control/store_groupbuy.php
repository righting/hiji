...
class store_groupbuyControl extends BaseSellerControl {
    ...
    public function __construct() {
        ...
        if (intval(C('groupbuy_allow')) !== 1){
            showMessage(Language::get('groupbuy_unavailable'),'index.php?controller=seller_center','','error');
        }
    }
    /**
     * 默认显示团购列表
    ...
    public function groupbuy_quota_add_saveOp() {
        ...
        //获取该用户已有套餐
        $current_groupbuy_quota= $model_groupbuy_quota->getGroupbuyQuotaCurrent($_SESSION['seller_session_info']['store_id']);
    ...
    public function groupbuy_saveOp() {
        ...
        if ($_GET['vr']) {
            if ($param['upper_limit'] > 0 && $goods_info['virtual_limit'] > 0
                ...
                showDialog(sprintf('虚拟团购活动的限购数量(%d)不能大于虚拟商品本身的限购数量(%d)',
                    $param['upper_limit'], $goods_info['virtual_limit']
                    ), 'index.php?controller=store_groupbuy');
            }
        ...
        if($result) {
            ...
            showDialog(Language::get('groupbuy_add_success'),'index.php?controller=store_groupbuy','succ');
        }else {
            showDialog(Language::get('groupbuy_add_fail'),'index.php?controller=store_groupbuy');
        }
    }
    ...
    public function groupbuy_goods_infoOp() {
        ...
        $data['goods_image'] = cthumb($goods_info['goods_image'], 240);
    ...
    private function _image_upload($file) {
        ...
        if(!empty($_FILES[$file]['name'])) {
            ...
            $uploaddir = ATTACH_PATH.DS.'groupbuy'.DS.$_SESSION['seller_session_info']['store_id'].DS;
    ...
    private function profile_menu($menu_key='') {
        ...
        switch ($menu_key){
        case 'groupbuy_add':
            $menu_array[] = array('menu_key'=>'groupbuy_add','menu_name'=>L('nc_member_path_new_group'),'menu_url'=>'index.php?controller=store_groupbuy&action=groupbuy_add');
            break;
        case 'groupbuy_add_vr':
            $menu_array[] = array('menu_key'=>'groupbuy_add_vr','menu_name'=>'新增虚拟团购','menu_url'=>'index.php?controller=store_groupbuy&action=groupbuy_add_vr');
            break;
        case 'groupbuy_quota_add':
            ...
            break;
        case 'groupbuy_edit':
            $menu_array[] = array('menu_key'=>'groupbuy_edit','menu_name'=>L('nc_member_path_edit_group'),'menu_url'=>'index.php?controller=store_groupbuy');
            break;
        case 'cancel':
