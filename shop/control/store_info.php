...
class store_infoControl extends BaseSellerControl {
    ...
    public function indexOp(){
        ...
        // 店铺信息
        $store_info = $model_store->getStoreInfoByID($_SESSION['seller_session_info']['store_id']);
        ...
        $store_bind_class_list = $model_store_bind_class->getStoreBindClassList(array('store_id'=>$_SESSION['seller_session_info']['store_id'],'state'=>array('in',array(1,2))), null);
    ...
    public function bind_classOp() {
        ...
        $store_bind_class_list = $model_store_bind_class->getStoreBindClassList(array('store_id'=>$_SESSION['seller_session_info']['store_id']), null);
        $goods_class = Model('goods_class')->getGoodsClassIndexedListAll();
        for($i = 0, $j = count($store_bind_class_list); $i < $j; $i++) {
            $store_bind_class_list[$i]['class_1_name'] = $goods_class[$store_bind_class_list[$i]['class_1']]['gc_name'];
            $store_bind_class_list[$i]['class_2_name'] = $goods_class[$store_bind_class_list[$i]['class_2']]['gc_name'];
            $store_bind_class_list[$i]['class_3_name'] = $goods_class[$store_bind_class_list[$i]['class_3']]['gc_name'];
            $store_bind_class_list[$i]['class_4_name'] = $goods_class[$store_bind_class_list[$i]['class_4']]['gc_name'];
        }
    ...
    public function bind_class_addOp() {
        ...
        //非自营店注销专项消费类别
        $store = Model('store')->where(['store_id'=>$_SESSION['seller_session_info']['seller_id']])->find();
    ...
    public function bind_class_saveOp() {
        ...
        if (preg_match('/^[\d,]+$/',$_POST['goods_class'])) {
            list($class_1, $class_2, $class_3, $class_4) = explode(',', trim($_POST['goods_class'],','));
        } else {
        ...
        if($result) {
            showDialog('申请成功，请等待系统审核','index.php?controller=store_info&action=bind_class','succ',empty($_GET['inajax']) ?'':'CUR_DIALOG.close();');
        }else {
            showDialog($lang['nc_common_save_fail']);
        }
    }
    ...
    public function reopenOp(){
        ...
        $reopen_list = $model_store_reopen->getStoreReopenList(array('re_store_id'=>$_SESSION['seller_session_info']['store_id']));
        ...
        //默认选中当前级别
        Tpl::output('current_grade_id',$_SESSION['seller_session_info']['grade_id']);

        //如果存在有未上传凭证或审核中的信息，则不能再申请续签
        $condition = array();
        $condition['re_state'] = array('in',array(0,1));
        $condition['re_store_id'] = $_SESSION['seller_session_info']['store_id'];
    ...
    public function reopen_addOp() {
        ...
        $data['re_store_id'] = $_SESSION['seller_session_info']['store_id'];
        $data['re_store_name'] = $_SESSION['seller_session_info']['store_name'];
...
//             $data['re_end_time'] = strtotime(date('Y-m-d 23:59:59', $data['re_start_time'])." +".intval($data['re_year'])." year");
        ...
        if ($insert) {
            if ($data['re_pay_amount'] == 0) {
//              $model_store->editStore(array('store_end_time'=>$data['re_end_time']),array('store_id'=>$_SESSION['seller_session_info']['store_id']));
    ...
    private function profile_menu($menu_type,$menu_key='',$array=array()) {
        ...
        switch ($menu_type) {
            case 'index':
                $menu_array[] = array('menu_key'=>'bind_class', 'menu_name'=>$lang['nc_member_path_bind_class'], 'menu_url'=>'index.php?controller=store_info&action=bind_class');
                if (!checkPlatformStore()) {
                    $menu_array[] = array('menu_key'=>'index', 'menu_name'=>$lang['nc_member_path_store_info'], 'menu_url'=>'index.php?controller=store_info&action=index');
                    $menu_array[] = array('menu_key'=>'reopen', 'menu_name'=>$lang['nc_member_path_store_reopen'], 'menu_url'=>'index.php?controller=store_info&action=reopen');
                }
                break;
            case 'bind_class':
                $menu_array = array(
                array('menu_key'=>'index', 'menu_name'=>$lang['nc_member_path_bind_class'], 'menu_url'=>'index.php?controller=store_bind_class&action=index'),
                );
                break;
            case 'add':
                $menu_array = array(
                array('menu_key'=>'index', 'menu_name'=>$lang['nc_member_path_bind_class'], 'menu_url'=>'index.php?controller=store_bind_class&action=index'),
                array('menu_key'=>'add', 'menu_name'=>$lang['nc_member_path_bind_class_add'], 'menu_url'=>'index.php?controller=store_bind_class&action=add')
                );
                break;
        }
        if(!empty($array)) {
