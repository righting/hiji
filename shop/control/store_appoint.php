...
class store_appointControl extends BaseSellerControl {
    ...
    public function indexOp() {
        ...
        $appoint_list = $model_arrtivalnotice->getArrivalNoticeList(array('store_id' => $_SESSION['seller_session_info']['store_id']), '*', '', '15');
    ...
    public function del_appointOp() {
        $id = intval($_GET['id']);
        $model_arrtivalnotice = Model('arrival_notice');
        $model_arrtivalnotice->delArrivalNotice(array('store_id' => $_SESSION['seller_session_info']['store_id'], 'an_id' => $id));
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array(
            array('menu_key'=>'member_appoint','menu_name'=>'预约/到货通知列表', 'menu_url'=>'index.php?controller=store_appoint')
        );
