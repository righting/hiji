...
class store_albumControl extends BaseSellerControl {
    ...
    public function album_cateOp(){
         ...
         * 验证是否存在默认相册
         */
        $return = $model_album->checkAlbum(array('album_aclass.store_id'=>$_SESSION['seller_session_info']['store_id'],'is_default'=>'1'));
        if(!$return){
            $album_arr = array();
            $album_arr['aclass_name'] = Language::get('album_default_album');
            $album_arr['store_id'] = $_SESSION['seller_session_info']['store_id'];
         ...
         */
        $param = array();
        $param['album_aclass.store_id'] = $_SESSION['seller_session_info']['store_id'];
...
//      Tpl::output('show_page',$page->show());
    ...
    public function album_addOp(){
         ...
         */
        $model_album = Model('album');
        $class_count = $model_album->countClass($_SESSION['seller_session_info']['store_id']);
    ...
    public function album_add_saveOp(){
        if (chksubmit()){
            ...
            if($class_count['count'] >= 20){
                showDialog(Language::get('album_class_save_max_20'),'index.php?controller=store_album','error',empty($_GET['inajax'])?'':'CUR_DIALOG.close();');
            }
            /**
             * 实例化相册模型
             */
            ...
            if($return){
                showDialog(Language::get('album_class_save_succeed'),'index.php?controller=store_album','succ',empty($_GET['inajax'])?'':'CUR_DIALOG.close();');
            }
        }
    ...
    public function album_editOp(){
        ...
        $param['value']     = array(intval($_GET['id']),$_SESSION['seller_session_info']['store_id']);
    ...
    public function album_edit_saveOp(){
         ...
         * 验证
         */
        $return = $model_album->checkAlbum(array('album_aclass.store_id'=>$_SESSION['seller_session_info']['store_id'],'album_aclass.aclass_id'=>intval($_POST['id'])));
        if($return){
            /**
             * 更新
             */
            ...
            if($re){
                showDialog(Language::get('album_class_edit_succeed'),'index.php?controller=store_album','succ',empty($_GET['inajax'])?'':'CUR_DIALOG.close();');
            }
        }else{
    ...
    public function album_delOp(){
         ...
         * 验证
         */
        $return = $model_album->checkAlbum(array('album_aclass.store_id'=>$_SESSION['seller_session_info']['store_id'],'album_aclass.aclass_id'=>intval($_GET['id']),'is_default'=>'0'));
        if(!$return){
        ...
        $param['value']     = array('1',$_SESSION['seller_session_info']['store_id']);
        $class_info = $model_album->getOneClass($param);
        $param = array();
        $param['aclass_id'] = $class_info['aclass_id'];
        $return = $model_album->updatePic($param,array('aclass_id'=>intval($_GET['id'])));
        if($return){
            showDialog(Language::get('album_class_file_del_succeed'),'index.php?controller=store_album','succ');
        }else{
            showDialog(Language::get('album_class_file_del_lose'));
        }
    }
    ...
    public function album_pic_listOp(){
        ...
        $param['album_pic.store_id']    = $_SESSION['seller_session_info']['store_id'];
        if($_GET['sort'] != ''){
            switch ($_GET['sort']){
                case '0':
                    $param['order']     = 'upload_time desc';
    ...
    public function pic_listOp(){
        ...
        if(!empty($_GET) && $_GET['id'] != '0'){
            ...
            $cparam['value']        = array(intval($_GET['id']),$_SESSION['seller_session_info']['store_id']);
    ...
    public function change_album_coverOp(){
        ...
        $return = $model_album->checkAlbum(array('album_aclass.store_id'=>$_SESSION['seller_session_info']['store_id'],'album_aclass.aclass_id'=>$pic_info['aclass_id']));
        if($return){
    ...
    public function album_pic_infoOp(){
         ...
         * 验证
         */
        $return = $model_album->checkAlbum(array('album_pic.store_id'=>$_SESSION['seller_session_info']['store_id'],'album_pic.apic_id'=>intval($_GET['id'])));
        if(!$return){
            showMessage(Language::get('album_parameter_error'),'','html','error');
        }
        ...
        $param = array();
        $param['field']     = array('aclass_id','store_id');
        $param['value']     = array(intval($_GET['class_id']),$_SESSION['seller_session_info']['store_id']);
        $class_info         = $model_album->getOneClass($param);
        Tpl::output('class_info',$class_info);
    ...
    public function album_pic_delOp(){
        ...
        $return = $model_album->checkAlbum(array('album_pic.store_id'=>$_SESSION['seller_session_info']['store_id'],'in_apic_id'=>$id));
        if(!$return){
            showDialog(Language::get('album_class_pic_del_lose'));
        }

        //删除图片
        $return = $model_album->delPic($id, $_SESSION['seller_session_info']['store_id']);
        if($return){
    ...
    public function replace_image_uploadOp() {
        ...
        $param = array();
        $param['field'] = array('apic_id', 'store_id');
        $param['value'] = array($id, $_SESSION['seller_session_info']['store_id']);
        ...
        $upload->set('default_dir', ATTACH_GOODS . DS . $_SESSION['seller_session_info']['store_id'] . DS . $pic_cover);
        ...
        // 取得图像大小
        if (!C('oss.open')) {
            list($width, $height, $type, $attr) = getimagesize(BASE_UPLOAD_PATH . DS . ATTACH_GOODS . DS . $_SESSION['seller_session_info']['store_id'] . DS . $apic_info['apic_cover']);
        } else {
            list($width, $height, $type, $attr) = getimagesize(C('oss.img_url') . '/' . ATTACH_GOODS . '/' . $_SESSION['seller_session_info']['store_id'] . DS . $apic_info['apic_cover']);
        }
        /**
         * 更新图片分类
         */
	...
	public function album_pic_watermarkOp(){
		...
		$store_wm_info = $model_store_wm->getOneStoreWMByStoreId($_SESSION['seller_session_info']['store_id']);
		if ($store_wm_info['wm_image_name'] == '' && $store_wm_info['wm_text'] == ''){
			showDialog(Language::get('album_class_setting_wm'),"index.php?controller=store_album&action=store_watermark",'error','CUR_DIALOG.close();');//"请先设置水印"
		}
		...
		foreach ($wm_list as $v) {
			$gd_image->create(BASE_UPLOAD_PATH.DS.ATTACH_GOODS.DS.$_SESSION['seller_session_info']['store_id'].DS.str_ireplace('.', '_1280.', $v['apic_cover']));//生成有水印的大图
		}
    ...
    public function store_watermarkOp(){
        ...
        if (empty($store_wm_info)){
            ...
            $model_store_wm->addStoreWM(array(
                ...
                'store_id'=>$_SESSION['seller_session_info']['store_id']
            ));
            $store_wm_info = $model_store_wm->getOneStoreWMByStoreId($_SESSION['seller_session_info']['store_id']);
        }
    ...
    private function profile_menu($menu_type,$menu_key=''){
        ...
        switch ($menu_type) {
            case 'album':
                $menu_array = array(
                1=>array('menu_key'=>'album','menu_name'=>Language::get('nc_member_path_my_album'),'menu_url'=>'index.php?controller=store_album'),
                2=>array('menu_key'=>'watermark','menu_name'=>Language::get('nc_member_path_watermark'),'menu_url'=>'index.php?controller=store_album&action=store_watermark')
                );
                break;
            case 'album_pic':
                $menu_array = array(
                1=>array('menu_key'=>'album','menu_name'=>Language::get('nc_member_path_my_album'),'menu_url'=>'index.php?controller=store_album'),
                3=>array('menu_key'=>'pic_list','menu_name'=>Language::get('nc_member_path_album_pic_list'),'menu_url'=>'index.php?controller=store_album&action=album_pic_list&id='.intval($_GET['id'])),
                2=>array('menu_key'=>'watermark','menu_name'=>Language::get('nc_member_path_watermark'),'menu_url'=>'index.php?controller=store_album&action=store_watermark')
                );
                break;
            case 'album_pic_info':
                $menu_array = array(
                1=>array('menu_key'=>'album','menu_name'=>Language::get('nc_member_path_my_album'),'menu_url'=>'index.php?controller=store_album'),
                3=>array('menu_key'=>'pic_info','menu_name'=>Language::get('nc_member_path_album_pic_info'),'menu_url'=>'index.php?controller=store_album&action=album_pic_info&id='.intval($_GET['id']).'&class_id='.intval($_GET['class_id'])),
                2=>array('menu_key'=>'watermark','menu_name'=>Language::get('nc_member_path_watermark'),'menu_url'=>'index.php?controller=store_album&action=store_watermark')
                );
                break;
        }
    ...
    public function ajax_change_imgmessageOp(){
         ...
         * 小图尺寸
         */
        list($width, $height, $type, $attr) = getimagesize(BASE_UPLOAD_PATH.DS.ATTACH_GOODS.DS.$_SESSION['seller_session_info']['store_id'].DS.$pic_info['0']['apic_cover']);
        if(strtoupper(CHARSET) == 'GBK'){
    ...
    public function ajax_check_class_nameOp(){
        ...
        $param['value']     = array($ac_name,$_SESSION['seller_session_info']['store_id']);
        $class_info = $model_album->getOneClass($param);
        if(!empty($class_info)){
            echo 'false';die;
        }else{
