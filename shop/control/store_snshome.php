...
//      //查询会员信息
        ...
        if ($result){
            ...
            if ($_POST['showtype'] == 1){
                $js .="$('#tracereply_".$stid."').load('index.php?controller=store_snshome&action=commenttop&id=".$stid."');";
            }else {
                $js .="$('#tracereply_".$stid."').load('index.php?controller=store_snshome&action=commentlist&id=".$stid."');";
            }
    ...
    public function addforwardOp(){
        ...
        $insert_arr['trace_content']            = "<dl class=\"fd-wrap\">
                                                        <dt>
                                                            <h3><a href=\"index.php?controller=store_snshome&sid=".$stracelog_info['strace_storeid']."\" target=\"_blank\">".$stracelog_info['strace_storename']."</a>".Language::get('nc_colon')."
    ...
    public function deltraceOp(){
        ...
        //删除动态
        $result = Model('store_sns_tracelog')->delStoreSnsTracelog(array('strace_id'=>$stid , 'strace_storeid'=>$_SESSION['seller_session_info']['store_id'] ));
        if ($result){
            //删除对应的评论
