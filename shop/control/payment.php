...
class paymentControl extends BaseHomeControl
{
    ...
    public function real_orderOp()
    {
        ...
        /*dump($order_pay_info['data']['order_list']);
        die;*/
        ...
        if (empty($api_pay_amount)) {
            /**循环处理订单2017年12月7日17:31:14新加需求**/
            // 检查当前订单是否为补单订单，如果是，则不参加分红
            $order_id_arr = array_column($order_list, 'order_id');
            // 获取在补单日志表中存在的订单
            $refund_return_bd_log_model = Model('refund_return_bd_log');
            $bd_list                    = $refund_return_bd_log_model->where(['order_id' => ['in', $order_id_arr]])->select();
            $logic_order                = Logic('order');
            if (empty($bd_list)) {
                foreach ($order_list as $order_info) {
                    $logic_order->splitOrderGoodsLists($order_info);
                }
                redirect(SHOP_SITE_URL . '/index.php?controller=buy&action=pay_ok&pay_sn=' . $order_pay_info['data']['pay_sn'] . '&pay_amount=' . ncPriceFormat($pay_amount));
            }
            $bd_order_id_arr = array_column($bd_list, 'order_id');
            foreach ($order_list as $order_info) {
                // 非补单订单才有分红
                if (!in_array($order_info['order_id'], $bd_order_id_arr)) {
                    $logic_order->splitOrderGoodsLists($order_info);
                }
            }
            redirect(SHOP_SITE_URL . '/index.php?controller=buy&action=pay_ok&pay_sn=' . $order_pay_info['data']['pay_sn'] . '&pay_amount=' . ncPriceFormat($pay_amount));
        }
        $result = Model('order')->editOrder(['api_pay_time' => TIMESTAMP], ['order_id' => ['in', $pay_order_id_list]]);
    ...
    public function vr_orderOp()
    {
        ...
        $payment_code = $_POST['payment_code'];
        $url          = 'index.php?controller=member_vr_order';
        ...
        //如果所需支付金额为0，转到支付成功页
        if (empty($api_pay_amount)) {
            redirect('index.php?controller=buy_virtual&action=pay_ok&order_sn=' . $order_info['order_sn'] . '&order_id=' . $order_info['order_id'] . '&order_amount=' . ncPriceFormat($order_info['order_amount']));
        }
    ...
    public function notifyOp()
    {
        ...
        } elseif ($order_type == 'point_order') {//积分商城订单
            $result['data'] = $logic_payment->getPointOrderInfo($out_trade_no);
        } else {
            exit();
        }
        ...
        if ($order_type == 'real_order') {
            ...
            $res              = $model_pointorder->table('points_order')->where(['point_ordersn' => $out_trade_no])->find();
            //新增平台收费记录
            $model_platform = Model('platform_profit');
            $model_platform->insert(['user_id' => $res['point_buyerid'], 'money' => $_GET['total_fee'] / 100, 'type' => 3, 'created_at' => date('Y-m-d H:i:s', time()), 'order_id' => $res['point_orderid'], 'remark' => '用户ID:' . $res['point_buyerid'] . '积分兑换商品' . '订单编号:' . $res['point_ordersn']]);
            //更改积分订单状态
            $model_pointorder->editPointOrder(['point_orderid' => $res['point_orderid']], ['point_orderstate' => 21]);
        ...
        if ($result['state']) {
            ...
            QueueClient::push('addConsume', ['member_id'      => $log_buyer_id, 'member_name' => $log_buyer_name,
    ...
    public function returnOp()
    {
        ...
        $url          = SHOP_SITE_URL . '/index.php?controller=' . $act;
        ...
        if ($payment_state != 'success') {
            ...
            if ($order_type == 'real_order') {
                ...
                $result = $logic_payment->updatePdOrder($out_trade_no, $trade_no, $payment_info, $order_pay_info);
            } elseif ($order_type == 'point_order') {//积分订单
                $model_pointorder = Model('pointorder');
                $res              = $model_pointorder->table('points_order')->where(['point_ordersn' => $out_trade_no])->find();
                //新增平台收费记录
                $model_platform = Model('platform_profit');
                $point_arr      = [];
                $point_arr[]    = ['user_id' => $res['point_buyerid'], 'money' => $res['point_service_charge'], 'type' => $model_platform::TYPE_INTEGRAL_FEE, 'created_at' => date('Y-m-d H:i:s', time()), 'order_id' => $res['point_orderid'], 'remark' => '用户 ID:' . $res['point_buyerid'] . '积分兑换商品' . '订单编号:' . $res['point_ordersn']];
                $point_arr[]    = ['user_id' => $res['point_buyerid'], 'money' => $res['point_shipping_fee'], 'type' => $model_platform::TYPE_INTEGRAL_FREIGHT, 'created_at' => date('Y-m-d H:i:s', time()), 'order_id' => $res['point_orderid'], 'remark' => '用户 ID:' . $res['point_buyerid'] . '积分兑换商品' . '订单编号:' . $res['point_ordersn']];
                $model_platform->insertAll($point_arr);

                //更改积分订单状态
                $model_pointorder->editPointOrder(['point_orderid' => $res['point_orderid']], ['point_orderstate' => 21]);
                redirect(SHOP_SITE_URL . '/index.php?controller=pointcart&action=step3&order_id=39');//跳转至积分商品兑换成功页面
            }
            if (!$result['state']) {
        ...
        //支付成功后跳转
        if ($order_type == 'real_order') {
            $pay_ok_url = SHOP_SITE_URL . '/index.php?controller=buy&action=pay_ok&pay_sn=' . $out_trade_no . '&pay_amount=' . ncPriceFormat($api_pay_amount);
        } elseif ($order_type == 'vr_order') {
            $pay_ok_url = SHOP_SITE_URL . '/index.php?controller=buy_virtual&action=pay_ok&order_sn=' . $out_trade_no . '&order_id=' . $order_pay_info['order_id'] . '&order_amount=' . ncPriceFormat($api_pay_amount);
        } elseif ($order_type == 'pd_order') {
    ...
    public function query_stateOp()
    {
        if ($_GET['pay_id'] && intval($_GET['pay_id']) > 0) {
            $info = Model('order')->getOrderPayInfo(['pay_id' => intval($_GET['pay_id']), 'buyer_id' => intval($_GET['buyer_id'])]);
            exit(json_encode(['state' => ($info['api_pay_state'] == '1'), 'pay_sn' => $info['pay_sn'], 'type' => 'r']));
        } elseif (intval($_GET['order_id']) > 0) {
            $info = Model('vr_order')->getOrderInfo(['order_id' => intval($_GET['order_id']), 'buyer_id' => intval($_GET['buyer_id'])]);
            exit(json_encode(['state' => ($info['order_state'] == '20'), 'pay_sn' => $info['order_sn'], 'type' => 'v']));
        }
    }
}
