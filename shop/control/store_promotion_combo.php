...
class store_promotion_comboControl extends BaseSellerControl {
    ...
    public function combo_listOp() {
        ...
        if ($hasList) {
            ...
            $combo_list = $model_combo_goods->getComboGoodsList(array('store_id' => $_SESSION['seller_session_info']['store_id']), 'distinct goods_id', 10, ' goods_id desc');
            if (!empty($combo_list)) {
                $goodsid_array = array();
        ...
        if (!empty($data)) {
            foreach ($data as $key => $val) {
                ...
                foreach ($val as $v) {
                    ...
                    if ($combo_info['store_id'] != $_SESSION['seller_session_info']['store_id'] || $is_general == false || $goods_id == $combo_gid) {
                        continue;
                    }
                    $array = array();
    ...
    public function choosed_goodsOp() {
        ...
        if (!checkPlatformStore()) {
            // 验证套餐时候过期
            $combo_info = $model_combo->getComboQuotaInfo(array('store_id' => $_SESSION['seller_session_info']['store_id'], 'cq_endtime' => array('gt', TIMESTAMP)));
            if (empty($combo_info)) {
                if ($_GET['inajax']) {
        ...
        if (empty($goods_info) || $goods_info['store_id'] != $_SESSION['seller_session_info']['store_id'] || !$model_goods->checkIsGeneral($goods_info)) {
            if ($_GET['inajax']) {
                showDialog('参数错误', '', 'succ', 'CUR_DIALOG.close();');
            }
