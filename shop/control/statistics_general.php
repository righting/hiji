...
class statistics_generalControl extends BaseSellerControl {
    ...
    public function generalOp(){
        ...
        //商品总数、收藏量
        $goods_list = $model->statByGoods(array('store_id'=>$_SESSION['seller_session_info']['store_id'],'is_virtual'=>0),'COUNT(*) as goodsnum, SUM(goods_collect) as gcollectnum');
        $statnew_arr['goodsnum'] = ($t = $goods_list[0]['goodsnum']) > 0?$t:0;
        $statnew_arr['gcollectnum'] = ($t = $goods_list[0]['gcollectnum']) > 0?$t:0;

        //店铺收藏量
        $store_list = $model->getoneByStore(array('store_id'=>$_SESSION['seller_session_info']['store_id']),'store_collect');
        ...
        $where['order_isvalid'] = 1;//计入统计的有效订单
        $where['order_add_time'] = array('between',array($stime,$etime));
        $where['store_id'] = array('neq',$_SESSION['seller_session_info']['store_id']);
        if (!checkPlatformStore()) {//如果不是平台店铺，则查询店铺经营类目的同行数据
            //查询店铺经营类目
            $store_bindclass = Model('store_bind_class')->getStoreBindClassList(array('store_id'=>$_SESSION['seller_session_info']['store_id']));
    ...
    public function pricesettingOp(){
        ...
        if (chksubmit()){
            ...
            $result = $model->where(array('store_id'=>$_SESSION['seller_session_info']['store_id']))->update($update_array);
            if ($result === true){
        ...
        $pricerange = $model->getfby_store_id($_SESSION['seller_session_info']['store_id'],'pricerange');
    ...
    public function orderprangeOp(){
        ...
        $pricerange = $model->getfby_store_id($_SESSION['seller_session_info']['store_id'],'orderpricerange');
        $pricerange = $pricerange?unserialize($pricerange):array();
        Tpl::output('pricerange',$pricerange);
    ...
    /*private function profile_menu($menu_key='') {
        $menu_array = array(
            1=>array('menu_key'=>'stat_general','menu_name'=>'店铺概况',    'menu_url'=>'index.php?controller=statistics_general&action=general')
        );
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'setting':
                $menu_array = array(
                    1=>array('menu_key'=>'pricesetting','menu_name'=>'商品价格区间',    'menu_url'=>'index.php?controller=statistics_general&action=pricesetting'),
                    2=>array('menu_key'=>'orderprange','menu_name'=>'订单金额区间', 'menu_url'=>'index.php?controller=statistics_general&action=orderprange'));
                break;
        }
        Tpl::output('member_menu',$menu_array);
        Tpl::output('menu_key',$menu_key);
