...
class store_deliver_setControl extends BaseSellerControl {
    ...
    public function daddress_addOp() {
        ...
        if (chksubmit()) {
            ...
            if (empty($address_info) && !is_array($address_info)){
                showMessage($lang['store_daddress_wrong_argument'],'index.php?controller=store_deliver_set&action=daddress_list','html','error');
            }
    ...
    public function daddress_delOp() {
        ...
        if ($delete){
            showDialog(Language::get('store_daddress_del_succ'),'index.php?controller=store_deliver_set&action=daddress_list','succ');
        }else {
            showDialog(Language::get('store_daddress_del_fail'),'','error');
        }
    }
   ...
   public function daddress_default_setOp() {
    ...
    public function expressOp() {
        ...
        if (chksubmit()){
            $data['store_id'] = $_SESSION['seller_session_info']['store_id'];
            if(is_array($_POST['cexpress']) && !empty($_POST['cexpress'])){
                $data['express'] = implode(',',$_POST['cexpress']);
            }else{
                $data['express'] = '';
            }
            if (!$model->getby_store_id($_SESSION['seller_session_info']['store_id'])){
                $result = $model->insert($data);
            }else{
                $result = $model->where(array('store_id'=>$_SESSION['seller_session_info']['store_id']))->update($data);
            }
            if ($result){
        ...
        //取得店铺启用的快递公司ID
        $express_select = $model->getfby_store_id($_SESSION['seller_session_info']['store_id'],'express');
    ...
    public function free_freightOp(){
        ...
        if (chksubmit()) {
            ...
            $model_store->editStore(array('store_free_price'=>$store_free_price),array('store_id'=>$_SESSION['seller_session_info']['store_id']));
    ...
    public function deliver_regionOp(){
        if (chksubmit()) {
            Model('store')->editStore(array('deliver_region'=>$_POST['area_ids'].'|'.$_POST['region']),array('store_id'=>$_SESSION['seller_session_info']['store_id']));
            showDialog(L('nc_common_save_succ'),'','succ');
        }
    ...
    public function print_setOp(){
        $model = Model();
        $store_info = $model->table('store')->where(array('store_id'=>$_SESSION['seller_session_info']['store_id']))->find();
        if(empty($store_info)){
            showDialog(Language::get('store_storeinfo_error'),'index.php?controller=store_printsetup','error');
        }
        if(chksubmit()){
            $obj_validate = new Validate();
            $obj_validate->validateparam = array(
            ...
            $rs = $model->table('store')->where(array('store_id'=>$_SESSION['seller_session_info']['store_id']))->update($update_arr);
            if ($rs){
    ...
    private function profile_menu($menu_type, $menu_key = '') {
        ...
        switch ($menu_type) {
            case 'daddress':
                $menu_array = array(
                array('menu_key'=>'daddress',   'menu_name'=>Language::get('store_deliver_daddress_list'),  'menu_url'=>'index.php?controller=store_deliver_set&action=daddress_list'),
                array('menu_key'=>'express',        'menu_name'=>Language::get('store_deliver_default_express'),    'menu_url'=>'index.php?controller=store_deliver_set&action=express'),
                array('menu_key'=>'free_freight',   'menu_name'=>'免运费额度',      'menu_url'=>'index.php?controller=store_deliver_set&action=free_freight'),
                array('menu_key'=>'deliver_region',   'menu_name'=>'默认配送地区',      'menu_url'=>'index.php?controller=store_deliver_set&action=deliver_region'),
                array('menu_key'=>'print_set', 'menu_name'=>'发货单打印设置', 'menu_url'=>'index.php?controller=store_deliver_set&action=print_set')
                );
                break;
        }
