...
class buyControl extends BaseBuyControl
{
    ...
    public function buy_step1Op()
    {
        ...
        //得到购买数据
        $logic_buy = Logic('buy');
        $result    = $logic_buy->buyStep1($_POST['cart_id'], $_POST['ifcart'], $_SESSION['member_id'], $_SESSION['seller_session_info']['store_id'], $_POST['jjg'], $this->member_info['orderdiscount'], $this->member_info['level'], $_POST['ifchain']);
        if (!$result['state']) {
...
//        Tpl::output('inv_info', $result['inv_info']);
    ...
    public function buy_step2Op()
    {
        ...
        if ($_SESSION['member_id']) {
            ...
            $member_info  = $model_member->getMemberInfoByID($_SESSION['member_id'], 'member_id,member_name,member_email,level_id,positions_id');
        }
        $result = $logic_buy->buyStep2($_POST, $_SESSION['member_id'], $_SESSION['member_name'], $_SESSION['member_email'], 0, $member_info['level_id']);
        if (!$result['state']) {
            showMessage($result['msg'], 'index.php?controller=cart', 'html', 'error');
        }

        //转向到商城支付页面
        redirect('index.php?controller=buy&action=pay&pay_sn=' . $result['data']['pay_sn']);
    }
    ...
    public function payOp()
    {
        ...
        if (!preg_match('/^\d{18}$/', $pay_sn)) {
            showMessage(Language::get('cart_order_pay_not_exists'), 'index.php?controller=member_order', 'html', 'error');
        }

        //查询支付单信息
        $model_order = Model('order');
        $pay_info    = $model_order->getOrderPayInfo(['pay_sn' => $pay_sn, 'buyer_id' => $_SESSION['member_id']], true);
        if (empty($pay_info)) {
            showMessage(Language::get('cart_order_pay_not_exists'), 'index.php?controller=member_order', 'html', 'error');
        }
        ...
        if (empty($order_list)) {
            showMessage('未找到需要支付的订单', 'index.php?controller=member_order', 'html', 'error');
        } elseif (!empty($order_list[0]['api_pay_time'])) { //此次发起为重复发起支付更换订单psy_sn
            $logic_buy1 = Logic('buy_1');
            $pay_sn_new = $logic_buy1->makePaySn($pay_info['buyer_id']);//生成新支付单号
            //更新订单表与订单支付表中支付单号
            try {
                $model_order->beginTransaction();
                $model_order->editOrder(['pay_sn' => $pay_sn_new], ['pay_sn' => $pay_sn]);
                $model_order->editOrderPay(['pay_sn' => $pay_sn_new], ['pay_sn' => $pay_sn]);
                $model_order->commit();
            } catch (Exception $e) {
            ...
            //更新当前待支付订单支付单号
            $pay_info['pay_sn']      = $pay_sn_new;
            $order_list[0]['pay_sn'] = $pay_sn_new;
        }
        ...
        if (empty($pay['pay_amount_online']) && empty($pay['pay_amount_offline'])) {
            redirect('index.php?controller=buy&action=pay_ok&pay_sn=' . $pay_sn . '&is_chain=' . $flag_chain . '&pay_amount=' . ncPriceFormat($pay_amount));
        }
        ...
        if ($pay['pay_amount_online'] > 0) {
            ...
            if (empty($payment_list)) {
                showMessage('暂未找到合适的支付方式', 'index.php?controller=member_order', 'html', 'error');
            }
    ...
    public function pd_payOp()
    {
        ...
        $pd_info     = $model_order->getPdRechargeInfo(['pdr_sn' => $pay_sn, 'pdr_member_id' => $_SESSION['member_id']]);
        if (empty($pd_info)) {
    ...
    {
        $list = Model('chain')->getChainList(['area_id' => intval($_GET['area_id']), 'store_id' => intval($_GET['store_id'])],
     ...
     * 加载补单列表
     */
    public function load_return_refundOp()
    {
        // 检查当前用户是否存在需要补单的订单，如果有，那么就要询问用户当前订单是否为补单订单，选择是之后需要检查当前订单与补单订单之间的利润
        $refund_return_model                 = Model('refund_return');
        $refund_return_where['buyer_id']     = $_SESSION['member_id'];
        ...
        $refund_return_list                  = $refund_return_model->where($refund_return_where)->select();
        Tpl::output('refund_return_list', $refund_return_list);
        Tpl::showpage('return_refund/add', 'null_layout');
    }

    public function response_msg($msg = '请求异常', $status = -1)
    {
    ...
    // 补单验证
    public function bu_dan_checkOp()
    {
        $post  = $_POST;
        ...
        if ($is_bd == 1) {
            ...
            $refund_return_list                    = $refund_return_model->where($refund_return_list_where)->field('refund_id,order_id,goods_id,order_goods_id')->select();
            // 获取要补的订单产生的总利润
            ...
            $buy_logic = Logic('buy');
            // 获取当前购买的商品id和购买的数量
            $now_goods_id_arr = $buy_logic->getPostGoodsIdForOrder($post);
            ...
            if ($diff_money < 0) {
                $this->response_msg('当前订单产生的利润未达到补单利润！满足条件还需要产生 ' . abs($diff_money) . ' 元的商品');
            }
        }
        $this->response_msg('success', 1);
    }
    ...
    public function add_addrOp()
    {
        ...
        if (chksubmit()) {
            ...
            $obj_validate->validateparam = [
                ["input" => $_POST["true_name"], "require" => "true", "message" => Language::get('cart_step1_input_receiver')],
                ["input" => $_POST["area_id"], "require" => "true", "validator" => "Number", "message" => Language::get('cart_step1_choose_area')]
            ];
    ...
    private function _buy_branch($post)
    {
        if (!$post['ifcart']) {
            ...
            if ($goods_info['is_virtual']) {
                redirect('index.php?controller=buy_virtual&action=buy_step1&goods_id=' . $goods_id . '&quantity=' . $quantity);
            }
        }
    }
}
