...
class store_decorationControl extends BaseSellerControl {
    ...
    public function decoration_settingOp() {
        ...
        $store_decoration_info = $model_store_decoration->getStoreDecorationInfo(array('store_id' => $_SESSION['seller_session_info']['store_id']));
    ...
    public function decoration_setting_saveOp() {
        ...
        $result = $model_store->editStore($update, array('store_id' => $_SESSION['seller_session_info']['store_id']));
        if($result) {
    ...
    public function decoration_album_uploadOp() {
        ...
        if($result) {
            ...
            Model('store')->editStore(
                ...
                array('store_id' => $_SESSION['seller_session_info']['store_id'])
            );
    ...
    public function decoration_editOp() {
        ...
        $decoration_info = $model_store_decoration->getStoreDecorationInfoDetail($decoration_id, $_SESSION['seller_session_info']['store_id']);
        if($decoration_info) {
    ...
    private function _validate_background_setting_input($decoration_id, $setting) {
        ...
        if(!empty($setting['background_image'])) {
            $setting['background_image_url'] = getStoreDecorationImageUrl($setting['background_image'], $_SESSION['seller_session_info']['store_id']);
    ...
    public function decoration_banner_saveOp() {
        ...
        if($result) {
            $data['message'] = '保存成功';
            $data['image_url'] = getStoreDecorationImageUrl($banner['image'], $_SESSION['seller_session_info']['store_id']);
        } else {
            $data['error'] = '保存失败';
        }
        echo json_encode($data);die;
    ...
    public function decoration_block_addOp() {
        ...
        $condition = array();
        $condition['decoration_id'] = $decoration_id;
        $decoration_info = $model_store_decoration->getStoreDecorationInfo($condition, $_SESSION['seller_session_info']['store_id']);
        if(!$decoration_info) {
            $data['error'] = L('param_error');
    ...
    public function decoration_previewOp() {
        ...
        //店铺信息
        $model_store = Model('store');
        $store_info = $model_store->getStoreOnlineInfoByID($_SESSION['seller_session_info']['store_id']);
    ...
    public function decoration_buildOp() {
        ...
        //更新商品数据
        $this->_update_module_goods_info($decoration_id, $_SESSION['seller_session_info']['store_id']);
        ...
        $file_name = md5($_SESSION['seller_session_info']['store_id']);
