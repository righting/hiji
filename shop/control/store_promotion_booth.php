...
class store_promotion_boothControl extends BaseSellerControl {
    ...
    public function booth_goods_listOp() {
        ...
        if ($hasList) {
            // 查询已选择商品
            $boothgoods_list = $model_booth->getBoothGoodsList(array('store_id' => $_SESSION['seller_session_info']['store_id']), 'goods_id');
    ...
    public function booth_quota_addOp() {
        if (chksubmit()) {
            ...
            if ($return) {
                ...
                Model('cron')->addCron(array('exetime' => $end_time, 'exeid' => $_SESSION['seller_session_info']['store_id'], 'type' => 4), true);
    ...
    public function choosed_goodsOp() {
        ...
        if (!checkPlatformStore()) {
            // 验证套餐时候过期
            $booth_info = $model_booth->getBoothQuotaInfo(array('store_id' => $_SESSION['seller_session_info']['store_id'], 'booth_quota_endtime' => array('gt', TIMESTAMP)), 'booth_quota_id');
            if (empty($booth_info)) {
        ...
        $bootgoods_info = $model_booth->getBoothGoodsList(array('store_id' => $_SESSION['seller_session_info']['store_id']), 'goods_id');
        // 已添加商品总数
        ...
        // 保存到推荐展位商品表
        $insert = array();
        $insert['store_id'] = $_SESSION['seller_session_info']['store_id'];
    ...
    public function del_choosed_goodsOp() {
        ...
        $result = Model('p_booth')->delBoothGoods(array('goods_id' => $gid, 'store_id' => $_SESSION['seller_session_info']['store_id']));
        if ($result) {
