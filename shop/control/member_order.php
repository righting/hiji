...
class member_orderControl extends BaseMemberControl {
    ...
    public function __construct() {
        ...
        Tpl::output('current_active_name','member_order');
    }
    ...
    public function indexOp() {
        ...
        if ($_GET['state_type'] != '') {
            $condition['order_state'] = str_replace(
                    array('state_new','state_pay','state_send','state_noeval','state_cancel'),
    ...
    public function show_orderOp() {
        ...
        if ($order_info['order_state'] == ORDER_STATE_SEND) {
            $order_info['order_confirm_day'] = $order_info['delay_time'] + ($list_setting['order_time']+$order_info['delays_time']) * 24 * 3600;
        }
    ...
    public function change_stateOp() {
        ...
        } else if ($_GET['state_type'] == 'order_thaw') {
            // 解冻订单-
            //给用户增加积分
            $this->_order_point($order_info,$_POST);
            //解冻后进入分红
            $result = $this->_order_thaw($order_info, $_POST);
        } else if ($_GET['state_type'] == 'order_delay') {
            // 延迟收货
            $result = $this->_order_delay($order_info, $_POST);
        ...
        if(!$result['state']) {
            ...
            showDialog($result['msg'],'reload','succ','',3);
        }
    }
     ...
     * @param $post
     */
    private function _order_point($order_info, $post) {
        if (!chksubmit()) {
            Tpl::output('order_info', $order_info);
            Tpl::showpage('member_order.thaw','null_layout');
            exit();
        } else {
        if (C('points_isuse') == 1) {
            //获取订单信息
            $info=Model()->table('orders')->where(['order_id'=>$order_info['order_id']])->find();
            /**
             * 修正可获得的HS积分 By Jimlin 2018-11-24 18:28
             */
            if($info['hs_count'] > 0){
                $ht_data['pl_memberid'] = $order_info['buyer_id'];
                $ht_data['pl_membername'] = $order_info['buyer_name'];
                $ht_data['orderprice'] = $info['hs_count'];
            ...
            if($info['h_count'] > 0){
                ...
                $h_data['orderprice'] = $info['h_count'];
     ...
     * @return multitype
     */
    private function _order_thaw($order_info, $post) {
        if (!chksubmit()) {
            Tpl::output('order_info', $order_info);
            Tpl::showpage('member_order.thaw','null_layout');
            exit();
            ...
            $model_order = Model('order');
            $logic_order = Logic('order');
            $if_allow = $model_order->getOrderOperateState('thaw',$order_info);
            if (!$if_allow) {
                return callback(false,'无权操作');
            }
            ...
            $return = $logic_order->changeOrderStateThaw($order_info,'buyer',$_SESSION['member_name'],'解冻订单');
     ...
     * @return multitype
     */
    private function _order_delay($order_info, $post) {
        if (!chksubmit()) {
            Tpl::output('order_info', $order_info);
            Tpl::showpage('member_order.delay','null_layout');
            exit();
        } else {
            $model_order = Model('order');
            $logic_order = Logic('order');
            $if_allow = $model_order->getOrderOperateState('delay',$order_info);
            if (!$if_allow) {
                return callback(false,'无权操作');
            }

            $return = $logic_order->changeOrderStateDelay($order_info,'buyer',$_SESSION['member_name'],'订单延迟收货');
            return $return;
        }
    }
