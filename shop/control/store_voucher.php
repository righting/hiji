...
class store_voucherControl extends BaseSellerControl{
    ...
    public function __construct() {
        ...
        if (C('voucher_allow') != 1){
            showMessage(Language::get('voucher_unavailable'),'index.php?controller=store','html','error');
        }
        //申请记录状态
    ...
    public function templatelistOp(){
        ...
        if (checkPlatformStore()) {
            ...
            //查询是否存在可用套餐
            $current_quota = $model_voucher->getCurrentQuota($_SESSION['seller_session_info']['store_id']);
        ...
        $param['voucher_t_store_id'] = $_SESSION['seller_session_info']['store_id'];
    ...
    public function voucherlistOp(){
        ...
        $where['voucher_t_store_id'] = $_SESSION['seller_session_info']['store_id'];
    ...
    public function voucher_exportOp(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){//显示下载链接
                ...
                Tpl::output('murl','index.php?controller=store_voucher&action=voucher_export');
    ...
    public function quotaaddOp(){
        if (chksubmit()){
            ...
            if(empty($current_quota)) {
                ...
                $param['quota_storeid'] = $_SESSION['seller_session_info']['store_id'];
                $param['quota_storename'] = $_SESSION['seller_session_info']['store_name'];
            ...
            if($reault){
                showDialog(Language::get('voucher_apply_buy_succ'),'index.php?controller=store_voucher&action=quotalist','succ');
            } else {
                showDialog(Language::get('nc_common_op_fail'),'index.php?controller=store_voucher&action=quotalist');
            }
        }else {
            //输出导航
    ...
    public function templateaddOp(){
        ...
        if ($isOwnShop = checkPlatformStore()) {
            ...
            //查询当前可以套餐
            $quotainfo = $model->getCurrentQuota($_SESSION['seller_session_info']['store_id']);
            if(empty($quotainfo)){
                showMessage(Language::get('voucher_template_quotanull'),'index.php?controller=store_voucher&action=quotaadd','html','error');
            }
            ...
            if ($count >= C('promotion_voucher_storetimes_limit')){
                ...
                showMessage($message,'index.php?controller=store_voucher&action=templatelist','html','error');
            }
        }
        ...
        if(empty($pricelist)){
            showMessage(Language::get('voucher_template_pricelisterror'),'index.php?controller=store_voucher&action=templatelist','html','error');
        }
        //获取领取方式
        ...
        if(chksubmit()){
            ...
            if ($error){
                ...
                $insert_arr['voucher_t_store_id'] = $_SESSION['seller_session_info']['store_id'];
                $insert_arr['voucher_t_storename'] = $_SESSION['seller_session_info']['store_name'];
                ...
                if (!empty($_FILES['customimg']['name'])){
                    ...
                    $upload->set('default_dir', ATTACH_VOUCHER.DS.$_SESSION['seller_session_info']['store_id']);
                ...
                if($rs){
                    ...
                    showDialog(Language::get('nc_common_save_succ'),'index.php?controller=store_voucher&action=templatelist','succ');
                }else{
                    showDialog(Language::get('nc_common_save_fail'),'index.php?controller=store_voucher&action=templatelist','error');
                }
            }
        }else{
            //店铺分类
            ...
            //查询店铺详情
            $store_info = Model('store')->getStoreInfoByID($_SESSION['seller_session_info']['store_id']);
    ...
    public function templateeditOp(){
        ...
        if ($t_id <= 0){
            showMessage(Language::get('wrong_argument'),'index.php?controller=store_voucher&action=templatelist','html','error');
        }
        $model = Model('voucher');
        //查询模板信息
        $param = array();
        ...
        if ($isOwnShop = checkPlatformStore()) {
            ...
            //查询套餐信息
            $quotainfo = $model->table('voucher_quota')->where(array('quota_id'=>$t_info['voucher_t_quotaid'],'quota_storeid'=>$_SESSION['seller_session_info']['store_id']))->find();
            if(empty($quotainfo)){
                showMessage(Language::get('voucher_template_quotanull'),'index.php?controller=store_voucher&action=quotaadd','html','error');
            }
        }
        ...
        if(chksubmit()){
            ...
            if ($error){
                ...
                if (!empty($_FILES['customimg']['name'])){
                    ...
                    if ($result){
                        ...
                        if (!empty($t_info['voucher_t_customimg'])){//如果模板存在，则删除原模板图片
                            @unlink(BASE_UPLOAD_PATH.DS.ATTACH_VOUCHER.DS.$_SESSION['seller_session_info']['store_id'].DS.$t_info['voucher_t_customimg']);
                            @unlink(BASE_UPLOAD_PATH.DS.ATTACH_VOUCHER.DS.$_SESSION['seller_session_info']['store_id'].DS.str_ireplace('.', '_small.', $t_info['voucher_t_customimg']));
                        }
                ...
                if($rs){
                    ...
                    showDialog(Language::get('nc_common_op_succ'),'index.php?controller=store_voucher&action=templatelist','succ');
                }else{
                    showDialog(Language::get('nc_common_op_fail'),'index.php?controller=store_voucher&action=templatelist','error');
                }
            }
        }else{
            if (!$t_info['voucher_t_customimg'] || !file_exists(BASE_UPLOAD_PATH.DS.ATTACH_VOUCHER.DS.$_SESSION['seller_session_info']['store_id'].DS.$t_info['voucher_t_customimg'])){
                $t_info['voucher_t_customimg'] = UPLOAD_SITE_URL.DS.defaultGoodsImage(240);
            }else{
                $t_info['voucher_t_customimg'] = UPLOAD_SITE_URL.DS.ATTACH_VOUCHER.DS.$_SESSION['seller_session_info']['store_id'].DS.str_ireplace('.', '_small.', $t_info['voucher_t_customimg']);
            }
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'voucher':
                $menu_array = array(
                1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('nc_member_path_store_voucher'), 'menu_url'=>'index.php?controller=store_voucher&action=templatelist'),
                );
                break;
            case 'quota_add':
                $menu_array = array(
                1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('nc_member_path_store_voucher'), 'menu_url'=>'index.php?controller=store_voucher&action=templatelist'),
                4=>array('menu_key'=>'quotaadd','menu_name'=>Language::get('voucher_applyadd'), 'menu_url'=>'index.php?controller=store_voucher&action=quotaadd')
                );
                break;
            case 'template':
                $menu_array = array(
                1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('nc_member_path_store_voucher'), 'menu_url'=>'index.php?controller=store_voucher&action=templatelist'),
                2=>array('menu_key'=>'templateadd','menu_name'=>Language::get('voucher_templateadd'),   'menu_url'=>'index.php?controller=store_voucher&action=templateadd'),
                );
                break;
            case 'templateedit':
                $menu_array = array(
