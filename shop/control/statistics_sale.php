...
class statistics_saleControl extends BaseSellerControl {
    ...
    public function salelistOp(){
        ...
        Tpl::output('show_page',$model->showpage(2));
        Tpl::output('actionurl',"index.php?controller={$this->search_arr['controller']}&action={$this->search_arr['action']}&order_type={$this->search_arr['order_type']}&t={$this->search_arr['t']}");
    ...
    public function buyingOp(){
        ...
        $where['order_add_time'] = array('between',$searchtime_arr);
        $field = '1';
        $pricerange = Model('store_extend')->getfby_store_id($_SESSION['seller_session_info']['store_id'],'orderpricerange');
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array(
            1=>array('menu_key'=>'sale','menu_name'=>'销售统计','menu_url'=>'index.php?controller=statistics_sale&action=sale'),
            2=>array('menu_key'=>'area','menu_name'=>'区域分布','menu_url'=>'index.php?controller=statistics_sale&action=area'),
            3=>array('menu_key'=>'buying','menu_name'=>'购买分析','menu_url'=>'index.php?controller=statistics_sale&action=buying'),
        );
