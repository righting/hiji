...
class store_promotion_bookControl extends BaseSellerControl {
    ...
    public function book_listOp() {
        ...
        if ($hasList) {
            $goods_list = $model_book->getAllGoodsList(array('store_id' => $_SESSION['seller_session_info']['store_id']), '*', null);
            if (!empty($goods_list)) {
    ...
    public function choosed_goodsOp() {
        ...
        if (!checkPlatformStore()) {
            // 验证套餐时候过期
            $book_info = $model_book->getBookQuotaInfo(array('store_id' => $_SESSION['seller_session_info']['store_id'], 'bkq_endtime' => array('gt', TIMESTAMP)));
            if (empty($book_info)) {
                if ($_GET['inajax']) {
