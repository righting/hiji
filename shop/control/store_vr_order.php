...
class store_vr_orderControl extends BaseSellerControl {
    ...
    private function _exchange() {
        ...
        if (empty($vr_code_info) || $vr_code_info['store_id'] != $_SESSION['seller_session_info']['store_id']) {
        ...
        if ($update) {
            ...
            $order_info['img_60'] = cthumb($order_info['goods_image'],60);
            $order_info['img_240'] = cthumb($order_info['goods_image'],240);
    ...
    private function profile_menu($menu_type='',$menu_key='') {
        ...
        switch ($menu_type) {
            case 'list':
            $menu_array = array(
            array('menu_key'=>'store_order',        'menu_name'=>Language::get('nc_member_path_all_order'), 'menu_url'=>'index.php?controller=store_vr_order'),
            array('menu_key'=>'state_new',          'menu_name'=>Language::get('nc_member_path_wait_pay'),  'menu_url'=>'index.php?controller=store_vr_order&action=index&state_type=state_new'),
            array('menu_key'=>'state_pay',          'menu_name'=>'已付款',  'menu_url'=>'index.php?controller=store_vr_order&action=index&state_type=state_pay'),
            array('menu_key'=>'state_success',      'menu_name'=>Language::get('nc_member_path_finished'),  'menu_url'=>'index.php?controller=store_vr_order&action=index&state_type=state_success'),
            array('menu_key'=>'state_cancel',       'menu_name'=>Language::get('nc_member_path_canceled'),  'menu_url'=>'index.php?controller=store_vr_order&action=index&state_type=state_cancel'),
            );
            break;
            case 'exchange':
                $menu_array = array(
                ...
                array('menu_key'=>'state_success',      'menu_name'=>Language::get('nc_member_path_finished'),  'menu_url'=>'index.php?controller=store_vr_order&action=index&state_type=state_success'),
                array('menu_key'=>'state_cancel',       'menu_name'=>Language::get('nc_member_path_canceled'),  'menu_url'=>'index.php?controller=store_vr_order&action=index&state_type=state_cancel'),
                array('menu_key'=>'exchange',           'menu_name'=>'兑换码兑换',  'menu_url'=>'index.php?controller=store_vr_order&action=exchange'),
                );
                break;
        }
