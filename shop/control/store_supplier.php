...
class store_supplierControl extends BaseSellerControl {
    ...
    public function sup_listOp() {
        ...
        $condition = array();
        $condition['sup_store_id'] = $_SESSION['seller_session_info']['store_id'];
        if ($_GET['sup_name'] != '') {
    ...
    public function sup_addOp() {
        $model_sup = Model('store_supplier');
        if($_GET['sup_id'] != '') {
            $sup_info = $model_sup->getStoreSupplierInfo(array('sup_id' => $_GET['sup_id'], 'sup_store_id' => $_SESSION['seller_session_info']['store_id']));
            if (empty($sup_info)){
    ...
    public function sup_saveOp(){
        ...
        $data['sup_store_id'] = $_SESSION['seller_session_info']['store_id'];
        $data['sup_store_name'] = $_SESSION['seller_session_info']['store_name'];
        if ($_POST['sup_id']) {
            $condition = array();
    ...
    public function sup_delOp() {
        ...
        if ($sup_id > 0){
            $model_sup->delStoreSupplier(array('sup_id'=>$sup_id, 'sup_store_id' => $_SESSION['seller_session_info']['store_id']));
            showDialog(Language::get('nc_common_del_succ'),'index.php?controller=store_supplier&action=sup_list','succ');
        }else {
    ...
    private function profile_menu($menu_type, $menu_key = '', $array = array()) {
        ...
        switch ($menu_type) {
        	case 'sup_list':
        	    $menu_array = array(
        	    array('menu_key'=>'sup_list', 'menu_name'=>'供货商', 'menu_url'=>'index.php?controller=store_supplier&action=sup_list')
        	    );
        	    break;
        }
        if(!empty($array)) {
