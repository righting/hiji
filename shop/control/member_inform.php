...
class member_informControl extends BaseMemberControl{
    ...
    public function __construct() {
        ...
        Tpl::output('current_active_name','member_inform');
        //定义状态常量
    }
    ...
    public function inform_saveOp() {
        ...
        if($model_inform->saveInform($input)) {
            showDialog(Language::get('inform_success'),'index.php?controller=member_inform&action=inform_list','succ');
        }
        else {
            showDialog(Language::get('inform_fail'),'index.php?controller=member_inform&action=inform_list','error');
        }
    }
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array(
            1=>array('menu_key'=>'inform_list','menu_name'=>'违规举报', 'menu_url'=>'index.php?controller=member_inform&action=inform_list'),
        );
