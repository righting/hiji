...
class store_goods_classControl extends BaseSellerControl {
    ...
    public function indexOp() {
        ...
        if($_GET['type'] == 'ok') {
            ...
            $goods_class        = $model_class->getStoreGoodsClassList(array('store_id'=>$_SESSION['seller_session_info']['store_id'],'stc_parent_id'=>0));
            Tpl::output('goods_class',$goods_class);
            Tpl::showpage('store_goods_class.add','null_layout');
        } else {
            $goods_class        = $model_class->getTreeClassList(array('store_id'=>$_SESSION['seller_session_info']['store_id']),2);
            $str    = '';
    ...
    public function goods_class_saveOp() {
        ...
        if(isset($_POST['stc_id'])) {
            ...
            if($state) {
                showDialog(Language::get('nc_common_save_succ'),'index.php?controller=store_goods_class&action=index','succ',empty($_GET['inajax']) ?'':'CUR_DIALOG.close();');
            } else {
            ...
            $class_array['stc_parent_id'] = $_POST['stc_parent_id'];
            $class_array['stc_state']     = $_POST['stc_state'];
            $class_array['store_id']      = $_SESSION['seller_session_info']['store_id'];
            $class_array['stc_sort']      = $_POST['stc_sort'];
    ...
    public function drop_goods_classOp() {
        ...
        if ($drop_state){
            showDialog(Language::get('nc_common_del_succ'),'index.php?controller=store_goods_class&action=store_goods_class','succ');
        }else{
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'store_goods_class':
                $menu_array = array(
                1=>array('menu_key'=>'store_goods_class','menu_name'=>'店铺分类',   'menu_url'=>'index.php?controller=store_goods_class&action=store_goods_class'));
                break;
        }
