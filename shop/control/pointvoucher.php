...
class pointvoucherControl extends BasePointShopControl {
    ...
    public function voucherexchangeOp(){
        ...
        if($_SESSION['is_login'] != '1'){
            ...
            $js = "CUR_DIALOG = ajax_form('vexchange', '".L('home_voucher_exchangtitle')."', 'index.php?controller=pointvoucher&action=voucherexchange&vid={$vid}', 550);";
            showDialog('','','js',$js);
            die;
        }
        $result = true;
        ...
        if ($result){
            //查询可兑换代金券模板信息
            $template_info = Model('voucher')->getCanChangeTemplateInfo($vid,intval($_SESSION['member_id']),intval($_SESSION['seller_session_info']['store_id']));
    ...
    public function voucherexchange_saveOp(){
        ...
        $model_voucher = Model('voucher');
        //验证是否可以兑换代金券
        $data = $model_voucher->getCanChangeTemplateInfo($vid,intval($_SESSION['member_id']),intval($_SESSION['seller_session_info']['store_id']));
        if ($data['state'] == false){
