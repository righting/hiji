...
//        $this->checkLogin();
    ...
    public function check_user_join_stateOp(){
        $model_seller = Model('seller');
        $seller_info = $model_seller->getSellerInfo(array('member_id' => $_SESSION['member_id']));
        if(!empty($seller_info)) {
            @header('location: index.php?controller=seller_login');
        }

        if($_GET['action'] != 'check_seller_name_exist' && $_GET['action'] != 'checkname') {
    ...
    private function check_joinin_state() {
        ...
        // 检查session中是否存在$_SESSION['enter_bind_mobile']，如果不存在，则弹框提示用户输入绑定的手机号来查询（因为不需要登录就能申请入驻平台供应商）
        $bind_mobile = isset($_SESSION['enter_bind_mobile']) ? $_SESSION['enter_bind_mobile'] : '';
        if($bind_mobile == ''){
            $this->search_progressOp();die;
        }
        $joinin_detail = $model_store_joinin->getOne(array('bind_mobile'=>$_SESSION['enter_bind_mobile']));
        if(!empty($joinin_detail)) {
            $this->joinin_detail = $joinin_detail;
            switch (intval($joinin_detail['joinin_state'])) {
                case STORE_JOIN_STATE_NEW:
                    $this->step4();
                    $this->show_join_message('入驻申请已经提交，请等待管理员审核,请牢记手机号码与邀请ID用于查询审核结果', FALSE, '3');
                    break;
                case STORE_JOIN_STATE_PAY:
                    $this->show_join_message('已经提交，请等待管理员核对后为您开通店铺', FALSE, '4');
                    break;
                case STORE_JOIN_STATE_VERIFY_SUCCESS:
                    if(!in_array($_GET['action'], array('pay', 'pay_save'))) {
                        $this->payOp();
                    }
                    break;
                case STORE_JOIN_STATE_VERIFY_FAIL:
                    if(!in_array($_GET['action'], array('step1', 'step2', 'step3', 'step4','ajax_upload_image'))) {
                        $this->show_join_message('审核失败:'.$joinin_detail['joinin_message'], SHOP_SITE_URL.DS.'index.php?controller=store_joinin&action=step1');
                    }
                    break;
                case STORE_JOIN_STATE_PAY_FAIL:
                    if(!in_array($_GET['action'], array('pay', 'pay_save'))) {
                        $this->show_join_message('付款审核失败:'.$joinin_detail['joinin_message'], SHOP_SITE_URL.DS.'index.php?controller=store_joinin&action=pay');
                    }
                    break;
    ...
    public function search_progressOp() {
        if(!empty($_POST)) {
            $mobile = $_POST['mobile'];
            ...
            $obj_validate->validateparam = array(
                ...
                array("input"=>$invite_id, "require"=>"true","validator"=>"Length","min"=>"1","max"=>"50","message"=>"邀请人ID不能为空且必须小于50个字"),
            );
            ...
            // 检查入驻申请信息是否存在
            $model_store_joinin = Model('store_joinin');
            $joinin_info = $model_store_joinin->getOne(['bind_mobile' => $mobile,'invite_id'=>$invite_id]);
            if(empty($joinin_info)) {
                // 如果信息不存在，提示信息不存在并跳到入驻申请首页
                showDialog('您输入的手机号未申请入驻，重新填写','','error');
            } else {
                // 如果信息存在，将手机号写入$_SESSION['enter_bind_mobile']，再次跳回首页
                $_SESSION['enter_bind_mobile'] = $mobile;
                $this->check_joinin_state();
            }
        }
        Tpl::output('step', '1');
        Tpl::output('sub_step', 'search_progress');

        showDialog('查询成功，即将跳转',urlShop('store_joinin','store_joinin_apply',['step'=>1,'sub_step'=>'search_progress']),'succ');
    }
    ...
    public function step2Op() {
        if(!empty($_POST)) {
            $param = array();
            $param['bind_mobile'] = $_POST['mobile'];
            if(empty($_POST['mobile'])){
                showDialog('手机号不能为空','','error');
            }
            $param['invite_id'] = $_POST['invite_id'];
            if(empty($_POST['invite_id'])){
                showDialog('请输入邀请人ID','','error');
            }
            ...
            $user_count = $member_model->where(['member_type'=>$member_model::MEMBER_TYPE_SUPPLIER,'member_mobile'=>$_POST['mobile']])->count();
            if($user_count > 0){
                showDialog('一个手机号只能绑定一个供应商','','error');
            }

            $invite_user_info = $member_model->where(['member_number'=>$_POST['invite_id']])->field('member_id,member_number,member_type,level_id,positions_id,is_dealers')->find();
            if(empty($invite_user_info)){
                showDialog('邀请人ID不存在','','error');
            }
            if((!in_array($invite_user_info['level_id'],[5,6])) && ($invite_user_info['is_dealers'] == 2)){
                showDialog('只限至尊会员和天尊会员或经销商才可向平台推荐供应商','','error');
            }
            $param['invite_user_id'] = $invite_user_info['member_id'];
            $param['invite_user_level_id'] = $invite_user_info['level_id'];

            $model_store_joinin = Model('store_joinin');
            $joinin_info = $model_store_joinin->getOne(array('bind_mobile' => $_POST['mobile']));
            if(empty($joinin_info)) {
                $model_store_joinin->save($param);
            } else {
                $model_store_joinin->modify($param, array('bind_mobile'=>$_POST['mobile']));
            }
            // 把绑定的手机号存入session
            $_SESSION['enter_bind_mobile'] = $_POST['mobile'];
        }
        showDialog('公司资质信息提交成功',urlShop('store_joinin','store_joinin_apply',['step'=>2,'sub_step'=>'step2']),'succ');
    }
    public function store_joinin_applyOp(){
        Tpl::output('step', $_GET['step']);
        Tpl::output('sub_step', $_GET['sub_step']);
        if ($_GET['step']==3){
            //商品分类
            $gc = Model('goods_class');
            $gc_list    = $gc->getGoodsClassListByParentId(0);
            $annual_fee_gc_id = rkcache('setting')['annual_fee_gc_id'];
    ...
    private function step2_save_valid($param) {
        $obj_validate = new Validate();
        $obj_validate->validateparam = array(
            array("input"=>$param["bind_mobile"], "require"=>"true", 'validator'=>'mobile',"message"=>'请正确填写手机号码'),
            array("input"=>$param['invite_id'], "require"=>"true","validator"=>"Length","min"=>"1","max"=>"50","message"=>"邀请人ID不能为空且必须小于50个字"),
...
//            array("input"=>$param['organization_code_electronic'], "require"=>"true","message"=>"组织机构代码电子版不能为空"),
    ...
    public function step3Op() {
        if(!empty($_POST)) {
            ...
            $model_store_joinin = Model('store_joinin');
            $model_store_joinin->modify($param, array('bind_mobile'=>$_SESSION['enter_bind_mobile']));
        }

        Tpl::output('step', '3');
        Tpl::output('sub_step', 'step3');
        Tpl::showpage('store_joinin_apply');
        showDialog('财务资质信息保存成功',urlShop('store_joinin','store_joinin_apply',['step'=>3,'sub_step'=>'step3']),'succ');
    }
...
//            array("input"=>$param['tax_registration_certif_elc'], "require"=>"true","message"=>"税务登记证号电子版不能为空"),
    ...
    public function step4Op() {
        ...
        $param['login_password'] = $_POST['password'];
        $this->step4_save_valid($param);
        unset($param['login_password']);
        $param['login_password'] = hj_password_hash($_POST['password']);
        $model_store_joinin = Model('store_joinin');
        $model_store_joinin->modify($param, array('bind_mobile'=>$_SESSION['enter_bind_mobile']));

       // @header('location: '.urlShop('store_joinin','check_user_join_state'));
        showDialog('申请提交成功',urlShop('store_joinin','check_user_join_state'),'succ');
    ...
    private function step4_save_valid($param) {
        ...
        $obj_validate->validateparam = array(
            ...
            array("input"=>$param['seller_name'], "require"=>"true","validator"=>"Length","min"=>"1","max"=>"50","message"=>"店铺名称不能为空且必须小于50个字"),
            array("input"=>$param['seller_name'], "require"=>"true","validator"=>"username",'regexp'=>"/^[a-zA-Z0-9_][a-zA-Z0-9_]+$/","message"=>"账户只能由字母、数字、下划线组成"),
            array("input"=>$param['login_password'], "require"=>"true","validator"=>"Length","min"=>"1","max"=>"20","message"=>"登录密码不能为空且必须小于20个字"),
    ...
    public function checknameOp() {
        ...
        if(!empty($store_info['store_name']) && $store_info['bind_mobile'] != $_SESSION['enter_bind_mobile']) {
            echo 'false';
        } else {
            echo 'true';
        }
     ...
     * 取消店铺申请
     */
    public function closeStoreApplyForOp(){
        $rs = ['status'=>-1];
        $mobile =  isset($_REQUEST['mobile'])?$_REQUEST['mobile']:'';
        $number =  isset($_REQUEST['number'])?$_REQUEST['number']:'';
        if(!empty($mobile) && !empty($number)){
            // 检查入驻申请信息是否存在
            $modelStoreJoinin = Model('store_joinin');
            $joinin_info = $modelStoreJoinin->getOne(['bind_mobile' => $mobile,'invite_id'=>$number]);
            if(!empty($joinin_info)) {
                //删除入驻申请
                $del=$modelStoreJoinin->where(['bind_mobile' => $mobile,'invite_id'=>$number,'joinin_state'=>['in','10,20']])->delete();
                if($del!==false){
                    $rs['status']=1;
                }
            }
