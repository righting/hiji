...
//         Tpl::output('help_list',$help_list);
        ...
        if (!checkPlatformStore()) {
            if (C('groupbuy_allow') == 1){
                // 团购套餐
                $groupquota_info = Model('groupbuy_quota')->getGroupbuyQuotaCurrent($_SESSION['seller_session_info']['store_id']);
                Tpl::output('groupquota_info', $groupquota_info);
            }
            if (intval(C('promotion_allow')) == 1){
                // 限时折扣套餐
                $xianshiquota_info = Model('p_xianshi_quota')->getXianshiQuotaCurrent($_SESSION['seller_session_info']['store_id']);
                Tpl::output('xianshiquota_info', $xianshiquota_info);
                // 满即送套餐
                $mansongquota_info = Model('p_mansong_quota')->getMansongQuotaCurrent($_SESSION['seller_session_info']['store_id']);
                Tpl::output('mansongquota_info', $mansongquota_info);
                // 优惠套装套餐
                $binglingquota_info = Model('p_bundling')->getBundlingQuotaInfoCurrent($_SESSION['seller_session_info']['store_id']);
                Tpl::output('binglingquota_info', $binglingquota_info);
                // 推荐展位套餐
                $boothquota_info = Model('p_booth')->getBoothQuotaInfoCurrent($_SESSION['seller_session_info']['store_id']);
                Tpl::output('boothquota_info', $boothquota_info);
            }
            if (C('voucher_allow') == 1){
                $voucherquota_info = Model('voucher')->getCurrentQuota($_SESSION['seller_session_info']['store_id']);
    ...
    public function statisticsOp() {
        ...
        // 全部商品数
        $goodscount = $model_goods->getGoodsCommonCount(array('store_id' => $_SESSION['seller_session_info']['store_id']));
        // 出售中的商品
        $goods_online = $model_goods->getGoodsCommonOnlineCount(array('store_id' => $_SESSION['seller_session_info']['store_id']));
        if (C('goods_verify')) {
            // 等待审核的商品
            $goods_waitverify = $model_goods->getGoodsCommonWaitVerifyCount(array('store_id' => $_SESSION['seller_session_info']['store_id']));
            // 审核失败的商品
            $goods_verifyfail = $model_goods->getGoodsCommonVerifyFailCount(array('store_id' => $_SESSION['seller_session_info']['store_id']));
        }
        // 仓库待上架的商品
        $goods_offline = $model_goods->getGoodsCommonOfflineCount(array('store_id' => $_SESSION['seller_session_info']['store_id']));
        // 违规下架的商品
        $goods_lockup = $model_goods->getGoodsCommonLockUpCount(array('store_id' => $_SESSION['seller_session_info']['store_id']));
        // 等待回复商品咨询
        if (C('dbdriver') == 'mysqli') {
            $consult = Model('consult')->getConsultCount(array('store_id' => $_SESSION['seller_session_info']['store_id'], 'consult_reply' => ''));
        } else {
            $consult = Model('consult')->getConsultCount(array('store_id' => $_SESSION['seller_session_info']['store_id'], 'consult_reply' => array('exp', 'consult_reply IS NULL')));
        }

        // 商品图片数量
        $imagecount = Model('album')->getAlbumPicCount(array('store_id' => $_SESSION['seller_session_info']['store_id']));

        $model_order = Model('order');
        // 交易中的订单
        $progressing = $model_order->getOrderCountByID('store',$_SESSION['seller_session_info']['store_id'],'TradeCount');
        // 待付款
        $no_payment = $model_order->getOrderCountByID('store',$_SESSION['seller_session_info']['store_id'],'NewCount');
        // 待发货
        $no_delivery = $model_order->getOrderCountByID('store',$_SESSION['seller_session_info']['store_id'],'PayCount');

        $model_refund_return = Model('refund_return');
        // 售前退款
        $condition = array();
        $condition['store_id'] = $_SESSION['seller_session_info']['store_id'];
        $condition['refund_type'] = ['in',[1,3]];
        $condition['order_lock'] = 2;
        $condition['refund_state'] = array('in', [1,2,4,5,6]);
        ...
        $condition = array();
        $condition['accused_id'] = $_SESSION['seller_session_info']['store_id'];
        ...
        $condition = array();
        $condition['ob_store_id'] = $_SESSION['seller_session_info']['store_id'];
    ...
    function quicklink_addOp() {
        if(!empty($_POST['item'])) {
            $_SESSION['seller_session_info']['seller_quicklink'][$_POST['item']] = $_POST['item'];
        }
    ...
    function quicklink_delOp() {
        if(!empty($_POST['item'])) {
            unset($_SESSION['seller_session_info']['seller_quicklink'][$_POST['item']]);
        }
        $this->_update_quicklink();
        echo 'true';
    }
    ...
    private function _update_quicklink() {
        $quicklink = implode(',', $_SESSION['seller_session_info']['seller_quicklink']);
        $update_array = array('seller_quicklink' => $quicklink);
        $condition = array('seller_id' => $_SESSION['seller_session_info']['seller_id']);
