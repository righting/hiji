...
class store_deliverControl extends BaseSellerControl {
    ...
    public function sendOp(){
        ...
        if (chksubmit()){
            ...
            $result = $logic_order->changeOrderSend($order_info, 'seller', $_SESSION['seller_session_info']['seller_name'], $_POST);
            if (!$result['state']) {
        ...
        if ($order_info['extend_order_common']['daddress_id'] > 0 ){
            ...
            //取默认地址
            $daddress_info = $model_daddress->getAddressList(array('store_id'=>$_SESSION['seller_session_info']['store_id']),'*','is_default desc',1);
        ...
        if ($order_info['extend_order_common']['reciver_info']['dlyp'] != '') {
            ...
            //快递公司
            $my_express_list = Model()->table('store_extend')->getfby_store_id($_SESSION['seller_session_info']['store_id'],'express');
    ...
    public function send_address_selectOp() {
        ...
        $address_list = Model('daddress')->getAddressList(array('store_id'=>$_SESSION['seller_session_info']['store_id']));
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'deliver':
                $menu_array = array(
                array('menu_key'=>'deliverno',          'menu_name'=>Language::get('nc_member_path_deliverno'), 'menu_url'=>'index.php?controller=store_deliver&action=index&state=deliverno'),
                array('menu_key'=>'delivering',         'menu_name'=>Language::get('nc_member_path_delivering'),    'menu_url'=>'index.php?controller=store_deliver&action=index&state=delivering'),
                array('menu_key'=>'delivered',      'menu_name'=>Language::get('nc_member_path_delivered'), 'menu_url'=>'index.php?controller=store_deliver&action=index&state=delivered'),
                );
                break;
            case 'search':
                $menu_array = array(
                1=>array('menu_key'=>'nodeliver',           'menu_name'=>Language::get('nc_member_path_deliverno'), 'menu_url'=>'index.php?controller=store_deliver&action=index&state=nodeliver'),
                2=>array('menu_key'=>'delivering',          'menu_name'=>Language::get('nc_member_path_delivering'),    'menu_url'=>'index.php?controller=store_deliver&action=index&state=delivering'),
                3=>array('menu_key'=>'delivered',       'menu_name'=>Language::get('nc_member_path_delivered'), 'menu_url'=>'index.php?controller=store_deliver&action=index&state=delivered'),
