...
class store_snsControl extends BaseSellerControl{
    ...
    public function addOp() {
        ...
        // where条件
        $where = array('store_id' => $_SESSION['seller_session_info']['store_id']);
    ...
    public function image_uploadOp() {
        ...
        if ($album_limit > 0) {
            $album_count = $model_album->getCount(array('store_id' => $_SESSION['seller_session_info']['store_id']));
        ...
        $class_info = $model_album->getOne(array('store_id' => $_SESSION['seller_session_info']['store_id'], 'is_default' => 1), 'album_class');
        // 上传图片
        ...
        $upload->set('fprefix', $_SESSION['seller_session_info']['store_id']);
        ...
        $insert_array['store_id'] = $_SESSION['seller_session_info']['store_id'];
        $model_album->addPic($insert_array);

        $data = array ();
        $data ['image'] = cthumb($img_path, 240, $_SESSION['seller_session_info']['store_id']);
    ...
    public function store_sns_saveOp(){
        ...
        switch ($_POST['type']){
            ...
            case '10':
                ...
                $where = array('store_id'=>$_SESSION['seller_session_info']['store_id'],'goods_id'=>array('in',$goods_id_array));
        ...
        if($rs){
            showDialog(Language::get('nc_common_op_succ'), 'index.php?controller=store_sns', 'succ');
        }else{
    ...
    public function settingOp(){
        ...
        if(chksubmit()){
            $update = array();
            $update['sauto_storeid']        = $_SESSION['seller_session_info']['store_id'];
        ...
        $sauto_info = $model_storesnssetting->getStoreSnsSettingInfo(array('sauto_storeid' => $_SESSION['seller_session_info']['store_id']));
    ...
    private function profile_menu($menu_key) {
        $menu_array = array(
                1=>array('menu_key'=>'store_sns_add', 'menu_name'=>Language::get('store_sns_add'), 'menu_url'=>'index.php?controller=store_sns&action=add'),
                2=>array('menu_key'=>'store_sns_setting', 'menu_name'=>Language::get('store_sns_setting'), 'menu_url'=>'index.php?controller=store_sns&action=setting'),
                3=>array('menu_key'=>'store_sns_brower', 'menu_name'=>Language::get('store_sns_browse'), 'menu_url'=>urlShop('store_snshome', 'index', array('sid' => $_SESSION['seller_session_info']['store_id'])), 'target'=>'_blank')
        );
