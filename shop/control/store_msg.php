...
class store_msgControl extends BaseSellerControl {
    ...
    public function msg_listOp() {
        ...
        if (!$_SESSION['seller_session_info']['seller_is_admin']) {
            $where['smt_code'] = array('in', $_SESSION['seller_session_info']['seller_smt_limits']);
        }
    ...
    public function msg_infoOp() {
        ...
        if (!in_array($_SESSION['seller_session_info']['seller_id'], $sm_readids)) {
            // 消息阅读表插入数据
            $condition = array();
            $condition['seller_id'] = $_SESSION['seller_session_info']['seller_id'];
            $condition['sm_id'] = $sm_id;
            Model('store_msg_read')->addStoreMsgRead($condition);

            $update = array();
            $sm_readids[] = $_SESSION['seller_session_info']['seller_id'];
    ...
    public function msg_settingOp() {
        ...
        // 店铺接收设置
        $setting_list = Model('store_msg_setting')->getStoreMsgSettingList(array('store_id' => $_SESSION['seller_session_info']['store_id']), '*', 'smt_code');
    ...
    public function edit_msg_settingOp() {
        ...
        // 店铺消息接收设置
        $setting_info = Model('store_msg_setting')->getStoreMsgSettingInfo(array('smt_code' => $code, 'store_id' => $_SESSION['seller_session_info']['store_id']));
    ...
    private function checkIsAdmin() {
        return $_SESSION['seller_session_info']['seller_is_admin'] ? true : false;
    }
