...
{
    const LINK_COU_LIST = 'index.php?controller=store_promotion_cou&action=cou_list';
    ...
    public function cou_listOp()
    {
        if (checkPlatformStore()) {
            ...
            $current_cou_quota = Model('p_cou_quota')->getCurrentCouQuota($_SESSION['seller_session_info']['store_id']);
    ...
    public function cou_quota_add_saveOp()
    {
        ...
        $current_cou_quota= $couQuotaModel->getCurrentCouQuota($_SESSION['seller_session_info']['store_id']);
    ...
    public function cou_add_saveOp()
    {
        ...
        if ($result) {
            ...
            $editUrl = 'index.php?controller=store_promotion_cou&action=cou_edit&cou_id=' . $result;
    ...
    public function cou_editOp()
    {
        ...
        $data = $couModel->getRelationalCouDetailById($_GET['cou_id'], $_SESSION['seller_session_info']['store_id']);
        if (empty($data)) {
    ...
    public function cou_edit_saveOp()
    {
        ...
        $cou_info = $couModel->getCouInfoByID($couId, $_SESSION['seller_session_info']['store_id']);
        if (empty($cou_info)) {
            showDialog('参数错误');
        }
    ...
    public function cou_delOp()
    {
        ...
        $cou_info = $couModel->getCouInfoByID($cou_id, $_SESSION['seller_session_info']['store_id']);
        if (!$cou_info) {
            showDialog('参数错误');
        }
    ...
    private function profile_menu($menu_key='')
    {
        $menu_array = array(
            1 => array(
                'menu_key' => 'cou_list',
                'menu_name' => '活动列表',
                'menu_url' => 'index.php?controller=store_promotion_cou&action=cou_list',
            ),
        );
        switch ($menu_key) {
            case 'cou_quota_add':
                $menu_array[] = array(
                    ...
                    'menu_name' => '购买套餐',
                    'menu_url' => 'index.php?controller=store_promotion_cou&action=cou_add',
                );
                break;
            case 'cou_add':
                $menu_array[] = array(
