...
class goodsControl extends BaseGoodsControl
{
    ...
    public function indexOp()
    {
        ...
        // 获取推广码
        $referral_key = isset($_GET['referral_key']) ? (string)$_GET['referral_key'] : '';
        /*$user_idNumber =  trim(explode('.',$_SERVER['HTTP_HOST'])[0]);//使用二级域名前辍转化为推广码
        if (!empty($user_idNumber)){
            //根据微商会员ID编码获取微商会员id
            $ws_info = Model('member')->field('member_id')->where(['member_number'=>$user_idNumber,'is_distribution'=>1])->find();
            if ($ws_info){
                $referral_key = hj_encrypt($ws_info['member_id']);
            }
        }*/

        //获取微店推广码
        $user_Number_key = isset($_SESSION['referral_key']) ? $_SESSION['referral_key'] : '';
        if (!empty($user_Number_key)) {
            $referral_key = $user_Number_key;
        }
        ...
        $inform_switch = true;
        // 检测商品是否下架,检查是否为店主本人
        if ($goods_info['goods_state'] != 1 || $goods_info['goods_verify'] != 1 || $goods_info['store_id'] == $_SESSION['seller_session_info']['store_id']) {
            $inform_switch = false;
        }
        ...
        $goods_info['referral_key'] = $referral_key;
        ...
        // 获取分销类型
//        $distribution_type = $model_goods->returnDistributionGroup();
//        Tpl::output('distribution_type', $distribution_type);
        $seo_param                = [];
...
//        Tpl::output('distribution_type', $distribution_type);
        ...
        $seo_param['description'] = $goods_info['goods_description'];
        Model('seo')->type('product')->param($seo_param)->show();
        Tpl::output('action', 'ht');
        Tpl::showpage('goods');
    }

    //  @TODO 获取商品分销信息
    public function addDistributeGoodsToStoreOp()
    {
        if (!chksubmit()) {
            showDialog('参数错误');
        }
        ...
        // 根据商品id获取商品信息，检查该商品是否可以分销
        // 商品详细信息
        $goods_model = Model('goods');
        $goods_info  = $goods_model->getGoodsInfo(['goods_id' => $goods_id], 'goods_id,store_id,is_distribution_goods,distribution_type');

        if (empty($goods_info)) {
            showDialog(L('goods_index_no_goods'), '', 'error', empty($_GET['inajax']) ? '' : 'CUR_DIALOG.close();');
        }
        $store_model = Model('store');
        $store_info  = $store_model->where(['store_id' => $goods_info['store_id']])->field('store_id,member_id')->find();
        if ($user_id == $store_info['member_id']) {
            showDialog('自己不能分销自己店铺的商品', '', 'error', empty($_GET['inajax']) ? '' : 'CUR_DIALOG.close();');
        }
        // 生成带推广邀请码的商品详情链接
        $encrypt_str    = $goods_model->generateGoodsReferralKey($goods_info['goods_id'], $goods_info['store_id'], $store_info['member_id'], $user_id);
        $goods_info_uri = $encrypt_str;
        // 根据用户id获取用户的分销店铺的商品信息，检查当前用户是否满足分销该商品的条件
        // 如果满足条件，将该商品添加到当前用户的分销店铺
        $distribute_goods  = Model('distribute_goods');
        $data['user_id']   = $user_id;
        $data['goods_id']  = $goods_id;
        $data['store_id']  = $goods_info['store_id'];
        $data['goods_url'] = $goods_info_uri;
        $res               = $distribute_goods->addGoods($data);

        showDialog($res['msg'], '', 'notice', '', 0);
    }
    ...
    public function addbrowseOp()
    {
        ...
        Model('goods_browse')->addViewedGoods($goods_id, $_SESSION['member_id'], $_SESSION['seller_session_info']['store_id']);
        exit();
    }
    ...
    public function comments_listOp()
    {
        ...
        // 当前位置导航
        $nav_link_list   = Model('goods_class')->getGoodsClassNav($goods_info['gc_id'], 0);
        $nav_link_list[] = ['title' => $goods_info['goods_name'], 'link' => urlShop('goods', 'index', ['goods_id' => $goods_id])];
    ...
    public function salelogOp()
    {
        ...
        Tpl::output('order_type', [1 => '原价', 2 => '团购', 3 => '折扣', 4 => '套装', 5 => '赠品', 8 => '原价', 9 => '换购']);
        Tpl::output('order_vr_type', [0 => '原价', 1 => '团购']);
    ...
    private function checkConsultAble($store_id = 0)
    {
        //检查是否为店主本身
        $store_self = false;
        if (!empty($_SESSION['seller_session_info']['store_id'])) {
            if (($store_id == 0 && intval($_GET['store_id']) == $_SESSION['seller_session_info']['store_id']) || ($store_id != 0 && $store_id == $_SESSION['seller_session_info']['store_id'])) {
                $store_self = true;
            }
        }
        //查询会员信息
    ...
    public function save_consultOp()
    {
        ...
        //判断是否是店主本人
        if ($_SESSION['seller_session_info']['store_id'] && $goods_info['store_id'] == $_SESSION['seller_session_info']['store_id']) {
    ...
    public function show_chainOp()
    {
        ...
        $stock_list        = $model_chain_stock->getChainStockList(['goods_id' => $goods_id, 'stock' => ['gt', 0]], 'chain_id');
        if (!empty($stock_list)) {
            $chainid_array = [];
