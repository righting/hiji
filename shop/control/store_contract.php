...
class store_contractControl extends BaseSellerControl {
    ...
    public function __construct() {
        ...
        if (C('contract_allow') != 1){
            showDialog('需开启“消费者保障服务”功能','index.php?controller=seller_center&action=index','error');
        }
    ...
    public function indexOp(){
        ...
        if (!$item_list) {
            showDialog('平台尚未开启任何消费者保障服务','index.php?controller=seller_center&action=index','error');
        }
        //查询店铺保障项目开启情况
        $c_list = $model_contract->getContractList(array('ct_storeid'=>$_SESSION['seller_session_info']['store_id']));
        if ($c_list) {
    ...
    public function ctiapplyOp(){
        ...
        $where['ct_storeid'] = $_SESSION['seller_session_info']['store_id'];
    ...
    private function applyadd_platformstore($item_info, $c_info){
        ...
        try {
            ...
            if ($c_info) {
                ...
                $insert_arr = array();
                $insert_arr['ct_storeid'] = $_SESSION['seller_session_info']['store_id'];
                $insert_arr['ct_storename'] = $_SESSION['seller_session_info']['store_name'];
            ...
            QueueClient::push('updateStoreGoodsContract', array('store_id'=>$_SESSION['seller_session_info']['store_id'],'item_id'=>$item_info['cti_id']));
            showDialog('加入成功','index.php?controller=store_contract&action=index','succ');
        }catch (Exception $e){
    ...
    private function applyadd_joininstore($item_info, $c_info){
        ...
        try {
            ...
            $insert_arr['cta_storeid'] = $_SESSION['seller_session_info']['store_id'];
            $insert_arr['cta_storename'] = $_SESSION['seller_session_info']['store_name'];
            ...
            $model_contract->commit();
            showDialog('加入申请成功，请耐心等待审核结果','index.php?controller=store_contract&action=index','succ');
        }catch (Exception $e){
            $model_contract->rollback();
            showDialog($e->getMessage(), '', 'error');
        }
    ...
    public function applypayOp(){
        ...
        $where['cta_storeid'] = $_SESSION['seller_session_info']['store_id'];
        ...
        if(chksubmit()){
            try {
                ...
                $model_contract->commit();
                showDialog('支付保证金操作成功，请耐心等待审核结果','index.php?controller=store_contract&action=index','succ');
            }catch (Exception $e){
                $model_contract->rollback();
                showDialog($e->getMessage(), '', 'error');
            }
    ...
    public function contractlogOp()
    {
        ...
        $where['clog_storeid'] = $_SESSION['seller_session_info']['store_id'];
        $costlog_count = $model_contract->contractCostlogCount($where);
        //查询店铺保障服务操作日志
        $where = array();
        $where['log_itemid'] = $itemid;
        $where['log_storeid'] = $_SESSION['seller_session_info']['store_id'];
    ...
    private function applyquit_platformstore($item_info, $c_info){
        ...
        try {
            ...
            $model_contract->commit();
            QueueClient::push('updateStoreGoodsContract', array('store_id'=>$_SESSION['seller_session_info']['store_id'],'item_id'=>$item_info['cti_id']));
            showDialog('退出成功','index.php?controller=store_contract&action=index','succ');
        }catch (Exception $e){
            $model_contract->rollback();
            showDialog($e->getMessage(), '', 'error');
        }
    ...
    private function applyquit_joininstore($item_info, $c_info){
        ...
        try {
            ...
            $insert_arr['ctq_storeid'] = $_SESSION['seller_session_info']['store_id'];
            $insert_arr['ctq_storename'] = $_SESSION['seller_session_info']['store_name'];
            ...
            $model_contract->commit();
            showDialog('退出申请提交成功，请耐心等待审核结果','index.php?controller=store_contract&action=index','succ');
        }catch (Exception $e){
            $model_contract->rollback();
            showDialog($e->getMessage(), '', 'error');
        }
    ...
    private function saveContractLog($item_info, $log_msg){
        $insert_arr = array();
        $insert_arr['log_storeid'] = $_SESSION['seller_session_info']['store_id'];
        $insert_arr['log_storename'] = $_SESSION['seller_session_info']['store_name'];
   ...
   /**
    ...
    private function profile_menu($menu_type='',$menu_key='') {
        ...
        switch ($menu_type) {
            case 'log':
                $menu_array = array(
                    1=>array('menu_key'=>'index', 'menu_name'=>'消费者保障服务', 'menu_url'=>'index.php?controller=store_contract&action=index'),
