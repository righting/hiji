...
class store_promotion_xianshiControl extends BaseSellerControl {
    ...
    const LINK_XIANSHI_LIST = 'index.php?controller=store_promotion_xianshi&action=xianshi_list';
    const LINK_XIANSHI_MANAGE = 'index.php?controller=store_promotion_xianshi&action=xianshi_manage&xianshi_id=';
    ...
    public function __construct() {
        ...
        if (intval(C('promotion_allow')) !== 1){
            showMessage(Language::get('promotion_unavailable'),'index.php?controller=store','','error');
        }
    ...
    public function xianshi_listOp() {
        ...
        if (checkPlatformStore()) {
            ...
            $current_xianshi_quota = $model_xianshi_quota->getXianshiQuotaCurrent($_SESSION['seller_session_info']['store_id']);
    ...
    public function xianshi_saveOp() {
        ...
        $param['store_name'] = $_SESSION['seller_session_info']['store_name'];
    ...
    public function xianshi_edit_saveOp() {
        ...
        $xianshi_info = $model_xianshi->getXianshiInfoByID($xianshi_id, $_SESSION['seller_session_info']['store_id']);
        if(empty($xianshi_info) || !$xianshi_info['editable']) {
            showMessage(L('param_error'),'','','error');
        }
    ...
    public function xianshi_goods_addOp() {
        ...
        if(empty($goods_info) || $goods_info['store_id'] != $_SESSION['seller_session_info']['store_id']) {
            $data['result'] = false;
        ...
        if($xianshi_goods_info) {
            ...
            $data_array['store_id']         = $_SESSION['seller_session_info']['store_id'];
    ...
    public function xianshi_goods_price_editOp() {
        ...
        $xianshi_goods_info = $model_xianshi_goods->getXianshiGoodsInfoByID($xianshi_goods_id, $_SESSION['seller_session_info']['store_id']);
        if(!$xianshi_goods_info) {
            $data['result'] = false;
            $data['message'] = L('param_error');
            echo json_encode($data);die;
    ...
    public function xianshi_goods_deleteOp() {
        ...
        $xianshi_info = $model_xianshi->getXianshiInfoByID($xianshi_goods_info['xianshi_id'], $_SESSION['seller_session_info']['store_id']);
        if(!$xianshi_info) {
            $data['result'] = false;
            $data['message'] = L('param_error');
            echo json_encode($data);die;
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array(
            1=>array('menu_key'=>'xianshi_list','menu_name'=>Language::get('promotion_active_list'),'menu_url'=>'index.php?controller=store_promotion_xianshi&action=xianshi_list'),
        );
        switch ($menu_key){
            case 'xianshi_add':
                $menu_array[] = array('menu_key'=>'xianshi_add','menu_name'=>Language::get('promotion_join_active'),'menu_url'=>'index.php?controller=store_promotion_xianshi&action=xianshi_add');
                break;
            case 'xianshi_edit':
                $menu_array[] = array('menu_key'=>'xianshi_edit','menu_name'=>'编辑活动','menu_url'=>'javascript:;');
                break;
            case 'xianshi_quota_add':
                $menu_array[] = array('menu_key'=>'xianshi_quota_add','menu_name'=>Language::get('promotion_buy_product'),'menu_url'=>'index.php?controller=store_promotion_xianshi&action=xianshi_quota_add');
                break;
            case 'xianshi_manage':
                $menu_array[] = array('menu_key'=>'xianshi_manage','menu_name'=>Language::get('promotion_goods_manage'),'menu_url'=>'index.php?controller=store_promotion_xianshi&action=xianshi_manage&xianshi_id='.$_GET['xianshi_id']);
                break;
        }
