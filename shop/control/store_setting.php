...
class store_settingControl extends BaseSellerControl {
    ...
    public function store_settingOp(){
        ...
        $store_id = $_SESSION['seller_session_info']['store_id'];//当前店铺ID
        /**
         * 获取店铺信息
         */
        ...
        if (chksubmit()){
            ...
            $model_class->editStore($param, array('store_id' => $store_id));
            showDialog(Language::get('nc_common_save_succ'),'index.php?controller=store_setting&action=store_setting','succ');
        }
        /**
         * 实例化店铺等级模型
         */
    ...
    public function store_slideOp() {
        ...
        if ($_POST['form_submit'] == 'ok'){
            ...
            $model_store->editStore($update, array('store_id' => $_SESSION['seller_session_info']['store_id']));

            // 删除upload表中数据
            $model_upload->delByWhere(array('upload_type'=>3,'item_id'=>$_SESSION['seller_session_info']['store_id']));
            showDialog(Language::get('nc_common_save_succ'),'index.php?controller=store_setting&action=store_slide','succ');
        }

        // 删除upload中的无用数据
        $upload_info = $model_upload->getUploadList(array('upload_type'=>3,'item_id'=>$_SESSION['seller_session_info']['store_id']),'file_name');
    ...
    public function silde_image_uploadOp(){
        ...
        if(intval($_POST['file_id']) > 0){
            ...
            $insert_array['item_id']        = $_SESSION['seller_session_info']['store_id'];
    ...
    public function themeOp(){
        ...
        $store_info = $store_class->getStoreInfoByID($_SESSION['seller_session_info']['store_id']);
        /**
         * 主题配置信息
         */
        $style_data = array();
    ...
    public function set_themeOp(){
        ...
        if (!empty($style) && file_exists(BASE_TPL_PATH.DS.'/store/style/'.$style.'/images/preview.jpg')){
            $store_class = Model('store');
            $rs = $store_class->editStore(array('store_theme'=>$style), array('store_id' => $_SESSION['seller_session_info']['store_id']));
    ...
    public function store_mobileOp()
    {
        ...
        if (chksubmit()) {
            ...
            showDialog('保存成功', 'index.php?controller=store_setting&action=store_mobile', 'succ');
        }
    ...
    private function profile_menu($menu_key='') {
        ...
        $menu_array = array(
            1=>array('menu_key'=>'store_setting','menu_name'=>Language::get('nc_member_path_store_config'),'menu_url'=>'index.php?controller=store_setting&action=store_setting'),
            4=>array('menu_key'=>'store_slide','menu_name'=>Language::get('nc_member_path_store_slide'),'menu_url'=>'index.php?controller=store_setting&action=store_slide'),
            5=>array('menu_key'=>'store_theme','menu_name'=>'店铺主题','menu_url'=>'index.php?controller=store_setting&action=theme'),
        );
