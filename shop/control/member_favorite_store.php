...
class member_favorite_storeControl extends CollectMemberControl{
    ...
    public function favoritestoreOp(){
        ...
        //判断店铺是否为当前会员所有
        if ($fav_id == $_SESSION['seller_session_info']['store_id']){
        ...
        if (!empty($fav_arr) && is_array($fav_arr)){
            ...
            if (!empty($favorites_list) && is_array($favorites_list)){
                ...
                if (!empty($fav_arr) && $result){
                    ...
                    showDialog(Language::get('favorite_del_success'),'index.php?controller=member_favorite_store&action=fslist','succ');
                }
            }else {
    ...
    private function profile_menu($menu_type,$menu_key='') {
        $menu_array = array(
            array('menu_key'=>'fav_store','menu_name'=>Language::get('nc_member_path_collect_store'), 'menu_url'=>'index.php?controller=member_favorite_store&action=fslist')
        );
        if ($menu_key == 'more') {
