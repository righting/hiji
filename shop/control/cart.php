...
class cartControl extends BaseBuyControl
{
    ...
    public function __construct()
    {
        ...
        $op = isset($_GET['action']) ? $_GET['action'] : $_POST['action'];
        ...
        Tpl::output('current_active_name', 'cart');
    }
        ...
        if ($_SESSION['member_id']) {
            ...
            if (!empty($cart_list)) {
                foreach ($cart_list as $k => $cart) {
                    ...
                    $cart_array['list'][$k]['goods_image'] = cthumb($cart['goods_image'], 60);
            ...
            foreach ($cart_list as $key => $cart) {
                ...
                $value['goods_image'] = cthumb($cart['goods_image'], 60);
    ...
    public function addOp()
    {
        ...
        if ($insert) {
            //购物车商品种数记入cookie
            setNcCookie('cart_goods_num', $model_cart->cart_goods_num, 2 * 3600);
            $data = ['state' => 'true', 'num' => $model_cart->cart_goods_num, 'amount' => ncPriceFormat($model_cart->cart_all_price)];
        } else {
            $data = ['state' => 'false'];
        }
    ...
    public function updateOp()
    {
        ...
        if ($cart_info['bl_id'] == '0') {
            ...
            if (empty($goods_info)) {
                ...
                $return['msg']      = '商品已被下架';
                $return['subtotal'] = 0;
                QueueClient::push('delCart', ['buyer_id' => $_SESSION['member_id'], 'cart_ids' => [$cart_id]]);
            ...
            if (intval($goods_info['goods_storage']) < $quantity) {
                ...
                $model_cart->editCart(['goods_num' => $goods_info['goods_storage']], ['cart_id' => $cart_id, 'buyer_id' => $_SESSION['member_id']]);
                exit(json_encode($return));
            }
        } else {
