...
class brandControl extends BaseCoolControl
{
    public function indexOp()
    {
        // 查询商品分类
        $goods_class_model = Model('goods_class');
        //$goods_class_list  = $goods_class_model->where(['gc_parent_id' => 0, 'status' => 1])->order('gc_sort')->select();
        // 获取品牌列表
        $brand_model = Model('brand');
        $brand_list  = $brand_model->where(['brand_apply' => 1])->order('class_id desc,brand_sort asc')->select();

        $model_web_config = Model('web_config');
        $index_html       = $model_web_config->where(['web_id' => 621])->find();
        Tpl::output('index_html', $index_html);
        Tpl::output('brand_list', $brand_list);
        //Tpl::output('goods_class_list', $goods_class_list);
        Tpl::output('webTitle', ' - 炫酷品牌');
        Tpl::showpage('brand');
    }
    ...
    public function brand_listOp()
    {
        ...
        if ($_GET['gc_id']) {
            ...
            $gc_id  = ['brand_apply' => '1', 'class_id' => $gc_ids];
        } else {
            $gc_id = ['brand_apply' => '1'];
        }
        ...
        //页面输出
        Tpl::output('index_sign', '23');
        Model('seo')->type('brand')->show();
        Tpl::showpage('brand/brand');
    }
...
//        Tpl::output('nav_link_list',$nav_link);
        ...
        Model('seo')->type('brand_list')->param(['name' => $brand_info['brand_name'] . '_品牌_图片_系列'])->show();
