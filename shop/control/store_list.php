...
class store_listControl extends BaseHomeControl {
	...
	public function indexOp(){
		...
		if (intval($_GET['cate_id']) > 0){
			$nav_link[1]['link'] = 'index.php?controller=shop_search';
			...
			if ($nav['sc_parent_id'] > 0){
				...
				$nav_link[] = array(
					'title'=>$tmp['sc_name'],
					'link'=>"index.php?controller=store_list&cate_id=".$nav['sc_parent_id']
				);
			}
			//存入当前级
			$nav_link[] = array(
		...
		Tpl::output('purl', urlShop($purl['controller'], $purl['action'], $purl['param']));
	...
	function getParam() {
	    ...
	    unset($param['action']); unset($param['curpage']);
	    $purl['param'] = $param;
	    return $purl;
	}
}
