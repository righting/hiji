...
class store_goods_addControl extends BaseSellerControl {
    ...
    private function checkStore(){
        ...
        if ($goodsLimit > 0) {
            // 是否到达商品数上限
            $goods_num = Model('goods')->getGoodsCommonCount(array('store_id' => $_SESSION['seller_session_info']['store_id']));
            if ($goods_num >= $goodsLimit) {
                showMessage(L('store_goods_index_goods_limit') . $goodsLimit . L('store_goods_index_goods_limit1'), 'index.php?controller=store_goods_online&action=goods_list', 'html', 'error');
            }
        }
    }
    ...
    public function add_step_oneOp() {
        ...
        // 商品分类
        $goods_class = $model_goodsclass->getGoodsClass($_SESSION['seller_session_info']['store_id'],0,1,$_SESSION['seller_session_info']['seller_group_id'],$_SESSION['seller_session_info']['seller_gc_limits']);

        // 常用商品分类
        $model_staple = Model('goods_class_staple');
        $param_array = array();
        $param_array['member_id'] = $_SESSION['seller_session_info']['member_id'];
    ...
    public function add_step_twoOp() {
        ...
        if (!$_SESSION['seller_session_info']['seller_gc_limits']&& $_SESSION['seller_session_info']['seller_group_id']) {
            $rs = Model('seller_group_bclass')->getSellerGroupBclassInfo(array('group_id'=>$_SESSION['seller_session_info']['seller_group_id'],'gc_id'=>$gc_id));
            if (!$rs) {
        ...
        Tpl::output('goods_class', $goods_class);
        Model('goods_class_staple')->autoIncrementStaple($goods_class, $_SESSION['seller_session_info']['member_id']);

        // 获取类型相关数据
        $typeinfo = Model('type')->getAttr($goods_class['type_id'], $_SESSION['seller_session_info']['store_id'], $gc_id);
        ...
        if (chksubmit()) {
            ...
            foreach ($_POST['img'] as $key => $value) {
                ...
                foreach ($value as $v) {
                    ...
                    $tmp_insert['store_id']         = $_SESSION['seller_session_info']['store_id'];
    ...
    public function image_uploadOp() {
        ...
        // 上传到阿里云 OSS 商
        $result =  $logic_goods->uploadGoodsImageToAliOss(
            $_POST['name'],
            $_SESSION['seller_session_info']['store_id'],
            $this->store_grade['sg_album_limit']
        );
    ...
    public function ajax_goods_classOp() {
        ...
        $model_goodsclass = Model('goods_class');
        $list = $model_goodsclass->getGoodsClass($_SESSION['seller_session_info']['store_id'], $gc_id, $deep,$_SESSION['seller_session_info']['seller_group_id'],$_SESSION['seller_session_info']['seller_gc_limits']);
        if (empty($list)) {
            exit();
        }
        /**
    ...
    public function ajax_stapledelOp() {
        ...
        $result = $model_staple->delStaple(array('staple_id' => $staple_id, 'member_id' => $_SESSION['seller_session_info']['member_id']));
        if ($result) {
            echo json_encode ( array (
                    'done' => true
            ) );
    ...
    public function ajax_show_commOp() {
        ...
        if ($gc_id_1 > 0) {
            ...
            $class_list = $model_goods_class->getGoodsClass($_SESSION['seller_session_info']['store_id'],0,1,$_SESSION['seller_session_info']['seller_group_id'],$_SESSION['seller_session_info']['seller_gc_limits']);
        ...
        if ($gc_id_2 > 0) {
            ...
            $class_list = $model_goods_class->getGoodsClass($_SESSION['seller_session_info']['store_id'], $gc_id_1, 2,$_SESSION['seller_session_info']['seller_group_id'],$_SESSION['seller_session_info']['seller_gc_limits']);
            if (empty ( $class_list ) || ! is_array ( $class_list )) {
                echo json_encode ( array (
                        'done' => false,
                        'msg' => ''
        ...
        if ($gc_id_3 > 0) {
            ...
            $class_list = $model_goods_class->getGoodsClass($_SESSION['seller_session_info']['store_id'], $gc_id_2, 3,$_SESSION['seller_session_info']['seller_group_id'],$_SESSION['seller_session_info']['seller_gc_limits']);
            if (empty ( $class_list ) || ! is_array ( $class_list )) {
                echo json_encode ( array (
                        'done' => false,
                        'msg' => ''
