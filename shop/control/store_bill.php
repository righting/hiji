...
class store_billControl extends BaseSellerControl {
    ...
    public function indexOp() {
        ...
        $ext_info = $model_store_ext->getStoreExtendInfo(array('store_id'=>$_SESSION['seller_session_info']['store_id']));
    ...
    public function bill_printOp(){
		...
		$condition['ob_store_id'] = intval($_SESSION['seller_session_info']['store_id']);
        $bill_info = $model_bill->getOrderBillInfo($condition);
        if (!$bill_info){
            showMessage('参数错误','','html','error');
        }
    ...
    public function export_orderOp(){
        ...
        $model_bill = Model('bill');
        $bill_info = $model_bill->getOrderBillInfo(array('ob_id'=>intval($_GET['ob_id']),'ob_store_id'=>$_SESSION['seller_session_info']['store_id']));
        if (!$bill_info){
            showMessage('参数错误','','html','error');
        }
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){
                ...
                Tpl::output('murl','index.php?controller=store_bill&action=show_bill&ob_id='.$_GET['ob_id']);
    ...
    public function export_bookOp(){
        ...
        $condition['book_store_id'] = $_SESSION['seller_session_info']['store_id'];
    ...
    public function export_refund_orderOp(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('list',$array);
                Tpl::output('murl','index.php?controller=store_bill&action=show_bill&query_type=refund&ob_id='.$_GET['ob_id']);
                Tpl::showpage('store_export.excel');
                exit();
            }else{
                //如果数量小，直接下载
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'list':
                $menu_array = array(
                1=>array('menu_key'=>'list','menu_name'=>'实物订单结算', 'menu_url'=>'index.php?controller=bill&action=list'),
                );
                break;
            case 'show':
                $menu_array = array(
                array('menu_key'=>'order_list','menu_name'=>'订单列表', 'menu_url'=>'index.php?controller=store_bill&action=show_bill&ob_id='.$_GET['ob_id']),
                array('menu_key'=>'refund_list','menu_name'=>'退款订单','menu_url'=>'index.php?controller=store_bill&action=show_bill&type=refund&ob_id='.$_GET['ob_id']),
                array('menu_key'=>'cost_list','menu_name'=>'促销费用','menu_url'=>'index.php?controller=store_bill&action=show_bill&type=cost&ob_id='.$_GET['ob_id'])
                );
                if (floatval($this->_bill_info['ob_order_book_totals']) > 0) {
                    array_push($menu_array,array('menu_key'=>'book_list','menu_name'=>'未退定金', 'menu_url'=>'index.php?controller=store_bill&action=show_bill&type=book&ob_id='.$_GET['ob_id']));
                }
                break;
        }
