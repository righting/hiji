 ...
 *
 *
 **你正在使用的程序由海吉壹佰技术交流中心www.ccynet.cn提供！这里有专业化ccynet技术指导！以便你更好的了解这个程序*/
...
class taobao_importControl extends BaseSellerControl {
        ...
        if(!checkPlatformStore()){
            ...
            if (intval($this->store_grade['sg_goods_limit']) != 0) {
                if ($goods_num >= $this->store_grade['sg_goods_limit']) {
                    showMessage(L('store_goods_index_goods_limit') . $this->store_grade['sg_goods_limit'] . L('store_goods_index_goods_limit1'), 'index.php?controller=store_goods&action=goods_list', 'html', 'error');
                }
            }
        }
    }
	...
	public function indexOp(){
		...
		if(!$_POST){
			...
			$store_goods_class	= $model_store_class->getClassTree(array('store_id'=>$_SESSION['seller_session_info']['store_id'],'stc_state'=>'1'));
		   ...
		   if (strtoupper(CHARSET) == 'GBK'){
			...
			// 商品数量
			$goods_num=$model_store_goods->getGoodsCommonCount(array('store_id'=>$_SESSION['seller_session_info']['store_id']));
			
			/**
			 * 商品数,空间使用，使用期限判断
			 */
			$model_store	= Model('store');
			$store_info		= $model_store->getStoreInfo(array('store_id'=>$_SESSION['seller_session_info']['store_id']));
			...
			if(intval($store_grade['sg_goods_limit']) != 0) {
				if($goods_num >= $store_grade['sg_goods_limit']) {
					showMessage($lang['store_goods_index_goods_limit'].$store_grade['sg_goods_limit'].$lang['store_goods_index_goods_limit1'],'index.php?controller=store_goods&action=goods_list','html','error');
				}
			...
			if(intval($store_info['store_end_time']) != 0) {
				if(time() >= $store_info['store_end_time']) {
					showMessage($lang['store_goods_index_time_limit'],'index.php?controller=store_goods&action=goods_list','html','error');
				}
			}
			/**
			 * 循环添加数据
			...
			if(is_array($records) and count($records) > 0){
				foreach($records as $k=>$record){
					if($remain_num>0 and $k>=$remain_num){
						showMessage($lang['store_goods_index_goods_limit'].$store_grade['sg_goods_limit'].$lang['store_goods_index_goods_limit1'].$lang['store_goods_import_end'].(count($records)-$remain_num).$lang['store_goods_import_products_no_import'],'index.php?controller=store_goods&action=taobao_import&step=4','html','error');
					}
