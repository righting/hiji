...
class store_chainControl extends BaseSellerControl {
    ...
    public function chain_listOp() {
        $chain_list = $this->model_chain->getChainList(array('store_id' => $_SESSION['seller_session_info']['store_id']), '*', 10);
    ...
    public function chain_addOp() {
        ...
        if (chksubmit()){
            ...
            if (!empty($_FILES['chain_img']['name'])){
                $upload->set('default_dir', ATTACH_CHAIN.DS.$_SESSION['seller_session_info']['store_id']);
    ...
    public function chain_editOp() {
        ...
        $chain_info = $this->model_chain->getChainInfo(array('chain_id' => $chain_id, 'store_id' => $_SESSION['seller_session_info']['store_id']));
        ...
        if (chksubmit()){
            ...
            if (!empty($_POST['chain_img']) && !empty($chain_info['chain_img'])){
                @unlink(BASE_UPLOAD_PATH.DS.ATTACH_CHAIN.DS.$_SESSION['seller_session_info']['store_id'].DS.$chain_info['chain_img']);
            }
            ...
            $result = $this->model_chain->editChain($update, array('chain_id' => $chain_id, 'store_id' => $_SESSION['seller_session_info']['store_id']));
            if ($result) {
    ...
    public function chain_delOp() {
        ...
        $result = $this->model_chain->delChain(array('chain_id' => array('in', $chain_id), 'store_id' => $_SESSION['seller_session_info']['store_id']));
        if ($result) {
