...
class store_promotion_soleControl extends BaseSellerControl {
    ...
    public function sole_goods_listOp() {
        ...
        if ($hasList) {
            // 查询已选择商品
            $solegoods_list = $model_sole->getSoleGoodsList(array('store_id' => $_SESSION['seller_session_info']['store_id']));
    ...
    public function sole_quota_addOp() {
        if (chksubmit()) {
            ...
            if ($return) {
                ...
                Model('cron')->addCron(array('exetime' => $end_time, 'exeid' => $_SESSION['seller_session_info']['store_id'], 'type' => 10), true);
    ...
    public function choosed_goodsOp() {
        ...
        $solegoods_info = Model('p_sole')->getSoleGoodsInfo(array('store_id' => $_SESSION['seller_session_info']['store_id'], 'goods_id' => $gid));
    ...
    public function choosed_goods_saveOp() {
        ...
        if (!checkPlatformStore()) {
            // 验证套餐时候过期
            $sole_info = $model_sole->getSoleQuotaInfo(array('store_id' => $_SESSION['seller_session_info']['store_id'], 'sole_quota_endtime' => array('gt', TIMESTAMP)), 'sole_quota_id');
            if (empty($sole_info)) {
        ...
        $solegoods_info = $model_sole->getSoleGoodsInfo(array('store_id' => $_SESSION['seller_session_info']['store_id'], 'goods_id' => $goods_info['goods_id']));
    ...
    public function del_choosed_goodsOp() {
        ...
        $result = Model('p_sole')->delSoleGoods(array('goods_id' => $gid, 'store_id' => $_SESSION['seller_session_info']['store_id']));
        if ($result) {
