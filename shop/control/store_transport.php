...
class store_transportControl extends BaseSellerControl {
    ...
    public function listOp(){
        ...
        $list = $model_transport->getTransportList(array('store_id'=>$_SESSION['seller_session_info']['store_id']),4);
    ...
    public function deleteOp(){
        ...
        $model_transport = Model('transport');
        $transport = $model_transport->getTransportInfo(array('id'=>$id));
        if ($transport['store_id'] != $_SESSION['seller_session_info']['store_id']){
    ...
    public function saveOp(){
        ...
        $trans_info['store_id']     = $_SESSION['seller_session_info']['store_id'];
        ...
        if ($result){
            header('location: index.php?controller=store_transport&type='.$_POST['type']);exit;
        }else{
            showMessage(Language::get('transport_op_fail'),$_SERVER['HTTP_REFERER'],'html','error');
        }
    }
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            ...
            case 'offpay_area':
                $menu_array = array(
                1=>array('menu_key'=>'transport',   'menu_name'=>Language::get('nc_member_path_postage'),       'menu_url'=>'index.php?controller=store_transport'),
                );
                if (checkPlatformStore()) {
                    $menu_array[] = array(
                        'menu_key' => 'offpay_area',
                        'menu_name' => '货到付款地区',
                        'menu_url' => 'index.php?controller=store_transport&action=offpay_area'
                    );
                }
                break;
        }
