...
class member_vr_refundControl extends BaseMemberControl {
    public function __construct(){
        ...
        Tpl::output('controller', 'member_refund');
    }
    /**
     * 添加兑换码退款
     *
    ...
    public function add_refundOp(){
        ...
        if ($order_id < 1) {//参数验证
            showDialog(Language::get('wrong_argument'),'index.php?controller=member_vr_refund&action=index','error');
        }
        $condition = array();
        ...
        if (!$order['if_refund']) {//检查状态,防止页面刷新不及时造成数据错误
            showDialog(Language::get('wrong_argument'),'index.php?controller=member_vr_order&action=index','error');
        }
        if (chksubmit() && $order['if_refund']){
            ...
            if ($state) {
                showDialog(Language::get('nc_common_save_succ'),'index.php?controller=member_vr_refund&action=index','succ');
            } else {
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'member_order':
                ...
                array('menu_key'=>'buyer_vr_refund','menu_name'=>'虚拟兑码退款',    'menu_url'=>'index.php?controller=member_vr_refund'));
                break;
        }
