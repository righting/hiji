...
class store_specControl extends BaseSellerControl {
    ...
    public function indexOp() {
        ...
        $gc_list = $model_goodsclass->getGoodsClass($_SESSION['seller_session_info']['store_id'],0,1,$_SESSION['seller_session_info']['seller_group_id'],$_SESSION['seller_session_info']['seller_gc_limits']);
    ...
    public function add_specOp(){
        ...
        // 规格值信息
        $sp_value_list = $model_spec->getSpecValueList(array('store_id' => $_SESSION['seller_session_info']['store_id'], 'sp_id' => $sp_id, 'gc_id'=>$gc_id));
    ...
    public function ajax_delspecOp(){
        ...
        $rs = Model('spec')->delSpecValue(array('sp_value_id' => $sp_value_id, 'store_id' => $_SESSION['seller_session_info']['store_id']));
        if ($rs) {
            echo 'true'; exit();
        } else {
            echo 'false'; exit();
    ...
    public function ajax_classOp() {
        ...
        // 读取商品分类
        if ($deep != 5) {
            $gc_list = $model_goodsclass->getGoodsClass($_SESSION['seller_session_info']['store_id'],$id,$deep,$_SESSION['seller_session_info']['seller_group_id'],$_SESSION['seller_session_info']['seller_gc_limits']);
        }
        // 分类不为空输出分类信息
        if (!empty($gc_list)) {
    ...
    private function _profile_menu($menu_type, $menu_key) {
        ...
        switch ($menu_type) {
            case 'spec':
                $menu_array = array(
                    array('menu_key' => 'spec', 'menu_name' => L('nc_member_path_store_spec'), 'menu_url' => 'index.php?controller=store_spec')
                );
            break;
        }
