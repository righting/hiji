...
class store_goods_onlineControl extends BaseSellerControl {
    ...
    public function goods_listOp() {
        ...
        // 商品分类
        $store_goods_class = Model('store_goods_class')->getClassTree(array('store_id' => $_SESSION['seller_session_info']['store_id'], 'stc_state' => '1'));
    ...
    public function edit_classOp() {
        ...
        // 商品分类
        $goods_class = $model_goodsclass->getGoodsClass($_SESSION['seller_session_info']['store_id']);
    ...
    public function drop_goodsOp() {
        ...
        $result = Logic('goods')->goodsDrop($commonid_array, $_SESSION['seller_session_info']['store_id'], $_SESSION['seller_session_info']['seller_id'], $_SESSION['seller_session_info']['seller_name']);
        if ($result['state']) {
            // 添加操作日志
    ...
    public function goods_unshowOp() {
        $common_id = $this->checkRequestCommonId($_GET['commonid']);
        $commonid_array = explode(',', $common_id);
        $result = Logic('goods')->goodsUnShow($commonid_array, $this->store_info['store_id'], $_SESSION['seller_session_info']['seller_id'], $_SESSION['seller_session_info']['seller_name']);
        if ($result['state']) {
            showDialog(L('store_goods_index_goods_unshow_success'), getReferer() ? getReferer() : 'index.php?controller=store_goods_online&action=goods_list', 'succ', '', 2);
        } else {
    ...
    public function edit_jingleOp() {
        if (chksubmit()) {
            ...
            $commonid_array = explode(',', $common_id);
            $where = array('goods_commonid' => array('in', $commonid_array), 'store_id' => $_SESSION['seller_session_info']['store_id']);
    ...
    public function add_giftOp() {
        ...
        if (empty($goodscommon_info) || $goodscommon_info['store_id'] != $_SESSION['seller_session_info']['store_id']) {
            showMessage(L('wrong_argument'), urlShop('seller_center'), 'html', 'error');
        }
    ...
    public function save_giftOp() {
        ...
        // 验证商品是否存在
        $model_gift = Model('goods_gift');
        $goods_list = $model_gift->getAllowGiftGoodsList(array('goods_commonid' => $commonid, 'store_id' => $_SESSION['seller_session_info']['store_id']), 'goods_id');
        if (empty($goods_list)) {
            showDialog(L('wrong_argument'));
        }
        // 删除该商品原有赠品
        ...
        foreach ($data as $key => $val) {
            ...
            foreach ($val as $k => $v) {
                ...
                if ($gift_info['store_id'] != $_SESSION['seller_session_info']['store_id'] || $is_general == false) {
                    continue;
                }
   ...
   * 设置商品佣金
    ...
    public function get_goods_list_ajaxOp() {
        ...
        if (empty($goodscommon_list) || $goodscommon_list['store_id'] != $_SESSION['seller_session_info']['store_id']) {
            echo 'false';exit();
        }
        $goods_list = $model_goods->getGoodsList(array('store_id' => $_SESSION['seller_session_info']['store_id'], 'goods_commonid' => $common_id), 'goods_id,goods_spec,store_id,goods_price,goods_serial,goods_storage_alarm,goods_storage,goods_image,invite_rate');
        if (empty($goods_list)) {
            echo 'false';exit();
        }
		// 分销权限
    ...
    public function edit_body_ajaxOp() {
        ...
        $goods_info = $model_goods->getGoodsInfoByID($_GET['goods_id']);
        if ($goods_info['store_id'] == $_SESSION['seller_session_info']['store_id']) {
	...
	public function maker_qrcodeOp()
	{
        ...
        $PhpQRCode->set('pngTempDir',BASE_UPLOAD_PATH.DS.ATTACH_STORE.DS.$_SESSION['seller_session_info']['store_id'].DS);
		//print_r($PhpQRCode);
		$model_goods = Model('goods');
		$where=array();
