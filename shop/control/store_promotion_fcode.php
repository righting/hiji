...
class store_promotion_fcodeControl extends BaseSellerControl {
    ...
    public function fcode_listOp() {
        ...
        if ($hasList) {
            $goods_list = $model_fcode->getFCodeGoodsList(array('store_id' => $_SESSION['seller_session_info']['store_id']), '*', null);
            if (!empty($goods_list)) {
    ...
    public function choosed_goodsOp() {
        ...
        if (!checkPlatformStore()) {
            // 验证套餐时候过期
            $fcode_info = $model_fcode->getFCodeQuotaInfo(array('store_id' => $_SESSION['seller_session_info']['store_id'], 'fcq_endtime' => array('gt', TIMESTAMP)));
            if (empty($fcode_info)) {
                if ($_GET['inajax']) {
