...
class searchControl extends BaseHomeControl
{
    ...
    public function indexOp()
    {
        ...
        $channel = isset($_GET['channel'])?$_GET['channel']:'index';
        ...
        $goods_num = $model_goods->getGoodsCount($condition);
        ...
        if ($_GET['yx'] == 1) {
            Tpl::output('index_sign', 24);
        } elseif ($_GET['new'] == 1) {
            Tpl::output('index_sign', 25);
        }
    ...
    public function get_booth_goodsOp()
    {
        ...
        // 查询添加到推荐展位中的商品id
        $boothgoods_list = Model('p_booth')->getBoothGoodsList(['gc_id' => ['in', $gcid_array]], 'goods_id', 0, 5, 'rand()');
...
//             Log::record('search\auto_complete'.$e->getMessage(),Log::RUN);
     ...
     * 获取平台推荐商品
     */
    public function get_plrecmmendOp()
    {
        $pl_recm = Model('web_config')->where(['web_page' => 'rcm_goods'])->find();
        if (!empty($pl_recm)) {
            exit($pl_recm['web_html']);
        }
    }
    ...
    public function get_gc_goods_recommendOp()
    {
        ...
        if ($rec_gc_id > 0) {
            ...
            if (!empty($rec_list)) {
                $goods_list = Model('goods')->getGoodsOnlineList(['goods_id' => ['in', array_keys($rec_list)]]);
                if (!empty($goods_list)) {
                    Tpl::output('goods_list', $goods_list);
