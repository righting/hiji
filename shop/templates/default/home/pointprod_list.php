<?php defined('ByCCYNet') or exit('Access Invalid!');?><link href="<?php echo SHOP_TEMPLATES_URL;?>/css/ccy-main.css" rel="stylesheet" type="text/css"><style type="text/css"> .classtab a.curr, .category .hover .class{background: #eb0a00;}.public-head-layout .logo-test{ color:#eb0a00} .category .sub-class{ border-color: #eb0a00;}</style><link href="<?php echo SHOP_TEMPLATES_URL;?>/css/home_point.css" rel="stylesheet" type="text/css">
          ...
          <ul>
            ...
            <li>会员等级：
              <select id="level" onchange="javascript:searchpointprod();">
                ...
                <option value="<?php echo $v['level'];?>" <?php echo (isset($_GET['level']) && ($_GET['level'] == $v['level']))?'selected':'';?>><?php echo $v['level_name'];?></option>
                <?php } ?>
                <?php } ?>
              </select>
            </li>
...
function searchpointprod(){
	var url = 'index.php?controller=pointprod&action=index';
