  ...
  <ul class="list_pic">
    ...
    <li class="item">
      <div class="goods-content" nctype_goods=" <?php echo $value['goods_id'];?>" nctype_store="<?php echo $value['store_id'];?>">
        <div class="goods-pic">
            <a href="<?php echo urlShop('goods',$output['channel'],array('goods_id'=>$value['goods_id']));?>" target="_blank" title="<?php echo $value['goods_name'];?>">
                <img ccynet-url="<?php echo cthumb($value['goods_image'], 360,$value['store_id']);?>"   rel='lazy' src="<?php echo SHOP_SITE_URL;?>/img/loading.gif"  title="<?php echo $value['goods_name'];?>" alt="<?php echo $value['goods_name'];?>" />
            </a>
        </div>
		...
		<?php /*?> <div class="goods-pic-scroll-show">
            <ul>
              ...
              <li<?php if($i==1) {?> class="selected"<?php }?>><a href="javascript:void(0);"><img src="<?php echo cthumb($val['goods_image'], 60,$value['store_id']);?>"/></a></li>
              <?php }?>
              <?php } else {?>
        ...
        <div class="goods-info clearfix">
          ...
          <div class="goods-name"><a href="<?php echo urlShop('goods',$output['channel'],array('goods_id'=>$value['goods_id']));?>" target="_blank" title="<?php echo $value['goods_jingle'];?>"><?php echo $value['goods_name_highlight'];?><em><?php echo $value['goods_jingle'];?></em></a></div>
           <div class="goodsinfo clearfix"><a href="<?php echo urlShop('goods', 'comments_list', array('goods_id' => $value['goods_id']));?>" target="_blank" class="goods-num"><em class="ccy-icon"></em><?php echo $value['evaluation_count'];?></a>
               <a href="<?php echo urlShop('show_store','index',array('store_id'=>$value['store_id']), $value['store_domain']);?>" title="<?php echo $value['store_name'];?>" class="seller-name"><em class="ccy-icon"></em>&nbsp;</a></div>
           <div class="add-cart">
