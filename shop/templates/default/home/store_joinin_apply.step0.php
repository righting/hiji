...
<div id="apply_agreement" class="apply-agreement">
  ...
  <div class="bottom"><a style="display: none" id="btn_apply_agreecbc_next" href="javascript:;" class="btn">个人入驻</a><a style=" margin-left:15px;" id="btn_apply_agreement_next" href="javascript:;" class="btn">企业入驻</a></div>
</div>
<script type="text/javascript">
$(document).ready(function(){
	$('#btn_apply_agreecbc_next').on('click', function() {
        if($('#input_apply_agreement').prop('checked')) {
            window.location.href = "index.php?controller=store_joinincc&action=step1";
        } else {
            alert('请阅读并同意协议');
        }
    });
    $('#btn_apply_agreement_next').on('click', function() {
        if($('#input_apply_agreement').prop('checked')) {
            window.location.href = "index.php?controller=store_joinin&action=step1";
        } else {
            alert('请阅读并同意协议');
        }
    });
