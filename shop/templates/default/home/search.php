...
<div class="ccy-container">
            ...
            <div class="ccy-slide-pp clearfix">
                <ul id="files" class="tree clearfix">
                    <?php foreach ($output['goods_class_array'] as $value) { ?>
                        <li><i class="tree-parent tree-parent-collapsed ccy-slidenone"></i>
                            <div class="ccy-slidenone"><a
                                        href="<?php echo urlShop('search', 'index', array('cate_id' => $value['gc_id'], 'channel' => $output['channel'], 'keyword' => $_GET['keyword'])); ?>"
                                ...
                                <ul class="ccy-two clearfix">
                                    <?php foreach ($value['class2'] as $val) { ?>
                                        <li><i class="tree-parent tree-parent-collapsed"></i>
                                            <div class="ccy-slideone"><a
                                                        href="<?php echo urlShop('search', 'index', array('cate_id' => $val['gc_id'], 'channel' => $output['channel'], 'keyword' => $_GET['keyword'])); ?>"
                                                ...
                                                <ul class="ccy-three">
                                                    <?php foreach ($val['class3'] as $v) { ?>
                                                        <li class="tree-parent tree-parent-collapsed"><i></i><a
                                                                    href="<?php echo urlShop('search', 'index', array('cate_id' => $v['gc_id'],'channel' => $output['channel'], 'keyword' => $_GET['keyword'])); ?>"
                ...
                <?php if (!isset($output['checked_brand']) || empty($output['checked_brand'])) { ?>
                        ...
                        <dl>
                            ...
                            <dd class="list">
                                <ul class="ccy-brand-tab" nctype="ul_initial" style="display:none;">
                                    <li data-initial="all"><a href="javascript:void(0);">所有品牌<i class="arrow"></i></a>
                                    </li>
                                    <?php if (!empty($output['initial_array'])) { ?>
                                        <?php foreach ($output['initial_array'] as $val) { ?>
                                            <li data-initial="<?php echo $val; ?>"><a
                                                        href="javascript:void(0);"><?php echo $val; ?><i
                                                            class="arrow"></i></a></li>
                                        <?php } ?>
                                    <?php } ?>
                                </ul>
                                <div id="ncBrandlist">
                                    <ul class="ccy-brand-con" nctype="ul_brand">
                                        ...
                                        foreach ($output['brand_array'] as $k => $v) {
                                            $i++; ?>
                                            <li data-initial="<?php echo $v['brand_initial'] ?>"
                                                <?php if ($i > 16) { ?>style="display:none;"<?php } ?>><a
                                                        href="<?php $b_id = (($_GET['b_id'] != '' && intval($_GET['b_id']) != 0) ? $_GET['b_id'] . '_' . $k : $k);
                                                        echo replaceParam(array('b_id' => $b_id)); ?>">
                                                    <?php if ($v['show_type'] == 0) { ?>
                                                        <img src="<?php echo brandImage($v['brand_pic']); ?>"
                                                             alt="<?php echo $v['brand_name']; ?>"/> <span>
                    <?php echo $v['brand_name']; ?>
                            ...
                            <?php if (count($output['brand_array']) > 16) { ?>
                                <dd class="all"><span nctype="brand_show"><i
                                ...
                                <dd class="list">
                                    <ul>
                                        ...
                                        foreach ($val['value'] as $k => $v) {
                                            $i++; ?>
                                            <li <?php if ($i > 10){ ?>style="display:none" nc_type="none"<?php } ?>><a
        ...
        <nav class="sort-bar" id="main-nav">
            ...
            <div class="sortbar-filter" nc_type="more-filter">
                ...
                <ul>
                    ...
                    <!--新品上市-->
                    <li><a href="<?php if ($_GET['new'] == 1) {
                            echo dropParam(array('new'));
                        } else {
                            echo replaceParam(array('new' => '1'));
                        } ?>" <?php if ($_GET['new'] == 1) { ?>class="selected"<?php } ?>><i></i>新品</a></li>
                    <!--精选尖货-->
                    <li><a href="<?php if ($_GET['yx'] == 1) {
                            echo dropParam(array('yx'));
                        } else {
                            echo replaceParam(array('yx' => '1'));
                        } ?>" <?php if ($_GET['yx'] == 1) { ?>class="selected"<?php } ?>><i></i>精选</a></li>
                    <!--赠品-->
                ...
                <?php foreach ($output['hot_goods_list'] as $value) { ?>
                    <li nctype_goods="<?php echo $value['goods_id']; ?>"
                        ...
                        <div class="goods-name"><a
                                    href="<?php echo urlShop('goods', $output['channel'], array('goods_id' => $value['goods_id'])); ?>"
                                    target="_blank"
                                    title="<?php echo $value['goods_jingle']; ?>"><?php echo $value['goods_name']; ?></a>
                        </div>
                        <div class="goods-price"
                             title="商品价格<?php echo $lang['nc_colon'] . $lang['currency'] . ncPriceFormat($value['goods_promotion_price']); ?>"><?php echo ncPriceFormatForList($value['goods_promotion_price']); ?>
                            <em class="market-price"
                                title="市场价：<?php echo $lang['currency'] . $value['goods_marketprice']; ?>"><?php echo ncPriceFormatForList($value['goods_marketprice']); ?></em>
                        </div>
                    </li>
                <?php } ?>
            </ul>
    ...
    <?php if (!empty($output['viewed_goods']) && is_array($output['viewed_goods'])) { ?>
        <div class="s3-module">
        <div class="title"><h3><b><?php echo $lang['goods_class_viewed_goods']; ?></b>你最近一段时间浏览的商品</h3></div>
        <div class="content">
            <div class="s3-sidebar-viewed ps-container" id="ccySidebarViewed">
                <ul>
                    <?php foreach ($output['viewed_goods'] as $k => $v) { ?>
                        <li class="ccy-sidebar-bowers">
                            <div class="goods-pic"><a
                                        ...
                                        target="_blank"><img src="<?php echo cthumb($v['goods_image'], 240); ?>"
                            ...
                            <div class="goods-price"
                                 title="商品价格<?php echo $lang['nc_colon'] . $lang['currency'] . ncPriceFormat($value['goods_promotion_price']); ?>"><?php echo $lang['currency']; ?><?php echo ncPriceFormat($v['goods_promotion_price']); ?></div>
                        </li>
                    <?php } ?>
                </ul>
            </div>
            <a href="<?php echo SHOP_SITE_URL; ?>/index.php?controller=member_goodsbrowse&action=list"
               class="ccy-sidebar-all-viewed">全部浏览历史</a></div></div><?php } ?>
    </div>
</div>
