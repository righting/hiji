...
<div class="wrapper pr">
    ...
    <div class="ncs-detail<?php if ($output['store_info']['is_own_shop']) echo ' ownshop'; ?>">
        ...
        <div id="ncs-goods-picture" class="ncs-goods-picture">
            <div class="gallery_wrap">
                <div class="gallery"><img title="鼠标滚轮向上或向下滚动，能放大或缩小图片哦~"
                                          src="<?php echo $output["goods_image"]["0"]["1"] ?>" class="cloudzoom"
                                          data-cloudzoom="zoomImage: '<?php echo $output["goods_image"]["0"]["2"] ?>'">
                </div>
            </div>
        ...
        <div class="ncs-goods-summary">
            ...
            <dl>
                <dt><?php echo $lang['goods_index_goods_price']; ?><?php echo $lang['nc_colon']; ?></dt>
                <dd class="cost-price">
                    <strong><?php echo $lang['currency'] . $output['goods']['goods_price']; ?></strong></dd>
            </dl>
            <dl>
                <dt>预定价格<?php echo $lang['nc_colon']; ?></dt>
                <dd class="price">
                    <strong><?php echo $lang['currency'] . ncPriceFormat((floatval($output['goods']['book_down_payment']) + floatval($output['goods']['book_final_payment']))); ?></strong>
                </dd>
            </dl>
        </div>
        <div class="ncs-book-down">
            <div class="rule-price">
                <strong>定金</strong><?php echo $lang['currency'] . ncPriceFormat($output['goods']['book_down_payment']); ?>
            </div>
            <div class="rule-symbol">+</div>
            <div class="rule-price">
                <strong>尾款</strong><?php echo $lang['currency'] . ncPriceFormat($output['goods']['book_final_payment']); ?>
            </div>
        ...
        <dl>
            ...
            <dd>
                剩余<strong><?php echo $output['remain']['day']; ?></strong>天<strong><?php echo $output['remain']['hour']; ?></strong>时<strong><?php echo $output['remain']['minute']; ?></strong>分
            </dd>
        </dl>
        <!-- S 预定商品 -->
        <?php } ?>
        ...
        <?php if ($output['goods']['goods_state'] != 10 && $output['goods']['goods_verify'] == 1) { ?>
            ...
            <?php if (isset($output['goods']['promotion_type']) || $output['goods']['have_gift'] == 'gift' || !empty($output['goods']['jjg_explain']) || !empty($output['mansong_info'])) { ?>
              ...
              echo $lang['nc_man']; ?>
                                            ...
                                            <?php if (!empty($rule['discount'])) { ?>
                                                ...
                                                <em><?php echo $lang['currency'] . ncPriceFormat($rule['discount']); ?></em>
                                            <?php } ?>
                                            <?php if (!empty($rule['goods_id'])) { ?>
                                                ，<?php echo $lang['nc_gift']; ?><a
                                                href="<?php echo $rule['goods_url']; ?>"
                                                title="<?php echo $rule['mansong_goods_name']; ?>" target="_blank">
                                                    赠品</a>
                                            <?php } ?>
              </span> <span class="sale-rule-more" nctype="show-rule"><a
                                                    href="javascript:void(0);">共<strong><?php echo count($output['mansong_info']['rules']); ?></strong>项，展开查看<i></i></a></span>
                                        <div class="sale-rule-content" style="display: none;" nctype="rule-content">
                                            <div class="title"><span
                                                        class="sale-name">满即送</span>共<strong><?php echo count($output['mansong_info']['rules']); ?></strong>项，促销活动规则<a
                                                        href="javascript:;" nctype="hide-rule">关闭</a></div>
                                            <div class="content">
                                                ...
                                                <ul class="mjs-info">
                                                    <?php foreach ($output['mansong_info']['rules'] as $rule) { ?>
                                                        <li> <span class="sale-rule"><?php echo $lang['nc_man']; ?>
                                                                <em><?php echo $lang['currency'] . ncPriceFormat($rule['price']); ?></em>
                                                                <?php if (!empty($rule['discount'])) { ?>
                                                                    ， <?php echo $lang['nc_reduce']; ?>
                                                                    <em><?php echo $lang['currency'] . ncPriceFormat($rule['discount']); ?></em>
                                            ...
                                            <div class="bottom"><a
                                                        href="<?php echo urlShop('show_store', 'mansong_goods', array('store_id' => $output['store_info']['store_id'])); ?>"
                                                        class="url" target="_blank">查看更多店铺“满即送”活动商品</a></div>
                                        </div>
                                    </div>
                                <?php } ?>
                                <!--E 满就送 -->
                                ...
                                <?php if ($output['goods']['jjg_explain']) { ?>
                                    <div class="ncs-jjg"><span class="sale-name">加价购</span> <span
                                                class="sale-rule"><?php echo $output['goods']['jjg_explain']; ?></span>
                                        <span nctype="show-rule" class="sale-rule-more"><a
                                                    href="javascript:void(0);">共<strong><?php echo count($output['goods']['jjg_info']['levels']); ?></strong>项，展开查看<i></i></a></span>
                                        <div class="sale-rule-content" style="display: none;" nctype="rule-content">
                                            <div class="title"><span
                                                        class="sale-name">加价购</span>共<strong><?php echo count($output['goods']['jjg_info']['levels']); ?></strong>项，促销活动规则<a
                                                        href="javascript:;" nctype="hide-rule">关闭</a></div>
                                            <div class="cou-rule-list">
                                                <div class="couRuleScrollbar">
                                                    ...
                                                    foreach ((array)$output['goods']['jjg_info']['levels'] as $levelId => $v) { ?>
                                                        <div class="cou-rule">
                                                            <h4>规则<?php echo $levelId; ?>
                                                                ：消费满<strong><?php echo $lang['currency'] . $v['mincost']; ?></strong>
                                                                <?php if ($v['maxcou'] > 0) { ?>
                                                                    可换购最多
                                                                    <strong><?php echo $v['maxcou']; ?></strong>种优惠商品
                                                                <?php } else { ?>
                                                                    可换购任意多种优惠商品
                                                                <?php } ?>
                                                            </h4>
                                                            <ul>
                                                                <?php foreach ((array)$output['goods']['jjg_info']['levelSkus'][$levelId] as $sku => $vv) {
                                                                    ...
                                                                    <li title="<?php echo $g['name']; ?>"><a mxf="sqde"
                                                                                                             target="_blank"
                                                                                                             href="<?php echo urlShop('goods', 'index', array('goods_id' => $sku)); ?>">
                                                                            <img alt=""
                                                                                 src="<?php echo cthumb($g['goods_image'], 60); ?>"/>
                                                                            <h5><?php echo $g['name']; ?></h5>
                                                                            <h6 title="换购价">
                                                                                ￥<?php echo $vv['price']; ?></h6>
                                                                        </a></li>
                                                                <?php } ?>
                                                            </ul>
                                                        </div>
                                            ...
                                            <div class="bottom"><a
                                                        href="<?php echo urlShop('show_store', 'cou_goods', array('store_id' => $output['store_info']['store_id'], 'cou_id' => $output['goods']['jjg_info']['info']['id'])); ?>"
                                                        class="url" target="_blank">查看更多店铺“加价购”活动商品</a></div>
                                        </div>
                                    </div>
                                <?php } ?>
                                <!-- E 加价购 -->
            ...
            <div class="ncs-logistics"><!-- S 物流与运费新布局展示 ByCCYNet海吉壹佰 -->
                    ...
                    <dl id="ncs-freight" class="ncs-freight">
                        ...
                        <dd class="ncs-freight_box">
                            <div id="ncs-freight-selector" class="ncs-freight-select">
                                <div class="text">
                                    <div><?php if ($output['store_info']['deliver_region'] == '|') {
                                            echo '请选择地区';
                                        } else {
                                        ...
                                        <div class="mt">
                                            <ul class="tab">
                                                <li data-index="0" data-widget="tab-item" class="curr"><a href="#none"
                                                                                                          class="hover"><em><?php echo $output['store_info']['deliver_region_names'][0] ? $output['store_info']['deliver_region_names'][0] : '请选择' ?></em><i>
                                                            ∨</i></a></li>
                                            </ul>
                                        </div>
                                        ...
                                        <?php foreach ($output['goods']['spec_value'][$key] as $k => $v) { ?>
                                            <?php if ($key == 1) { ?>
                                                ...
                                                <li class="sp-img"><a href="javascript:void(0);"
                                                                ...
                                                                src="<?php echo $output['spec_image'][$k]; ?>"/><?php echo $v; ?>
                                                        <i></i></a></li>
                                            <?php } else { ?>
                                                <!-- 文字类型规格-->
                                                <li class="sp-txt"><a href="javascript:void(0)"
                                                                      ...
                                                                      data-param="{valid:<?php echo $k; ?>}"><?php echo $v; ?>
                                                        <i></i></a></li>
                                            <?php } ?>
                                        <?php } ?>
                                    </ul>
                ...
                <?php if ($output['goods']['goods_state'] != 0 && $output['goods']['goods_storage'] >= 0) { ?>
                    <div class="ncs-figure-input">
                        ...
                        <a href="javascript:void(0)" class="increase"
                           <?php if ($output['goods']['is_fcode'] != 1) { ?>nctype="increase"<?php } ?>>&nbsp;</a> <a
                                href="javascript:void(0)" class="decrease"
                                <?php if ($output['goods']['is_fcode'] != 1) { ?>nctype="decrease"<?php } ?>>&nbsp;</a>
                    </div>
                    <div class="ncs-point" style="display: none;"><i></i>
                        <!-- S 库存 -->
                        <span>您选择的商品库存<strong
                                    nctype="goods_stock"><?php echo $output['goods']['goods_storage']; ?></strong><?php echo $lang['nc_jian']; ?></span>
                        <!-- E 库存 -->
          ...
          <?php echo $output['goods']['upper_limit']; ?> </strong>件商品。</span>
                ...
                <!-- S 提示已选规格及库存不足无法购买 -->
                <?php if ($output['goods']['goods_state'] == 0 || $output['goods']['goods_storage'] <= 0) { ?>
                    <div nctype="goods_prompt" class="ncs-point"><i></i>
                        <span>您选择的商品<strong>库存不足</strong>；请选择店内其它商品或申请<a href="javascript:void(0);"
                                                                         nctype="arrival_notice" class="arrival"
                                                                         title="到货通知">到货通知</a>提示。</span></div>
                <?php } ?>
                <!-- E 提示已选规格及库存不足无法购买 -->
                <!-- S 购买按钮 -->
                <div class="ncs-btn">
                    ...
                    <div class="ncs-cart-popup">
                        <dl>
                            ...
                            <dd class="btns"><a href="javascript:void(0);" class="ncbtn-mini ncbtn-mint"
                                                onClick="location.href='<?php echo SHOP_SITE_URL . DS ?>index.php?controller=cart'"><?php echo $lang['goods_index_view_cart']; ?></a>
                ...
                <div class="ncs-saleout">
                    <dl>
                        ...
                        <dd><?php echo $lang['goods_index_is_no_show_message_two_1']; ?>&nbsp;<a
                                    ...
                                    class="ncbtn-mini"><?php echo $lang['goods_index_is_no_show_message_two_2']; ?></a>&nbsp;<?php echo $lang['goods_index_is_no_show_message_two_3']; ?>
                        </dd>
                    </dl>
                </div>
            </div>
                ...
                <dl>
                    ...
                    <dd>
                        <?php foreach ($output['goods']["contractlist"] as $gcitem_k => $gcitem_v) { ?>
                            <span
                                <?php if ($gcitem_v['cti_descurl']){ ?>onclick="window.open('<?php echo $gcitem_v['cti_descurl']; ?>');"<?php } ?>> <img
                                        src="<?php echo $gcitem_v['cti_icon_url_60']; ?>"/><?php echo $gcitem_v['cti_name']; ?> </span>
                        <?php } ?>
                    </dd>
                </dl>
            </div>
    ...
    <div class="ncs-handle">
        ...
        <a href="javascript:void(0);" class="compare" nc_type="compare_<?php echo $output['goods']['goods_id']; ?>"
           data-param='{"gid":"<?php echo $output['goods']['goods_id']; ?>"}'><i></i>加入对比</a>
        <!-- S 分销信息 -->
<!--        --><?php //if ($_SESSION['is_distribution']==1){?>
<!--        <a href="javascript:void(0);" class="distribute-info"-->
<!--           data-uri="--><?php //echo urlShop('goods','getDistributeInfo',['id'=>$output['goods']['goods_id']]) ?><!--"><i class="icon-share-alt"></i>立即推广</a>-->
<!--            --><?php //}else{?>
<!--                <a href="--><?php //echo urlMember('distribution','apply') ?><!--">申请分销</a>-->
<!--        --><?php //}?>
        <a href="javascript:void(0);" id="share">分 享</a>
        <!-- S 举报 -->
        <?php if ($output['inform_switch']) { ?>
            <a href="<?php if ($_SESSION['is_login']) { ?>index.php?controller=member_inform&action=inform_submit&goods_id=<?php echo $output['goods']['goods_id']; ?><?php } else { ?>javascript:login_dialog();<?php } ?>"
                ...
                <div class="top">
                    ...
                    <div class="btns"><span>您可对已购商品进行评价</span>
                        <p><a href="<?php if ($output['goods']['is_virtual']) {
                            ...
                            } ?>" class="ncbtn ncbtn-grapefruit" target="_blank"><i
            ...
            <div class="ncs-goods-info-content bd" id="ncGoodsTraded">
                <div class="top">
                    <div class="price"><?php echo $lang['goods_index_goods_price']; ?>
                        <strong><?php echo ncPriceFormat($output['goods']['goods_price']); ?></strong><?php echo $lang['goods_index_yuan']; ?>
        ...
        <?php if ($output['viewed_goods']) { ?>
            ...
            <div class="ncs-sidebar-container ncs-top-bar">
                ...
                <div class="content">
                        ...
                        <ol>
                            <?php foreach ((array)$output['viewed_goods'] as $g) { ?>
                                <li>
                                    <dl>
                                        <dt>
                                            <a href="<?php echo urlShop('goods', 'index', array('goods_id' => $g['goods_id'])); ?>"><?php echo $g['goods_name']; ?></a>
                                        </dt>
                                        <dd class="goods-pic"><a
                                                    href="<?php echo urlShop('goods', 'index', array('goods_id' => $g['goods_id'])); ?>"><span
                                                        class="thumb size60"><i></i><img
                                                            src="<?php echo cthumb($g['goods_image'], 60); ?>"
                                                            onload="javascript:DrawImage(this,60,60);"></span></a>
                                            <p><span class="thumb size100"><i></i><img
                                                            src="<?php echo cthumb($g['goods_image'], 240); ?>"
                                                            onload="javascript:DrawImage(this,100,100);"
                                                            title="<?php echo $g['goods_name']; ?>"><big></big><small></small></span>
                                            </p>
                                        </dd>
                    ...
                    <p><a href="<?php echo SHOP_SITE_URL; ?>/index.php?controller=member_goodsbrowse&action=list"
                          class="nch-sidebar-all-viewed">全部浏览历史</a></p></div>
            </div>
        <?php } ?>
    </div>
                                ...
                                <dl>
                                    ...
                                    <dd class="goods-pic"><a
                                                ...
                                                target="_blank" title="<?php echo $goods_commend['goods_jingle']; ?>"><img
                                                    src="<?php echo cthumb($goods_commend['goods_image'], 240); ?>"
...
<div class="con" style="display:none;   " id="share_con">
    <div  style="background-color:rgb(249,246,223);" >
...
<span style="display: none" id="getDistributeInfoBox">
    <link href="/shop/templates/default/css/eject_con.css" rel="stylesheet" type="text/css">
    <div class="eject_con">
        <div id="warning" class="alert alert-error"></div>
        <form method="post" target="_parent" action="<?php echo urlShop('goods','addDistributeGoodsToStore') ?>" id="apply_form">
            <input type="hidden" name="form_submit" value="ok"/>
            <input type="hidden" name="goods_id" value="<?php echo $output['goods']['goods_id']; ?>"/>
            <dl>
                <dt><i class="required">*</i>佣金比例：</dt>
                <dd>
                    <span class="text w150" ><?php echo $output['goods']['commission_rate']; ?>%</span>
                </dd>
            </dl>
            <dl>
                <dt>佣金：</dt>
                <dd>
                    <span class="text w150" ><?php echo $output['goods']['commission_amount']; ?> 元</span>
                </dd>
            </dl>
            <dl>
                <dt>分销类型：</dt>
                <dd>
                    <span class="text w150" ><?php echo $output['distribution_type'][$output['goods']['distribution_type']]; ?></span>
                </dd>
            </dl>

            <div class="bottom">
                <label class="submit-border"><input type="button" class="submit" value="加入微商城"/></label>
            </div>
        </form>
    </div>
</span>
...
<script type="text/javascript">
    $(function(){
        // 查看分销详情
//        $('.distribute-info').click(function () {
//            var id = 1;
//            var title = '分销详情';
//            var html = $('#getDistributeInfoBox').html();
//            var width = 480;
//            CUR_DIALOG = html_form(id, title, html, width, 0);
//            return false;
//        });

        // 添加到我的分销店铺
        $('body').on('click','.distribute-info',function(){
            ajaxpost('apply_form');
        });
    });
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $('a[nctype="addcart_submit"]').click(function () {
            ...
            addcart(<?php echo $output['goods']['goods_id'];?>, checkQuantity(),"<?php echo $output['goods']['referral_key'];?>", 'addcart_callback');
        });
        <?php if (!($output['goods']['is_virtual'] == 1 && $output['goods']['virtual_indate'] < TIMESTAMP)) {?>
        // 立即购买
        $('a[nctype="buynow_submit"]').click(function () {
            if (typeof(allow_buy) != 'undefined' && allow_buy === false) return;
            buynow(<?php echo $output['goods']['goods_id']?>, checkQuantity(),"<?php echo $output['goods']['referral_key'];?>");
        });
        <?php }?>
        <?php }?>
        // 到货通知
    ...
    // 立即购买js
    function buynow(goods_id, quantity,referral_key, chain_id, area_id, area_name, area_id_2) {
        <?php if ($_SESSION['is_login'] !== '1'){?>
        login_dialog();
        <?php }else{?>
        if (!quantity) {
        ...
        <?php if ($_SESSION['seller_session_info']['store_id'] == $output['goods']['store_id']) { ?>
        alert('不能购买自己店铺的商品');
        return;
        <?php } ?>
        $("#cart_id").val(goods_id + '|' + quantity+'|'+referral_key);
    ...
    $(function () {
        ...
        $('#transport_pannel>a').click(function () {
            ...
            var url = 'index.php?controller=goods&action=calc&rand=' + Math.random();
        ...
        <?php if ($output['goods']['is_virtual'] == 1) {?>
        $("#ncStoreMap").load('index.php?controller=show_map&action=index&w=420&h=400&store_id=<?php echo $output['goods']['store_id'];?>', function () {
            if ($(this).html() != '') {
                $(this).show();
            }
        });
        <?php } ?>
        $("#salelog_demo").load('index.php?controller=goods&action=salelog&goods_id=<?php echo $output['goods']['goods_id'];?>&store_id=<?php echo $output['goods']['store_id'];?>&vr=<?php echo $output['goods']['is_virtual'];?>', function () {
            // Membership card
            $(this).find('[nctype="mcard"]').membershipCard({type: 'shop'});
        });
        $("#consulting_demo").load('index.php?controller=goods&action=consulting&goods_id=<?php echo $output['goods']['goods_id'];?>&store_id=<?php echo $output['goods']['store_id'];?>', function () {
            // Membership card
            $(this).find('[nctype="mcard"]').membershipCard({type: 'shop'});
        });
...
//触及显示缩略图
        ...
        //记录浏览历史
        $.get("index.php?controller=goods&action=addbrowse", {gid:<?php echo $output['goods']['goods_id'];?>});
        //初始化对比按钮
        initCompare();

        $('[nctype="show_chain"]').click(function () {
            _goods_id = $(this).attr('data-goodsid');
            ajax_form('show_chain', '查看门店', 'index.php?controller=goods&action=show_chain&goods_id=' + _goods_id, 640);
        });
    ...
    $(document).ready(function () {
        $("#ncs-freight-selector").hover(function () {
            ...
            if (typeof nc_a === "undefined") {
                $.getJSON(SITEURL + "/index.php?controller=index&action=json_area&callback=?", function (data) {
                    nc_a = data;
        ...
        function _calc() {
            ...
            if (_args != '') {
                ...
                if (typeof calced_area[calc_area_id] == 'undefined') {
                    //需要请求配送区域设置
                    $.getJSON(SITEURL + "/index.php?controller=goods&action=calc&" + _args + "&myf=<?php echo $output['store_info']['store_free_price']?>&callback=?", function (data) {
