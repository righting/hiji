...
<div id="pictureFolder" class="ncsc-picture-folder">
  <dl class="ncsc-album-intro">
    <dt class="albume-name"><a href="index.php?controller=store_album&action=album_pic_list&id=<?php echo $output['class_info']['aclass_id']?>"><?php echo $output['class_info']['aclass_name']?></a></dt>
    <dd class="album-covers">
      <?php if($output['class_info']['aclass_cover'] != ''){ ?>
      <img src="<?php echo UPLOAD_SITE_URL.DS.ATTACH_GOODS.DS.$_SESSION['seller_session_info']['store_id'].DS.str_replace('_small', '_tiny', $output['class_info']['aclass_cover']);?>">
      <?php }else{?>
    ...
    <div class="ad-nav">
      <div class="ad-thumbs">
        <ul class="ad-thumb-list">
          ...
          <li><a href="<?php echo cthumb($v['goods_image'], 1280);?>" value="<?php echo $v['apic_id'] ?>"> <img title="<?php echo $v['apic_name']?>" src="<?php echo cthumb($v['goods_image'], 60);?>" class="image0"/>
...
<script>
  $(function() {
	...
	$('.ad-back').live('click',function(){
		...
		$('.ad-thumb-list').load('index.php?controller=store_album&action=album_ad_ajax&id=<?php echo $_GET['id'];?>&class_id=<?php echo $_GET['class_id'];?>&curpage='+curpage);
	});
...
function ajax_change_imgmessage(url){
	...
	$.getJSON("<?php echo SHOP_SITE_URL; ?>/index.php?controller=store_album&action=ajax_change_imgmessage", {'url':url}, function(data){
