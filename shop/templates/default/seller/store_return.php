...
<form method="get" action="index.php">
  <input type="hidden" name="controller" value="store_return" />
  ...
  <tbody>
    ...
    <tr class="bd-line" >
        ...
        <td class="w50"><div class="pic-thumb"><a href="<?php echo urlShop('goods','index',array('goods_id'=> $val['goods_id']));?>" target="_blank">
            <img src="<?php echo cthumb($val['goods_image'],60);?>" onMouseOver="toolTip('<img src=<?php echo cthumb($val['goods_image'],240); ?>>')" onMouseOut="toolTip()"/></a></div></td>
        ...
        <dd><?php echo $lang['refund_order_ordersn'].$lang['nc_colon'];?><a href="index.php?controller=store_order&action=show_order&order_id=<?php echo $val['order_id']; ?>" target="_blank"><?php echo $val['order_sn'];?></a></dd>
      ...
      <td class="nscs-table-handle">
        ...
        <span><a href="index.php?controller=store_return&action=edit&return_id=<?php echo $val['refund_id']; ?>" class="btn-bluejeans"><i class="icon-edit"></i><p>处理</p></a></span>
        <?php } else { ?>
       <span> <a href="index.php?controller=store_return&action=view&return_id=<?php echo $val['refund_id']; ?>" class="btn-bittersweet"><i class="icon-eye-open"></i><p><?php echo $lang['nc_view'];?></p></a></span>
        <?php } ?>
        <?php if ($val['seller_state'] == 2 && $val['return_type'] == 2 && $val['goods_state'] == 2) { ?>
        <span><a href="javascript:void(0)" class="btn-mint" nc_type="dialog" dialog_title="<?php echo $lang['nc_edit'];?>" dialog_id="return_order" dialog_width="480" uri="index.php?controller=store_return&action=receive&return_id=<?php echo $val['refund_id']; ?>"><i class="icon-check-sign"></i><p><?php echo '收货';?></p></a></span>
        <?php } ?>
        </td>
    </tr>
    <?php } ?>
