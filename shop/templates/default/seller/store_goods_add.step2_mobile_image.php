...
<div class="goods-gallery add-step2"><a class='sample_demo' id="select_submit" href="index.php?controller=store_album&action=pic_list&item=goods" style="display:none;"><?php echo $lang['nc_submit'];?></a>
  ...
  <ul class="list">
    ...
    <li onclick="<?php if ($output['type'] == 'replace') {?>replace<?php } else {?>insert<?php }?>_mobile_img('<?php echo cthumb($v['goods_image'], 1280);?>');"><a href="JavaScript:void(0);"><img src="<?php echo cthumb($v['goods_image'], 240);?>" title='<?php echo $v['apic_name']?>'/></a></li>
    <?php }?>
  </ul>
  <?php }else{?>
...
$(function(){
    ...
    $('#jumpMenu').change(function(){
        $('[nctype="mea_img"]').load('index.php?controller=store_album&action=pic_list&item=mobile<?php if ($output['type'] == 'replace') {?>&type=replace<?php }?>&id=' + $(this).val());
    });
});
</script>