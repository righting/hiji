...
<div class="ncc-receipt-info">
    ...
    <table class="ncc-table-style">
        ...
        <tr class="bundling-list">
            ...
            <td><a class="ncc-goods-thumb" href="%url%" target="_blank"> <img alt="%name%" data-src="%imgUrl%"/> </a>
            </td>
            <td class="tl">
        ...
        <tr>
            <th colspan="20"> <!-- S 店铺名称 -->
                ...
                <?php if (!empty($output['store_mansong_rule_list'][$store_id])) { ?>
                    <div class="ncc-store-sale ms">
                        <span>满即送</span><?php echo $output['store_mansong_rule_list'][$store_id]['desc']; ?> </div>
                <?php } ?>
                ...
                <?php if ($cart_info['bl_id'] == '0') { ?>
                    <td class="w100"><a
                                ...
                                target="_blank" class="ncc-goods-thumb"><img
                                    src="<?php echo cthumb($cart_info['goods_image']); ?>"
                    ...
                    <tr class="shop-list <?php echo $cart_info['state'] && $cart_info['storage_state'] ? '' : 'item_disabled'; ?>  bundling-list">
                        ...
                        <td>
                            <a href="<?php echo urlShop('goods', 'index', array('goods_id' => $goods_info['goods_id'])); ?>"
                               target="_blank" class="ncc-goods-thumb"><img
                                        src="<?php echo cthumb($goods_info['goods_image'], $store_id); ?>"
                        ...
                        <td class="td-border-right"><em goods_id="<?php echo $goods_info['goods_id']; ?>"
                                                        cart_id="<?php echo $cart_info['cart_id']; ?>"
                                                        nc_type="eachGoodsTotal<?php echo $store_id ?>"
                                                        class="goods-subtotal"><?php echo ncPriceFormat($goods_info['bl_goods_price'] * $cart_info['goods_num']); ?></em>
                            <span style="color: #F00;display:none">无货</span></td>
                    </tr>
                <?php } ?>
            <?php } ?>
                ...
                <div class="ncc-store-account">
                    ...
                    <?php if ($output['store_mansong_rule_list'][$store_id]['discount'] > 0) { ?>
                        <dl>
                            ...
                            <dd class="sum"><em id="eachStoreManSong_<?php echo $store_id; ?>"
                                                class="subtract">-<?php echo $output['store_mansong_rule_list'][$store_id]['discount']; ?></em>
                            </dd>
                        </dl>
                    <?php } ?>
        ...
        <tr>
            <td colspan="20"><?php if (!empty($output['ifcart'])) { ?>
                    <a href="index.php?controller=cart" class="ncc-prev-btn"><i
...
<input name="deduction_money" value="<?php echo array_sum($output['Hs']['points']); ?>" type="hidden"/>
<input name="htzc_goods" value="<?php echo implode(',', array_keys($output['Hs']['points'])) ?>" type="hidden"/>
<div id="dialog-confirm" title="海吉壹佰提示您：" style="display:none;">
    <div style="float: right;margin-top:-30px;margin-right:-10px;border:1px solid white;padding:0 5px;font-size:12px;border-radius:50%;"><a style="color:white;" title="关闭弹框" href="javascript:$('#dialog-confirm').dialog('close');">X</a></div>
    <p>您还未升到最高级别会员, 本订单可用升级积分【<?php echo array_sum($output['Hs']['points']) ?>】Hs,请选择是否用于升级？</p>
</div>
<style>
    .ui-dialog {
...
<script>
    function submitNext() {
        ...
        // 如果当前用户存在补单，提示用户是否将当前商品设置为补单商品
        <?php if(!empty($output['refund_return_list'])){ ?>
            // 如果选择了不补单
            var _is_bd = $('input[name="is_bd"]:checked').val();
            if(_is_bd == 1){
                $.post("<?php echo urlShop('buy', 'bu_dan_check') ?>", $('#order_form').serialize(), function (response) {
                    if (response['status'] != 1) {
                        showDialog(response['msg'], 'error', '', '', '', '', '', '', '', '', 4);
                    } else {
                        SUBMIT_FORM = false;
                        $('#order_form').submit();
                    }
        ...
        <?php }else{ ?>
        <?php if ( ($output['Hs']['is_bind'] == 1) ) {?>
            <?php if ( ($output['Hs']['points_sum'] > 0) && (array_sum($output['Hs']['points']) > 0)) {?>
                $("#dialog-confirm").dialog({
                    ...
                    buttons: {
                        ...
                        '不升级': function () {
                            $("input[name='deduction_money']").val(0);
                            $("input[name='htzc_goods']").val("");
                            $(this).dialog("close");
                            SUBMIT_FORM = false;
                            $('#order_form').submit();
                            return;
