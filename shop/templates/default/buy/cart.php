...
<link href="/shop/templates/default/css/style.css" rel="stylesheet" type="text/css">
...
<?php } ?> /></td>
          ...
          <td class="w100"><a href="<?php echo urlShop('goods','index',array('goods_id'=>$cart_info['goods_id']));?>" target="_blank" class="ncc-goods-thumb"><img src="<?php echo cthumb($cart_info['goods_image']);?>" alt="<?php echo $cart_info['goods_name']; ?>" /></a></td>
          <?php } ?>
...
$(function() {
    ...
    $('#pl_recommend').load('<?=urlShop('search','get_plrecmmend')?>',function(){
        $("#pl_recommend .hot-min").css('width','1200px');
        $("#pl_recommend li a").css('margin-left','100px');
        $("#pl_recommend .flex-control-nav").css('display','none');
        $(this).show();
    });
    // 领取代金券
