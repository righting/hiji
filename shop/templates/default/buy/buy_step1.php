...
<div class="ncc-main">
  <div class="ncc-title">
    ...
    <h5>请仔细核对填写收货等信息，以确保物流快递及时准确投递。</h5>
  </div>
    <?php if (!empty($output['refund_return_list'])) { ?>
        <?php include template('buy/return_refund/load');?>
    <?php } ?>
    ...
    <input value="buy" type="hidden" name="controller">
    <input value="buy_step2" type="hidden" name="action">
    <!-- 来源于购物车标志 -->
...
<!--    <input value="--><?php //echo $output['inv_info']['inv_id'];?><!--" name="invoice_id" id="invoice_id" type="hidden">-->
	...
	<!-- 商品推广邀请码 -->
	<input value="<?php echo $output['referral_key'];?>" name="referral_key" id="referral_key" type="hidden">
</div>
</form>
