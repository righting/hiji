...
<h3><?php echo $lang['talk_detail'];?></h3>
  ...
  <form action="index.php?controller=member_complain&action=apply_handle" method="post" id="handle_form">
...
function get_complain_talk() {
    ...
    $.ajax({
        type:'POST',
        url:'index.php?controller=member_complain&action=get_complain_talk',
        cache:false,
...
function publish_complain_talk() {
        $.ajax({
            type:'POST',
                url:'index.php?controller=member_complain&action=publish_complain_talk',
                cache:false,
