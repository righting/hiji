...
<div class="essential">
  ...
  <form method="get" action="index.php">
    <input type="hidden" name="controller" value="member_vr_refund" />
  ...
  <table class="ncm-default-table order">
    ...
    <tbody>
      ...
      <tr class="bd-line" >
        <td class="bdl"></td>
        <td class="w50"><div class="pic-thumb"><a href="<?php echo urlShop('goods','index',array('goods_id'=> $val['goods_id']));?>" target="_blank"><img src="<?php echo cthumb($val['goods_image'],60);?>"  onMouseOver="toolTip('<img src=<?php echo cthumb($val['goods_image'],240);?>>')" onMouseOut="toolTip()"/></a></div></td>
        <td class="tl"><dl class="goods-name">
            <dt><a href="<?php echo urlShop('goods','index',array('goods_id'=> $val['goods_id']));?>" target="_blank"><?php echo $val['goods_name']; ?></a></dt>
            <dd><?php echo $lang['refund_order_ordersn'].$lang['nc_colon'];?><a href="index.php?controller=member_vr_order&action=show_order&order_id=<?php echo $val['order_id']; ?>" target="_blank"><?php echo $val['order_sn'];?></a></dd>
          </dl></td>
        <td><?php echo $val['refund_amount'];?></td>
        <td><?php echo $output['admin_array'][$val['admin_state']]; ?></td>
        <td class="bdr"><a href="index.php?controller=member_vr_refund&action=view&refund_id=<?php echo $val['refund_id']; ?>" class="ncbtn"><?php echo $lang['nc_view'];?> </a></td>
      </tr>
      <?php } ?>
    </tbody>
    <tfoot>
