...
<div class="ncm-flow-layout">
  <div class="ncm-flow-container">
    ...
    <form id="evalform" method="post" action="index.php?controller=member_evaluate&action=<?php echo $_GET['action'];?>&order_id=<?php echo $_GET['order_id'];?>">
      ...
      <table class="ncm-default-table mb30">
        <tbody>
          ...
          <tr class="bd-line">
            <td class="w20"></td>
            <td class="pic-mode w200"><div class="pic-thumb"><a href="index.php?controller=goods&goods_id=<?php echo $goods['goods_id']; ?>" target="_blank"><img src="<?php echo $goods['goods_image_url']; ?>"/></a></div>
              <dl class="goods-name">
                <dt><a href="index.php?controller=goods&goods_id=<?php echo $goods['goods_id'];?>" target="_blank" title="<?php echo $goods['goods_name'];?>"><?php echo $goods['goods_name'];?></a></dt>
                <dd title="<?php echo $goods['goods_spec'];?>"><?php echo $goods['goods_spec'];?></dd>
              </dl></td>
            <td valign="top" class="tl"><div class="ncgeval">商品评分：
                <div class="raty vm">
                  <input nctype="score" name="goods[<?php if ($_GET['action'] != 'add_vr') { echo $goods['rec_id'];} else { echo $goods['goods_id'];}?>][score]" type="hidden">
                </div>
              </div>
              <textarea name="goods[<?php if ($_GET['action'] != 'add_vr') { echo $goods['rec_id'];} else { echo $goods['goods_id'];}?>][comment]" cols="150" class="w450 mt10 mb10 h40" placeholder="请输入要评价的内容，不要超过150个字符。"></textarea>
              <div class="show-pic">
                <?php if ($_GET['action'] != 'add_vr') {?>
                ...
                <input type="checkbox" class="checkbox vm" name="goods[<?php if ($_GET['action'] != 'add_vr') { echo $goods['rec_id'];} else { echo $goods['goods_id'];}?>][anony]">
      ...
      <?php if (!$output['store_info']['is_own_shop'] && $_GET['action'] != 'add_vr') { ?>
      <div class="essential-nav">
        <ul class="tab">
...
$(function(){
    ...
    <?php if(!empty($output['order_goods'])){?>
    <?php foreach($output['order_goods'] as $goods){?>
    <?php $param_id = $_GET['action'] != 'add_vr' ? $goods['rec_id'] : $goods['goods_id'];?>
    // 图片上传
