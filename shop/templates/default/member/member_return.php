...
<div class="essential">
  ...
  <form method="get" action="index.php">
    <table class="ncm-search-table">
      <input type="hidden" name="controller" value="member_return" />
  ...
  <table class="ncm-default-table order">
    ...
    <tbody>
      ...
      <tr>
        <td class="bdl"></td>
        <td class="w50"><div class="pic-thumb"><a href="<?php echo urlShop('goods','index',array('goods_id'=> $val['goods_id']));?>" target="_blank"><img src="<?php echo cthumb($val['goods_image'],60);?>" onMouseOver="toolTip('<img src=<?php echo cthumb($val['goods_image'],60);?>>')" onMouseOut="toolTip()"/></a></div></td>
        <td class="tl"><dl class="goods-name">
            <dt><a href="<?php echo urlShop('goods','index',array('goods_id'=> $val['goods_id']));?>" target="_blank"><?php echo $val['goods_name']; ?></a></dt>
            <dd><?php echo $lang['refund_order_ordersn'].$lang['nc_colon'];?><a href="index.php?controller=member_order&action=show_order&order_id=<?php echo $val['order_id']; ?>" target="_blank"><?php echo $val['order_sn'];?></a></dd>
          </dl></td>
        ...
        <td class="bdr"><?php if($val['seller_state'] == 2 && $val['return_type'] == 2 && $val['goods_state'] == 1) { ?>
          <p><a href="index.php?controller=member_return&action=ship&return_id=<?php echo $val['refund_id']; ?>" class="ncbtn">退货</a></p>
          <?php } else { ?>
          <a href="index.php?controller=member_return&action=view&return_id=<?php echo $val['refund_id']; ?>" class="ncbtn">查看</a>
          <?php } ?>
          ...
          <p><a href="javascript:void(0)" class="ncbtn ncbtn-bittersweet" nc_type="dialog" dialog_title="延迟时间" dialog_id="return_delay" dialog_width="480"
        	    uri="index.php?controller=member_return&action=delay&return_id=<?php echo $val['refund_id']; ?>"> 延时 </a></p>
          <?php } ?></td>
      </tr>
      <?php } ?>
      <?php } else { ?>
