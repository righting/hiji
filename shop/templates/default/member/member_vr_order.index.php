...
<div class="essential">
  ...
  <form method="get" action="index.php" target="_self">
    <table class="ncm-search-table">
      <input type="hidden" name="controller" value="member_vr_order" />
      <tr>
        <td>&nbsp;</td>
  ...
  <table class="ncm-default-table order">
    ...
    <tbody>
      ...
      <tr>
        <td class="bdl"></td>
        <td class="w70"><div class="ncm-goods-thumb"><a href="<?php echo urlShop('goods','index',array('goods_id'=>$order_info['goods_id']));?>" target="_blank" onMouseOver="toolTip('<img src=<?php echo cthumb($order_info['goods_image'], 240);?>>')" onMouseOut="toolTip()"/><img src="<?php echo cthumb($order_info['goods_image'], 60);?>"/></a></div></td>
        ...
        <td> <?php if($order_info['if_refund']){ ?>
          <a href="index.php?controller=member_vr_refund&action=add_refund&order_id=<?php echo $order_info['order_id']; ?>">退款</a>
          <?php } ?></td>
        ...
        <td class="bdl"><p><?php echo $order_info['state_desc'];?></p>
          <p><a href="index.php?controller=member_vr_order&action=show_order&order_id=<?php echo $order_info['order_id'];?>">订单详情</a></p></td>
        <td class="bdl bdr">

		  <?php if($order_info['if_pay']){ ?>
          <p><a class="ncbtn ncbtn-bittersweet" href="index.php?controller=buy_virtual&action=pay&order_id=<?php echo $order_info['order_id']; ?>"><i class="icon-shield"></i>订单支付</a></p>
          <?php } ?>
          ...
          <p><a class="ncbtn ncbtn-aqua" href="index.php?controller=member_evaluate&action=add_vr&order_id=<?php echo $order_info['order_id']; ?>"><i class="icon-thumbs-up-alt"></i><?php echo $lang['member_order_want_evaluate'];?></a></p>
          <?php } ?>
