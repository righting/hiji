...
function update_data(id) {//更新
    var get_text = $.ajax({
        type: "POST",
        url: 'index.php?controller=web_config&action=code_update',
...
function update_dialog(id) {//初始化数据
    switch (id) {
        ...
        case "recommend_list":
            ...
            break;
        case "brand_list":
            $("#show_brand_list").load('index.php?controller=web_config&action=' + id);//查询数据
...
function get_goods_class() {//查询子分类
    ...
    if (gc_id > 0) {
        $.get('index.php?controller=web_config&action=category_list&id=' + gc_id, function (data) {
            $(data).each(function () {
                var obj = $(this);
...
//个人微商推荐商品
function ws_get_recommend_goods() {//查询商品
    var gc_id = 0;
    $('#recommend_gcategory > select').each(function () {
        if ($(this).val() > 0) gc_id = $(this).val();
    });
    var goods_name = $.trim($('#recommend_goods_name').val());
    if (gc_id > 0 || goods_name != '') {
        $("#show_recommend_goods_list").load('index.php?controller=distribution&action=recommend_list&' + $.param({
            'id': gc_id,
            'goods_name': goods_name
        }));
    }
...
function select_recommend_goods(goods_id) {//商品选择
    ...
    text_append += '<a href="' + SHOP_SITE_URL + '/index.php?controller=goods&goods_id=' + goods_id + '" target="_blank">';
    ...
    text_append += '<input name="recommend_list[' + id + '][goods_list][' + goods_id + '][market_price]" value="' + market_price + '" type="hidden">';
    text_append += '<input name="recommend_list[' + id + '][goods_list][' + goods_id + '][goods_salenum]" value="' + goods_salenum + '" type="hidden">';
...
function get_recommend_brand() {//查询品牌
    ...
    if (brand_name != '' || brand_initial != '') {
        $("#show_brand_list").load('index.php?controller=web_config&action=brand_list&' + $.param({
...
function update_brand() {//更新品牌
    ...
    if (get_text == '1') {
        ...
        $("img[select_brand_id]").each(function () {
            ...
            $(".ccy-btbrand ul.brands").append('<li><span>' + text_append + '</span></li>');
        });
...
//促销区左侧广告图片更新
function update_sales_pic(id, pic) {//更新图片
    if (id == 'tit') {
        var tit_floor = $.trim($('#tit_floor').val());
        var tit_title = $.trim($('#tit_title').val());
        var get_type = $("#upload_tit_form input:checked").val();
    ...
    var obj = $("#picture_" + id);
    obj.html('<img style="width:150px;height:250px" src="' +  pic + '" />');
    recommend_show=id;
    update_sale();
    DialogManager.close("upload_adv");
}
//中部推荐图片上传
function show_recommend_pic_dialog(id) {//弹出框
    ...
    var _size = obj.find("div[select_recommend_pic_id='" + id + "']").size();
    ...
    if (obj.find("div[select_recommend_pic_id='" + id + "']").size() == 0) {//不存在时添加
        ...
        for (var _pic_id = 2; _pic_id <= 11; _pic_id++) {
            text_append += '<a href="javascript:void(0);" recommend_pic_id="' + _pic_id + '" class="left-a">160*160</a>';
        }
...
function get_goods_list() {//查询商品
    ...
    if (gc_id > 0 || goods_name != '') {
        $("#show_sale_goods_list").load('index.php?controller=web_config&action=goods_list&' + $.param({
            'id': gc_id,
...
function select_sale_goods(goods_id) {//商品选择
    ...
    text_append += '<span class="thumb size-72x72">';
    text_append += '<i></i>';
    text_append += '<img select_goods_id="' + goods_id + '" title="' + goods_name + '" goods_name="' + goods_name + '" src="' + goods_pic + '" goods_price="' + goods_price + '" market_price="' + market_price + '" goods_salenum="' + goods_salenum + '" onload="javascript:DrawImage(this,72,72);" />';
    text_append += '</span></div>';
    text_append += '<div class="goods-name">';
    text_append += '<a href="' + SITEURL + '/index.php?controller=goods&goods_id=' + goods_id + '" target="_blank">';
    text_append += goods_name + '</a>';
    text_append += '</div>';
...
function update_sale() {//更新
    ...
    var obj = $("div[select_sale_id='" + id + "']");
    var text_append = '';
    var recommend_name = obj.find("dd input[name='sale_list["+id+"][recommend][name]']").val();
	var recommend_pic = $("#picture_"+id).children("img").attr("src"); 
	var recommend_url = obj.find("dd input[name='sale_list["+id+"][recommend][url]']").val();
	var recommend_title = obj.find("dd input[name='sale_list["+id+"][recommend][title]']").val();
	var recommend_subhead1 = obj.find("dd input[name='sale_list["+id+"][recommend][subhead1]']").val();
	var recommend_subhead2 = obj.find("dd input[name='sale_list["+id+"][recommend][subhead2]']").val();
    $("dl[sale_id='" + id + "'] dt h4").html(recommend_name);
    $("dl[sale_id='" + id + "'] dt input").val(recommend_name);
	$("#sale_list_form input[name='sale_list["+id+"][recommend][pic]']").val(recommend_pic);
	$("#sale_list_form input[name='sale_list["+id+"][recommend][url]']").val(recommend_url);
	$("#sale_list_form input[name='sale_list["+id+"][recommend][title]']").val(recommend_title);
	$("#sale_list_form input[name='sale_list["+id+"][recommend][subhead1]']").val(recommend_subhead1);
	$("#sale_list_form input[name='sale_list["+id+"][recommend][subhead2]']").val(recommend_subhead2);
    obj.find("img").each(function () {
        var goods = $(this);
        ...
        text_append += '<input name="sale_list[' + id + '][goods_list][' + goods_id + '][goods_pic]" value="' + goods_pic + '" type="hidden">';
