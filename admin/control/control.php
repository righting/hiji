...
class SystemControl{
    ...
    protected function __construct() {
        ...
        if (($_GET['branch']!='' || $_GET['action']=='ajax') && strtoupper(CHARSET) == 'GBK'){
    ...
    protected final function systemLogin() {
        ...
        if (!key_exists('gid',(array)$user) || !isset($user['sp']) || (empty($user['name']) || empty($user['id']))){
            @header('Location: '. ADMIN_SITE_URL .'/index.php?controller=login&action=login');exit;
        }else {
    ...
    protected final function checkPermission($link_nav = null) {
        ...
        $act = $_GET['controller'] ? $_GET['controller'] : $_POST['controller'];
        $act = $act == '' ? 'index' : $act;
    ...
    protected final function sublink($links = array(), $actived = '', $file='index.php') {
        ...
        foreach ($links as $k=>$v) {
            ...
            if (empty($array['action'])) $array['action'] = 'index';
            if (!$this->checkPermission($array)) continue;
            $href = ($array['action'] == $actived ? null : "href=\"{$file}?{$v['url']}\"");
            $class = ($array['action'] == $actived ? "class=\"current\"" : null);
     ...
     * @return string
     */
    protected final function newSubLink($links = array(), $actived = '') {
        $linkstr = '';
        foreach ($links as $k=>$v) {
            parse_str($v['url'],$array);
            if (empty($array['action'])) $array['action'] = 'index';
            if (!$this->checkPermission($array)) continue;
            $href = ($array['bonus_type'] == $actived ? null : "href=\"{$v['url']}\"");
            $class = ($array['bonus_type'] == $actived ? "class=\"current\"" : null);
            $lang = $v['text'] ? $v['text'] : L($v['lang']);
            $linkstr .= sprintf('<li><a %s %s><span>%s</span></a></li>',$href,$class,$lang);
        }
        return "<ul class=\"tab-base nc-row\">{$linkstr}</ul>";
    ...
    protected final function log($lang = '', $state = 1, $admin_name = '', $admin_id = 0) {
        ...
        $data['ip']         = getIp();
        $data['url']        = $_REQUEST['controller'].'&'.$_REQUEST['action'];
