...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="index.php?controller=member&action=member" title="返回列表"><i class="fa fa-arrow-circle-o-left"></i></a>
      <div class="subject">
  ...
  <form id="user_form" enctype="multipart/form-data" method="post">
    ...
    <dl class="row">
        <dt class="tit">
            <label><?php echo $lang['member_index_number']?></label>
        </dt>
        <dd class="opt">
            <?php echo substr($output['member_array']['member_number'],0,7);?>
            <input type="hidden" name="member_number_const" value="<?php echo substr($output['member_array']['member_number'],0,7);?>">
            <input type="text"  name="member_number"  class="input-txt" value="<?php echo substr($output['member_array']['member_number'],-9);?>"  />
            <p class="notic">ID编号总16位,允许修改后9位,已修改[<?php echo $output['member_array']['member_number_edit_num']; ?>]次</p>
        </dd>
    </dl>
      <dl class="row">
        <dt class="tit">
...
$(function(){
	...
	function ajaxFileUpload()
	{
		...
		(
			{
				url : '<?php echo ADMIN_SITE_URL?>/index.php?controller=common&action=pic_upload&form_submit=ok&uploadpath=<?php echo ATTACH_AVATAR;?>',
				secureuri:false,
				fileElementId:'_pic',
				dataType: 'json',
				...
				{
					if (data.status == 1){
						ajax_form('cutpic','<?php echo $lang['nc_cut'];?>','<?php echo ADMIN_SITE_URL?>/index.php?controller=common&action=pic_cut&type=member&x=120&y=120&resize=1&ratio=1&filename=<?php echo UPLOAD_SITE_URL.'/'.ATTACH_AVATAR;?>/avatar_<?php echo $_GET['member_id'];?>.jpg&url='+data.url,690);
					}else{
						alert(data.msg);
					}
    ...
    $('#user_form').validate({
        ...
        rules : {
            ...
            member_email   : {
                ...
                email : true,
				remote   : {
                    url :'index.php?controller=member&action=ajax&branch=check_email',
                    type:'get',
                    data:{
                        user_name : function(){
