...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?controller=promotion_combo&action=get_combo_goods_xml',
        colModel: [
...
function fg_del(id) {
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('index.php?controller=promotion_combo&action=del_combo_goods', {id:id}, function(data){
            if (data.state) {
