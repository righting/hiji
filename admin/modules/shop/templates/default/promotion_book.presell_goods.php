...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?controller=promotion_book&action=get_presell_goods_xml',
        colModel: [
...
function fg_del(id) {
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('index.php?controller=promotion_book&action=del_presell_goods', {id:id}, function(data){
            if (data.state) {
