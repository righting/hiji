...
function update_flex(){
    $('.ncap-stat-general-single').load("<?php echo urlAdminShop('stat_bonus_for_months',$output['action'],['get_type'=>'xml','bonus_type'=>$output['this_type']])?>&" + $("#formSearch").serialize(),
        function () {
            $('.timer').each(count);
        });

    $("#flexigrid").flexigrid({
        url: "<?php echo urlAdminShop('stat_bonus_for_months',$output['action'],['get_type'=>'xml','bonus_type'=>$output['this_type']])?>&"+$("#formSearch").serialize(),
        colModel : [
            ...
            {display: '分红金额', name : 'total_money',  width : 80, sortable : true, align: 'center'},
...
function fg_operation(name, bDiv){
    var stat_url = "<?php echo urlAdminShop('stat_bonus_for_months',$output['action'],['bonus_type'=>$output['this_type']])?>";
