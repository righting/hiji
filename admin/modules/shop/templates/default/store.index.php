...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?controller=store&action=get_xml',
        colModel : [
        ...
        buttons : [
			...
			{display: '<i class="fa fa-plus"></i>新增数据', name : 'ccynet_add', bclass : 'add', title : '添加一条新数据到列表', onpress : fg_operations }					
        ],
        searchitems : [
    ...
    $('#ncsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?controller=store&action=get_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });

    // 高级搜索重置
    $('#ncreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?controller=store&action=get_xml'}).flexReload();
        $("#formSearch")[0].reset();
    });
});
...
function fg_operations(name, bDiv) {
    if (name == 'ccynet_add') {
        window.location.href = 'index.php?controller=store&action=ccynet_add';
    }
}
