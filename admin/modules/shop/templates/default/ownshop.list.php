...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?controller=ownshop&action=get_xml',
        colModel : [
...
function fg_operation(name, bDiv) {
    if($('.sign').length>=2){
        ...
        if (name == 'add') {
            window.location.href = 'index.php?controller=ownshop&action=add';
        }
    }
}
...
function fg_delete(id) {
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('index.php?controller=ownshop&action=del', {id:id}, function(data){
            if (data.state) {
