...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?controller=snstrace&action=get_xml',
        colModel : [
...
function fg_operation(name, bDiv) {
    if (name == 'add') {
        window.location.href = 'index.php?controller=snstrace&action=commentlist';
    }
}
function fg_del(id) {
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('index.php?controller=snstrace&action=tracedel', {id:id}, function(data){
            if (data.state) {
