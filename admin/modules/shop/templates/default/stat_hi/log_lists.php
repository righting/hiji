...
function update_flex(){
    $('.ncap-stat-general-single').load("<?php echo urlAdminShop('stat_hi','index',['get_type'=>'xml'])?>&" + $("#formSearch").serialize(),
        function () {
            $('.timer').each(count);
        });

    $("#flexigrid").flexigrid({
        url: "<?php echo urlAdminShop('stat_hi','hi_log',['get_type'=>'xml','user_id'=>$output['user_id']])?>&"+$("#formSearch").serialize(),
        colModel : [
            ...
            {display: '获取时间', name : 'recommend_team_hi',  width : 300, sortable : true, align: 'center'},
            ],
        buttons : [
