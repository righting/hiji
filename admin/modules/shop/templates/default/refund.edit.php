...
<div class="page">
  ...
  <form id="post_form" method="post" action="index.php?controller=refund&action=edit&refund_id=<?php echo $output['refund']['refund_id']; ?>">
    <input type="hidden" name="form_submit" value="ok" />
    <input type="hidden" name="delete_refund" id='delete_refund' value="0" />
    <div class="ncap-form-default">
      ...
      <dl class="row">
        ...
        <dl class="row">
            <dt class="tit">退款模式</dt>
            <dd class="opt"><?php echo $output['refund_return_type']; ?> </dd>
        </dl>
        <dl class="row">
            <dt class="tit">退款状态</dt>
            <dd class="opt"><?php echo $output['refund_return_info']['state_cn']; ?> </dd>
        </dl>
      <dl class="row">
      ...
      <dl class="row">
        ...
        <dd class="opt"><?php echo ncPriceFormat($output['order']['pay_amount']);?></dd>
          ...
          <dl class="row">
            <dt class="tit"></dt>
            <dd class="opt">
                <a href="<?php echo ADMIN_SITE_URL;?>/index.php?controller=refund&action=alipay&refund_id=<?php echo $output['refund']['refund_id']; ?>" target="_blank">支付宝退款</a>  
      ...
      <?php } ?>
        ...
        <?php if($output['refund_return_info']['is_show_admin_msg'] == 1){ ?>
        <div class="title">
            <h3>平台退款审核</h3>
        </div>
      <dl class="row">
    ...
    <?php } ?>
   <?php if($output['refund_return_info']['is_show_button'] == 1){ ?>
      <div class="bot">
          <a href="JavaScript:void(0);" class="ncap-btn-big ncap-btn-green" id="submitBtn">
              <?php echo $output['refund_return_info']['button_name'];?>
          </a>

          <a href="JavaScript:void(0);" class="ncap-btn-big ncap-btn-red" id="refundSubmitBtn">
              拒绝退货退款
          </a>
      </div>
    <?php } ?>
...
$(function(){
	...
	$("#wxpayBtn").click(function(){
	    var ajaxurl = '<?php echo ADMIN_SITE_URL;?>/index.php?controller=refund&action=wxpay&refund_id=<?php echo $output['refund']['refund_id']; ?>';
		show_msg(ajaxurl);
	});
	$("#alipayBtn").click(function(){
	    var ajaxurl = '<?php echo ADMIN_SITE_URL;?>/index.php?controller=refund&action=get_detail&refund_id=<?php echo $output['refund']['refund_id']; ?>';
		show_msg(ajaxurl);
	});
    $('#post_form').validate({
