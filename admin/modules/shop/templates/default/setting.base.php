  ...
  <form method="post" enctype="multipart/form-data" name="form1">
    ...
    <div class="ncap-form-default">
      ...
      <dl class="row">
        ...
        <dl class="row">
            <dt class="tit">
                <label for="site_email">专项商品类别</label>
            </dt>
            <dd class="opt">
                <select id="annual_fee_gc_id"  name="annual_fee_gc_id" >
                    <option value="">请选择分类</option>
                    <?php foreach($output['gc_data'] as $v) :?>
                    <option <?php if ($v['gc_id']==$output['list_setting']['annual_fee_gc_id'])  echo 'selected'?> value="<?php echo $v['gc_id'] ?>"><?php echo $v['gc_name'] ?></option>
                    <?php endforeach; ?>
                </select>
                <span class="err"></span>
                <p class="notic">设置微分销会员抵扣年费专项消费商品类别</p>
            </dd>
        </dl>
        <dl class="row">
            <dt class="tit">
                <label for="site_email">会员实名自动审核</label>
            </dt>
            <dd class="opt">
                <div class="onoff">
                    <label for="guest_comment_enable" class="cb-enable <?php if($output['list_setting']['member_auth_auto'] == '1'){ ?>selected<?php } ?>" title="<?php echo $lang['nc_yes'];?>"><?php echo $lang['nc_yes'];?></label>
                    <label for="guest_comment_disabled" class="cb-disable <?php if($output['list_setting']['member_auth_auto'] == '0'){ ?>selected<?php } ?>" title="<?php echo $lang['nc_no'];?>"><?php echo $lang['nc_no'];?></label>
                    <input id="guest_comment_enable" name="member_auth_auto" <?php if($output['list_setting']['member_auth_auto'] == '1'){ ?>checked="checked"<?php } ?> value="1" type="radio">
                    <input id="guest_comment_disabled" name="member_auth_auto" <?php if($output['list_setting']['member_auth_auto'] == '0'){ ?>checked="checked"<?php } ?> value="0" type="radio">
                </div>
                <p class="notic">开启自动审核后，会员上传实名信息后则自动审核为通过</p>
            </dd>
        </dl>
