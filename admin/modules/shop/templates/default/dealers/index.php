...
<script>
    function update_flex(){
        $('.ncap-stat-general-single').load("<?php echo urlAdminShop('dealers','index',['get_type'=>'xml'])?>&" + $("#formSearch").serialize(),
            function () {
                $('.timer').each(count);
            });

        $("#flexigrid").flexigrid({
            url: "<?php echo urlAdminShop('dealers','index',['get_type'=>'xml'])?>&"+$("#formSearch").serialize(),
            colModel : [
                ...
                {display: '联系人姓名', name : 'name', width : 200, sortable : false, align: 'center'},
                {display: '职称', name : 'title',  width : 120, sortable : true, align: 'center'},
                {display: '手机号', name : 'mobile',  width : 120, sortable : true, align: 'center'},
                {display: '加盟地区', name : 'address',  width : 120, sortable : true, align: 'center'},
                {display: '状态', name : 'status_cn',  width : 300,  sortable : true, align: 'center'},
...
// 高级搜索统计周期选择
    ...
    function fg_operation(name, bDiv){
        var stat_url = "<?php echo urlAdminShop('dealers','index')?>";
