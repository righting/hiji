...
$(function(){
    ...
    $('#ncsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?controller=order&action=get_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });
    // 高级搜索重置
    $('#ncreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?controller=order&action=get_xml'}).flexReload();
        $("#formSearch")[0].reset();
    });
    $("#flexigrid").flexigrid({
        url: 'index.php?controller=order&action=get_xml',
        colModel : [
...
function fg_csv(ids) {
    id = ids.join(',');
    //return console.log($("#flexigrid").flexSimpleSearchQueryString()+'&action=export_step1&order_id=' + id);
    window.location.href = $("#flexigrid").flexSimpleSearchQueryString()+'&action=export_step1&order_id=' + id;
}
function fg_cancel(id) {
	...
	$.ajax({
        type: "GET",
        dataType: "json",
        url: "index.php?controller=order&action=change_state&state_type=cancel",
        data: "order_id="+id,
