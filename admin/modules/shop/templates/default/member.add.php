    ...
    <div class="ncap-form-default">
      ...
      <dl class="row">
        ...
        <dl class="row">
            ...
            <dd class="opt">
                <label>
                    <input type="radio" value="1" name="is_dealers">
                    是</label>
                <label>
                    <input type="radio" checked="checked" value="2" name="is_dealers">
                    否 </label>
                <span class="err"></span></dd>
        </dl>
...
<input type="hidden" id="checksubmit" value="1"/>
		...
		(
			{
				...
				{
					if (data.status == 1){
						ajax_form('cutpic','<?php echo $lang['nc_cut'];?>','<?php echo ADMIN_SITE_URL?>/index.php?controller=common&action=pic_cut&type=member&x=120&y=120&resize=1&ratio=1&url='+data.url,690);
					}else{
						alert(data.msg);
					}
    ...
    $('#user_form').validate({
        ...
        rules : {
			member_name: {
				...
				maxlength: 20,
				remote   : {
                    url :'index.php?controller=member&action=ajax&branch=check_user_name',
                    type:'get',
                    data:{
                        user_name : function(){
