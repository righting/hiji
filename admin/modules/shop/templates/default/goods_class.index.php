...
<div class="page">
  ...
  <?php if($output['class_id']==0){?>
        <table class="flex-table" style="margin-top: 65px;">
          <thead>
            <tr>
          ...
          <tbody>
            ...
            <tr data-id="<?php echo $v['gc_id'];?>">
              ...
              <td class="handle">
                ...
                <ul>
                  <li><a href="index.php?controller=goods_class&action=goods_class_edit&gc_id=<?php echo $v['gc_id'];?>">编辑分类信息</a></li>
                  <li><a href="index.php?controller=goods_class&action=goods_class_add&gc_parent_id=<?php echo $v['gc_id'];?>">新增下级分类</a></li>
                  <li><a href="index.php?controller=goods_class&action=goods_class&gc_id=<?php echo $v['gc_id'];?>">查看下级分类</a></li>
                  <li><a href="index.php?controller=goods_class&action=nav_edit&gc_id=<?php echo $v['gc_id'];?>">编辑分类导航</a></li>
                </ul>
                </span></td>
...
function fg_operation(name, bDiv) {
    if (name == 'add') {
        window.location.href = 'index.php?controller=goods_class&action=goods_class_add';
    } else if (name == 'del') {
        ...
        window.location.href = 'index.php?controller=goods_class&action=goods_class_export';
    }
}
function fg_del(ids) {
    ...
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('index.php?controller=goods_class&action=goods_class_del', {id:id}, function(data){
            if (data.state) {
                location.reload();
            } else {
                showError(data.msg)
