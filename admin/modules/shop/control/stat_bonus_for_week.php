...
class stat_bonus_for_weekControl extends SystemControl
{

    private function getTopLinks()
    {
        $links = [
            ['url' => urlAdminShop('stat_bonus_for_week', 'index', ['bonus_type' => 11]), 'text' => '管理普惠周奖'], // 管理普惠周奖
            ['url' => urlAdminShop('stat_bonus_for_week', 'index', ['bonus_type' => 5]), 'text' => '中层管理周分红'], // 中层管理周分红
        ];
        return $links;
    }
...
//        }
    ...
    public function indexOp()
    {
        ...
        $field        = 'user_id,sum(money) as total_money';
        ...
        $begin_day   = $user_bonus_logic->getCurrentWeeklyBeginDate();    // 本周开始时间
        $end_day     = $user_bonus_logic->getCurrentWeeklyEndDate();        // 本周结束时间
        ...
        $type     = $user_bonus_log_model::TYPE_SALES_MANAGEMENT_WEEK_BONUS;
        ...
        $where['type']       = $type;
        $page                = 15;
        $lists               = $user_bonus_log_model->where($where)->field($field)->page($page)->group('user_id')->order('created_at desc')->select();
