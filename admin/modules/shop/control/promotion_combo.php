...
class promotion_comboControl extends SystemControl{
    private $links = array(
        array('url'=>'controller=promotion_combo&action=index','text'=>'商品列表'),
        array('url'=>'controller=promotion_combo&action=combo_quota_list','text'=>'套餐列表'),
        array('url'=>'controller=promotion_combo&action=combo_setting','text'=>'设置')
    );
    public function __construct(){
        ...
        if (intval($_GET['promotion_allow']) !== 1 && intval(C('promotion_allow')) !== 1){
            $url = array(
                ...
                array(
                    'url'=>'index.php?controller=promotion_combo&promotion_allow=1',
                    'msg'=>L('open'),
                )
            );
