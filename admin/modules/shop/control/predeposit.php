...
class predepositControl extends SystemControl
{
    ...
    public function recharge_editOp()
    {
        $id = intval($_GET['id']);
        if ($id <= 0) {
            showMessage(Language::get('admin_predeposit_parameter_error'), 'index.php?controller=predeposit&action=predeposit', '', 'error');
        }
        //查询充值信息
        ...
        if (empty($info)) {
            showMessage(Language::get('admin_predeposit_record_error'), 'index.php?controller=predeposit&action=predeposit', '', 'error');
        }
        if (!chksubmit()) {
            //显示支付接口列表
        ...
        try {
            ...
            $this->log($log_msg, 1);
            //记录消费日志
            QueueClient::push('addConsume', ['member_id'      => $info['pdr_member_id'], 'member_name' => $info['pdr_member_name'],
                                             'consume_amount' => $info['pdr_amount'], 'consume_time' => TIMESTAMP, 'consume_remark' => '管理员更改充值单[' . $info['pdr_sn'] . ']状态,充值成功']);
            showMessage(Language::get('admin_predeposit_recharge_edit_success'), 'index.php?controller=predeposit&action=predeposit');
        } catch (Exception $e) {
            $model_pd->rollback();
            $this->log($log_msg, 0);
            showMessage($e->getMessage(), 'index.php?controller=predeposit&action=predeposit', 'html', 'error');
        }
    }
    ...
    public function pd_cash_payOp()
    {
        $id = intval($_GET['id']);
        if ($id <= 0) {
            showMessage(Language::get('admin_predeposit_parameter_error'), 'index.php?controller=predeposit&action=pd_cash_list', '', 'error');
        }
        $model_pd                       = Model('predeposit');
        $condition                      = [];
        $condition['pdc_id']            = $id;
        ...
        if (!is_array($info) || count($info) < 0) {
            showMessage(Language::get('admin_predeposit_record_error'), 'index.php?controller=predeposit&action=pd_cash_list', '', 'error');
        }
        ...
        try {
            ...
            $model_pd->commit();
            $this->log($log_msg, 1);
            showMessage(Language::get('admin_predeposit_cash_edit_success'), 'index.php?controller=predeposit&action=pd_cash_list');
        } catch (Exception $e) {
            $model_pd->rollback();
            $this->log($log_msg, 0);
            showMessage($e->getMessage(), 'index.php?controller=predeposit&action=pd_cash_list', 'html', 'error');
        }
    }
    ...
    public function export_step1Op()
    {
        ...
        $order = '';
        $param = ['pdr_id', 'pdr_sn', 'pdr_member_id', 'pdr_member_name', 'pdr_amount', 'pdr_add_time', 'pdr_payment_name', 'pdr_trade_sn', 'pdr_payment_state', 'pdr_payment_time', 'pdr_admin'];
        ...
        if (!is_numeric($_GET['curpage'])) {
            ...
            if ($count > self::EXPORT_SIZE) {   //显示下载链接
                ...
                Tpl::output('murl', 'index.php?controller=predeposit&action=predeposit');
                Tpl::setDirquna('shop');/*创 驰 云 网 络 ccynet.com*/
    ...
    public function export_cash_step1Op()
    {
        ...
        if (!is_numeric($_GET['curpage'])) {
            ...
            if ($count > self::EXPORT_SIZE) {   //显示下载链接
                ...
                Tpl::output('list', $array);
                Tpl::output('murl', 'index.php?controller=predeposit&action=pd_cash_list');
                Tpl::setDirquna('shop');/*创 驰 云 网 络 ccynet.com*/
                Tpl::showpage('export.excel');
            } else {  //如果数量小，直接下载
    ...
    public function export_mx_step1Op()
    {
        ...
        $order = '';
        $param = ['lg_id', 'lg_member_id', 'lg_member_name', 'lg_av_amount', 'lg_freeze_amount', 'lg_add_time', 'lg_desc', 'lg_admin_name'];
        if (in_array($_GET['sortname'], $param) && in_array($_GET['sortorder'], ['asc', 'desc'])) {
            $order = $_GET['sortname'] . ' ' . $_GET['sortorder'];
        }
        $model_pd = Model('predeposit');
        if (!is_numeric($_GET['curpage'])) {
            ...
            if ($count > self::EXPORT_SIZE) {   //显示下载链接
                ...
                Tpl::output('list', $array);
                Tpl::output('murl', 'index.php?controller=predeposit&action=pd_log_list');
                Tpl::setDirquna('shop');/*创 驰 云 网 络 ccynet.com*/
                Tpl::showpage('export.excel');
            } else {  //如果数量小，直接下载
    ...
    public function get_xmlOp()
    {
        ...
        foreach ($recharge_list as $value) {
            ...
            $operation                      .= "<a class='btn green' href='javascript:void(0)' onclick=\"ajax_form('recharge_info','查看充值编号“" . $value['pdr_sn'] . "”的明细','index.php?controller=predeposit&action=recharge_info&id=" . $value['pdr_id'] . "', '640')\"><i class='fa fa-list-alt'></i>查看</a>";
    ...
    public function get_cash_xmlOp()
    {
        ...
        $order = '';
        $param = ['pdc_id', 'pdc_sn', 'pdc_member_id', 'pdc_member_name', 'pdc_amount', 'pdc_add_time', 'pdc_bank_name', 'pdc_bank_no'
        ...
        foreach ($cash_list as $value) {
            ...
            $param['operation']             .= "<a class='btn green' href='javascript:void(0)' onclick=\"ajax_form('cash_info','查看提现编号“" . $value['pdc_sn'] . "”的明细', 'index.php?controller=predeposit&action=pd_cash_view&id=" . $value['pdc_id'] . "', 640)\" ><i class='fa fa-list-alt'></i>查看</a>";
