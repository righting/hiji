...
class settingControl extends SystemControl{
    private $links = array(
        array('url'=>'controller=setting&action=base','lang'=>'web_set'),
        array('url'=>'controller=setting&action=dump','lang'=>'dis_dump')
    );
    ...
    public function baseOp(){
        ...
        if (chksubmit()){
            ...
            if (!empty($_POST['annual_fee_gc_id'])){
                $update_array['annual_fee_gc_id'] = $_POST['annual_fee_gc_id'];
            }
             $update_array['member_auth_auto'] = $_POST['member_auth_auto'];
        ...
        //获取商品分类
        $gc_data = Model('goods_class')->getGoodsClassListAll();
        Tpl::output('gc_data',$gc_data);
        //输出子菜单
