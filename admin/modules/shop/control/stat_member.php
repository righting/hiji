...
{
    private $links = [
        ['url' => 'controller=stat_member&action=newmember', 'lang' => 'stat_newmember'],
        ['url' => 'controller=stat_member&action=analyze', 'lang' => 'stat_memberanalyze'],
        //array('url'=>'controller=stat_member&action=scale','lang'=>'stat_scaleanalyze'),
        //array('url'=>'controller=stat_member&action=area','lang'=>'stat_areaanalyze'),
        ['url' => 'controller=stat_member&action=buying', 'lang' => 'stat_buying'],
    ];
    ...
    public function __construct()
    {
        ...
        //处理搜索时间
        if (!isset($this->search_arr['action']) || in_array($_REQUEST['action'], ['newmember', 'analyze', 'scale', 'area', 'buying', 'get_scale_xml'])) {
    ...
    public function newmemberOp()
    {
        ...
        if ($this->search_arr['search_type'] == 'day') {
            ...
            Tpl::output('actionurl', 'index.php?controller=stat_member&action=newmember&search_type=day&search_time=' . date('Y-m-d', $this->search_arr['day']['search_time']));
        }
        ...
        if ($this->search_arr['search_type'] == 'week') {
            ...
            Tpl::output('actionurl', 'index.php?controller=stat_member&action=newmember&search_type=week&searchweek_year=' . $this->search_arr['week']['current_year'] . '&searchweek_month=' . $this->search_arr['week']['current_month'] . '&searchweek_week=' . $this->search_arr['week']['current_week']);
        }
        ...
        if ($this->search_arr['search_type'] == 'month') {
            ...
            Tpl::output('actionurl', 'index.php?controller=stat_member&action=newmember&search_type=month&searchmonth_year=' . $this->search_arr['month']['current_year'] . '&searchmonth_month=' . $this->search_arr['month']['current_month']);
        }
    ...
    public function analyzeOp()
    {
        ...
        for ($i = 0; $i < 15; $i++) {
            $stat_ordernum_arr['series'][0]['data'][] = ['name' => strval($ordernum_listtop15[$i]['statm_membername']), 'y' => intval($ordernum_listtop15[$i]['ordernum'])];
        }
        ...
        for ($i = 0; $i < 15; $i++) {
            $stat_goodsnum_arr['series'][0]['data'][] = ['name' => strval($goodsnum_listtop15[$i]['statm_membername']), 'y' => intval($goodsnum_listtop15[$i]['goodsnum'])];
        }
        ...
        for ($i = 0; $i < 15; $i++) {
            $stat_orderamount_arr['series'][0]['data'][] = ['name' => strval($orderamount_listtop15[$i]['statm_membername']), 'y' => floatval($orderamount_listtop15[$i]['orderamount'])];
        }
    ...
    public function get_analyzeinfo_xmlOp()
    {
        ...
        $list     = $model->statByStatmember($where, $field, [$page, $countnum], 0, "{$stat_field} desc,statm_memberid desc", 'statm_memberid');
        $curpage  = ($t = intval($_REQUEST['curpage'])) ? $t : 1;
        $statlist = [];
    ...
    public function showmemberOp()
    {
        ...
        $this->links[] = ['url' => 'controller=stat_member&action=showmember', 'lang' => 'stat_memberlist'];


        // 统计每个等级的用户数量
        $member_model                 = Model('member');
        $old_level_statistics         = $member_model->where(['member_id' => ['in', array_column($member_list, 'member_id')]])->field('level_id,count(level_id) as level_count')->group('level_id')->select();
    ...
    public function get_member_xmlOp()
    {
        ...
        $statlist     = [];
        if (!empty($list) && is_array($list)) {
            $fields_array                 = ['member_name', 'member_truename', 'member_email', 'member_time', 'member_login_num', 'member_login_time', 'member_login_ip',
                'level_name', 'member_points', 'available_predeposit', 'freeze_predeposit'];
            $user_id_arr                  = array_column($list, 'member_id');
            $user_id_info_arr             = $member_model->where(['member_id' => ['in', $user_id_arr]])->field('member_id,member_number,level_id,member_name,member_nickname')->select();
            ...
            foreach ($list as $k => $v) {
                ...
                $out_array['level_name']           = $level_type_name_for_level_id[$user_id_info_arr_for_id[$v['member_id']]['level_id']];
    ...
    public function get_scale_xmlOp()
    {
        ...
        $field = ' statm_memberid, min(statm_membername) as statm_membername, min(statm_time) as statm_time, SUM(statm_orderamount) as orderamount, SUM(statm_predincrease) as predincrease, -SUM(statm_predreduce) as predreduce, SUM(statm_pointsincrease) as pointsincrease, -SUM(statm_pointsreduce) as pointsreduce ';
        //排序
        $order_type = ['statm_membername', 'orderamount', 'predincrease', 'predreduce', 'pointsincrease', 'pointsreduce'];
        $sort_type  = ['asc', 'desc'];
