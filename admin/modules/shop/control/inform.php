...
{
    private $links = [
        ['url' => 'controller=inform&action=inform_list', 'text' => '未处理'],
        ['url' => 'controller=inform&action=inform_handled_list', 'text' => '已处理'],
        ['url' => 'controller=inform&action=inform_subject_type_list', 'text' => '类型设置'],
        ['url' => 'controller=inform&action=inform_subject_list', 'text' => '主题设置']
    ];
    ...
    public function __construct()
    {
        ...
        if ($_GET['action'] == 'index') $_GET['action'] = 'inform_list';
    ...
    public function get_xmlOp()
    {
        ...
        if ($_POST['query'] && in_array($_POST['qtype'], ['inform_goods_name', 'inform_member_name', 'inform_store_name', 'inform_type', 'inform_subject'])) {
            $condition[$_POST['qtype']] = $_POST['query'];
        }
        if (in_array($_POST['sortname'], ['inform_datetime', 'inform_member_id', 'inform_goods_id', 'inform_store_id']) && in_array($_POST['sortorder'], ['asc', 'desc'])) {
        ...
        if ($inform_list && is_array($inform_list)) {
            ...
            foreach ($inform_list as $k => $v) {
                ...
                if ($_GET['type'] == 'waiting') {
                    $list['operation'] = "<a class=\"btn orange\" href=\"index.php?controller=inform&action=show_handle_page&inform_id={$v['inform_id']}&inform_goods_name={$v['inform_goods_name']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
                } else {
                    $list['operation'] = '--';
                }
                ...
                if ($_GET['type'] == '') {
                    $list['inform_handle_type']    = str_replace([1, 2, 3], ['无效举报', '恶意举报', '有效举报'], $v['inform_handle_type']);
    ...
    public function inform_subject_type_saveOp()
    {
        ...
        $obj_validate->validateparam = [
            ["input" => $input['inform_type_name'], "require" => "true", "validator" => "Length", "min" => "1", "max" => "50", "message" => Language::get('inform_type_null')],
            ["input" => $input['inform_type_desc'], "require" => "true", "validator" => "Length", "min" => "1", "max" => "100", "message" => Language::get('inform_type_desc_null')],
        ];
        ...
        if ($error != '') {
            ...
            showMessage(Language::get('nc_common_save_succ'), 'index.php?controller=inform&action=inform_subject_type_list');
        }
    }
    ...
    public function inform_subject_type_dropOp()
    {
        ...
        if (empty($inform_type_id)) {
            showMessage(Language::get('param_error'), 'index.php?controller=inform');
        }
        ...
        showMessage(Language::get('nc_common_del_succ'), 'index.php?controller=inform&action=inform_subject_type_list');
    ...
    public function inform_subject_saveOp()
    {
        ...
        $obj_validate                = new Validate();
        $obj_validate->validateparam = [
            ["input" => $input['inform_subject_type_name'], "require" => "true", "validator" => "Length", "min" => "1", "max" => "50", "message" => Language::get('inform_subject_null')],
            ["input" => $input['inform_subject_content'], "require" => "true", "validator" => "Length", "min" => "1", "max" => "50", "message" => Language::get('inform_content_null')],
            ["input" => $input['inform_subject_type_id'], "require" => "true", "validator" => "Number", "message" => Language::get('param_error')],
        ];
        $error                       = $obj_validate->validate();
        ...
        if ($error != '') {
            ...
            showMessage(Language::get('nc_common_save_succ'), 'index.php?controller=inform&action=inform_subject_list');
        }
    }
    ...
    public function inform_subject_dropOp()
    {
        ...
        showMessage(Language::get('nc_common_del_succ'), 'index.php?controller=inform&action=inform_subject_list');
    }
    ...
    public function inform_handleOp()
    {
        ...
        $obj_validate->validateparam = [
            ...
            ["input" => $inform_handle_message, "require" => "true", "validator" => "Length", "min" => "1", "max" => "100", "message" => Language::get('inform_handle_message_null')],
        ];
        $error                       = $obj_validate->validate();
        if ($error != '') {
            showMessage($error);
        ...
        if ($model_inform->updateInform($update_array, $where_array)) {
            ...
            showMessage(Language::get('nc_common_op_succ'), 'index.php?controller=inform&action=inform_list');
        } else {
