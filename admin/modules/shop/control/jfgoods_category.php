...
class jfgoods_categoryControl extends SystemControl{
    ...
    public function get_xmlOp(){
        ...
        if (is_array($bannerList)){
            foreach ($bannerList as $k => $v){
                $list = array();
                $list['operation'] = "<a class='btn red' onclick=\"fg_delete({$v['id']})\"><i class='fa fa-trash-o'></i>删除</a><a class='btn blue' href='index.php?controller=jfgoods_category&action=jfgoods_category_edit&id={$v['id']}'><i class='fa fa-pencil-square-o'></i>编辑</a>";
                $list['id'] = $v['id'];
     ...
     * 添加分组
     */
    public function jfgoods_category_addOp(){
        $lang   = Language::getLangContent();
        $model = Model('jfgoods_category');
        /**
         * 保存
        ...
        if (chksubmit()){
            ...
            if ($result){
                $url = array(array('url'=>'index.php?controller=jfgoods_category&action=index','msg'=>'跳转至分类列表'));
        ...
        Tpl::setDirquna('shop');
        Tpl::showpage('jfgoods_category_add');
    }

    /**
     * 编辑分组
     */
    public function jfgoods_category_editOp(){
        $lang   = Language::getLangContent();
        $model = Model('jfgoods_category');
        /**
         * 保存
        ...
        if (chksubmit()){
            ...
            $result = Db::update('jfgoods_category',$updates_array,'id='.intval($_POST['id']));
            if ($result){
                $url = array(array('url'=>'index.php?controller=jfgoods_category&action=index','msg'=>'跳转至分类列表'));
        ...
        Tpl::output('info',$info);
        Tpl::setDirquna('shop');
        Tpl::showpage('jfgoods_category_edit');
    }
    ...
    public function deleteOp() {
        if (preg_match('/^[\d]+$/', $_GET['del_id'])) {
            ...
            $result = Db::delete('jfgoods_category','id='.$id);
            if($result){
