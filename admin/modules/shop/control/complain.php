...
class complainControl extends SystemControl
{
    ...
    private $links = [
        ['url' => 'controller=complain&action=complain_new_list', 'text' => '新投诉'],
        ['url' => 'controller=complain&action=complain_appeal_list', 'text' => '待申诉'],
        ['url' => 'controller=complain&action=complain_talk_list', 'text' => '对话中'],
        ['url' => 'controller=complain&action=complain_handle_list', 'text' => '待仲裁'],
        ['url' => 'controller=complain&action=complain_finish_list', 'text' => '已关闭'],
        ['url' => 'controller=complain&action=complain_subject_list', 'text' => '主题设置'],
        ['url' => 'controller=complain&action=complain_setting', 'text' => '时效设置']
    ];
    ...
    public function __construct()
    {
        ...
        if ($_GET['action'] == 'index') $_GET['action'] = 'complain_new_list';
    ...
    */
    public function get_new_xmlOp()
    {
        ...
        if (empty($complain_list)) $complain_list = [];
        $data              = [];
        ...
        foreach ($complain_list as $k => $complain_info) {
            $list = [];
            if ($_GET['state'] == 99) {
                $list['operation'] = "<a class=\"btn orange\" href=\"index.php?controller=complain&action=complain_progress&complain_id={$complain_info['complain_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            } else {
                $list['operation'] = "<a class=\"btn orange\" href=\"index.php?controller=complain&action=complain_progress&complain_id={$complain_info['complain_id']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
            }
    ...
    public function complain_verifyOp()
    {
        ...
        if (intval($complain_info['complain_state']) === self::STATE_NEW) {
            ...
            if ($model_complain->updateComplain($update_array, $where_array)) {
                ...
                showMessage(Language::get('complain_verify_success'), 'index.php?controller=complain&action=complain_new_list');
            } else {
                showMessage(Language::get('complain_verify_fail'), 'index.php?controller=complain&action=complain_new_list');
            }
        } else {
    ...
    public function complain_closeOp()
    {
        ...
        $obj_validate->validateparam = [
            ["input" => $final_handle_message, "require" => "true", "validator" => "Length", "min" => "1", "max" => "255", "message" => Language::get('final_handle_message_error')],
        ];
    ...
    public function complain_subject_saveOp()
    {
        ...
        $obj_validate                = new Validate();
        $obj_validate->validateparam = [
            ["input" => $input['complain_subject_content'], "require" => "true", "validator" => "Length", "min" => "1", "max" => "50", "message" => Language::get('complain_subject_content_error')],
            ["input" => $input['complain_subject_desc'], "require" => "true", "validator" => "Length", "min" => "1", "max" => "100", "message" => Language::get('complain_subject_desc_error')],
        ];
        $error                       = $obj_validate->validate();
        if ($error != '') {
            showMessage($error);
            ...
            showMessage(Language::get('complain_subject_add_success'), 'index.php?controller=complain&action=complain_subject_list');
        }
    }
    ...
    public function complain_subject_dropOp()
    {
        ...
        if (empty($complain_subject_id)) {
            showMessage(Language::get('param_error'), 'index.php?controller=complain');
        }
        $model_complain_subject                 = Model('complain_subject');
        $update_array                           = [];
        ...
        if ($model_complain_subject->updateComplainSubject($update_array, $where_array)) {
            ...
            showMessage(Language::get('complain_subject_delete_success'), 'index.php?controller=complain&action=complain_subject_list');
        } else {
            showMessage(Language::get('complain_subject_delete_fail'), 'index.php?controller=complain&action=complain_subject_list');
        }
    }
    ...
    private function get_complain_state_link($complain_state)
    {
        switch (intval($complain_state)) {
            case self::STATE_NEW:
                return 'index.php?controller=complain&action=complain_new_list';
                break;
            case self::STATE_APPEAL:
                return 'index.php?controller=complain&action=complain_appeal_list';
                break;
            case self::STATE_TALK:
                return 'index.php?controller=complain&action=complain_talk_list';
                break;
            case self::STATE_HANDLE:
                return 'index.php?controller=complain&action=complain_handle_list';
                break;
            case self::STATE_FINISH:
                return 'index.php?controller=complain&action=complain_finish_list';
                break;
            default:
                showMessage(Language::get('param_error'));
        }
    ...
    {
        $menu_array = [
            'complain_new_list'     => ['menu_type' => 'link', 'menu_name' => Language::get('complain_new_list'), 'menu_url' => 'index.php?controller=complain&action=complain_new_list'],
            'complain_appeal_list'  => ['menu_type' => 'link', 'menu_name' => Language::get('complain_appeal_list'), 'menu_url' => 'index.php?controller=complain&action=complain_appeal_list'],
            'complain_talk_list'    => ['menu_type' => 'link', 'menu_name' => Language::get('complain_talk_list'), 'menu_url' => 'index.php?controller=complain&action=complain_talk_list'],
            'complain_handle_list'  => ['menu_type' => 'link', 'menu_name' => Language::get('complain_handle_list'), 'menu_url' => 'index.php?controller=complain&action=complain_handle_list'],
            'complain_finish_list'  => ['menu_type' => 'link', 'menu_name' => Language::get('complain_finish_list'), 'menu_url' => 'index.php?controller=complain&action=complain_finish_list'],
            'complain_subject_list' => ['menu_type' => 'link', 'menu_name' => Language::get('complain_subject_list'), 'menu_url' => 'index.php?controller=complain&action=complain_subject_list'],
            'complain_subject_add'  => ['menu_type' => 'link', 'menu_name' => Language::get('complain_subject_add'), 'menu_url' => 'index.php?controller=complain&action=complain_subject_add'],
            'complain_setting'      => ['menu_type' => 'link', 'menu_name' => Language::get('complain_setting'), 'menu_url' => 'index.php?controller=complain&action=complain_setting'],
            'complain_progress'     => ['menu_type' => 'link', 'menu_name' => Language::get('complain_progress'), 'menu_url' => '###'],
        ];
