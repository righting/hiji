...
class returnControl extends SystemControl{
    ...
    private $links = array(
            array('url'=>'controller=return&action=return_manage','text'=>'待处理'),
            array('url'=>'controller=return&action=return_all','text'=>'所有记录')
    );
    public function __construct(){
        parent::__construct();
        $model_refund = Model('refund_return');
        $model_refund->getRefundStateArray();
        if ($_GET['action'] == 'index') $_GET['action'] = 'return_manage';
    ...
    public function get_manage_xmlOp() {
        ...
        foreach ($return_list as $k => $return_info) {
            ...
            $list['operation'] = "<a class=\"btn orange\" href=\"index.php?controller=return&action=edit&return_id={$return_info['refund_id']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
            ...
            if ($return_info['goods_image'] != '') {
                $list['goods_image'] = "<a href='".cthumb($return_info['goods_image'],360)."' target='_blank' class='pic-thumb-tip' onMouseOut='toolTip()' onMouseOver='toolTip(\"<img src=".cthumb($return_info['goods_image'],240).">\")'><i class='fa fa-picture-o'></i></a> ";
            } else {
                $list['goods_image'] = '';
            }
    ...
    public function get_all_xmlOp() {
        ...
        foreach ($return_list as $k => $return_info) {
            ...
            $list['operation'] .= "<a class=\"btn green\" href=\"index.php?controller=return&action=view&return_id={$return_info['refund_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            $list['refund_sn'] = $return_info['refund_sn'];
            $list['refund_amount'] = ncPriceFormat($return_info['refund_amount']);
            if(!empty($return_info['pic_info'])) {
                $info = unserialize($return_info['pic_info']);
    ...
    public function editOp() {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                showMessage(Language::get('nc_common_save_succ'),'index.php?controller=return&action=return_manage');
            } else {
                showMessage(Language::get('nc_common_save_fail'));
            }
        }
    ...
    private function createCsv($return_list) {
        ...
        foreach ($return_list as $k => $return_info) {
            ...
            $list['seller_times'] = !empty($return_info['seller_time']) ? date('Y-m-d H:i:s',$return_info['seller_time']) : '';
            if ($return_info['goods_image'] != '') {
                $list['goods_image'] = cthumb($return_info['goods_image'],360);
            } else {
                $list['goods_image'] = '';
            }
            $list['goods_id'] = !empty($return_info['goods_id']) ? $return_info['goods_id'] : '';
