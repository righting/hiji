...
class storeControl extends SystemControl{
    ...
    private $_links = array(
        array('url'=>'controller=store&action=store','text'=>'管理'),
        array('url'=>'controller=store&action=store_joinin','text'=>'开店申请'),
        array('url'=>'controller=store&action=reopen_list','text'=>'续签申请'),
        array('url'=>'controller=store&action=store_bind_class_applay_list','text'=>'经营类目申请'),
        array('url'=>'controller=store&action=bill_cycle','text'=>'结算周期设置')
    );
    ...
    public function get_xmlOp() {
        ...
        foreach ($store_list as $value) {
            ...
            $operation = "<a class='btn green' href='index.php?controller=store&action=store_joinin_detail&member_id=".$value['member_id']."'><i class='fa fa-list-alt'></i>查看</a><span class='btn'><em><i class='fa fa-cog'></i>" . L('nc_set') . " <i class='arrow'></i></em><ul><li><a href='index.php?controller=store&action=store_edit&store_id=" . $value['store_id'] . "'>编辑店铺信息</a></li><li><a href='index.php?controller=store&action=store_bind_class&store_id=" . $value['store_id'] . "'>修改经营类目</a></li>";
    ...
    public function get_bill_cycle_xmlOp() {
        ...
        foreach ($store_list as $value) {
            $param = array();
            $store_state = $this->getStoreState($value);
            $operation = "<a class='btn blue' href='index.php?controller=store&action=bill_cycyle_edit&store_id=".$value['store_id']."'><i class='fa fa-pencil-square-o'></i>编辑</a>";
            $operation .= "</ul></span>";
            $param['operation'] = $operation;
            $param['store_id'] = $value['store_id'];
            $store_name = "<a class='" . $store_state . "' href='". urlShop('show_store', 'index', array('store_id' => $value['store_id'])) ."' target='blank'>";
    ...
    public function export_csvOp() {
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('murl','index.php?controller=store&action=index');
				Tpl::setDirquna('shop');/*创 驰 云 网 络 ccynet.com*/
    ...
    public function store_editOp(){
        ...
        if (chksubmit()){
            ...
            if (strlen(trim($_POST['seller_password'])>=6) ){
                $pw = hj_password_hash($_POST['seller_password']);
                if (trim($_POST['member_id'])){
                    $res = Model('member')->editMember(['member_id'=>$_POST['member_id']],['member_passwd'=>$pw]);
                }
            }
            ...
            if ($result){
                $url = array(
                array(
                'url'=>'index.php?controller=store&action=store',
                'msg'=>$lang['back_store_list'],
                ),
                array(
                'url'=>'index.php?controller=store&action=store_edit&store_id='.intval($_POST['store_id']),
    ...
    public function bill_cycyle_editOp(){
        ...
        if (chksubmit()){
            ...
            if ($result){
                ...
                showMessage($lang['nc_common_save_succ'],'index.php?controller=store&action=bill_cycle');
            }else {
                $this->log('设置店铺结算周期['.$_POST['store_name'].']',1);
                showMessage($lang['nc_common_save_fail'],'index.php?controller=store&action=bill_cycle');
            }
        }
    ...
    public function edit_save_joininOp() {
        if (chksubmit()) {
            ...
            if ($result) {
                showMessage(L('nc_common_op_succ'), 'index.php?controller=store&action=store');
            } else {
    ...
    public function ccynet_addOp()
    {
        if (chksubmit())
        {
            ...
            $saveArray['passwd'] = hj_password_hash(trim($_POST['member_passwd']));
            ...
            showMessage('操作成功','index.php?controller=store&action=store');
            return;
        }
		Tpl::setDirquna('shop');/*创 驰 云 网 络 ccynet.com*/
    ...
    public function get_joinin_xmlOp() {
        ...
        foreach ($store_list as $value) {
            ...
            if(in_array(intval($value['joinin_state']), array(STORE_JOIN_STATE_NEW, STORE_JOIN_STATE_PAY))) {
                $operation = "<a class='btn orange' href='". urlAdminShop('store','store_joinin_detail',['bind_mobile'=>$value['bind_mobile']]) ."'><i class='fa fa-check-circle'></i>审核</a>";
            } else {
                $operation = "<a class='btn green' href='". urlAdminShop('store','store_joinin_detail',['bind_mobile'=>$value['bind_mobile']]) ."'><i class='fa fa-list-alt'></i>查看</a>";
            }
            $param['operation'] = $operation;
            $param['invite_id'] = $value['invite_id'];
            $param['bind_mobile'] = $value['bind_mobile'];
...
//            $param['company_registered_capital'] = $value['company_registered_capital'];
            $param['member_id'] = isset($value['member_id']) ? $value['member_id'] : '';
            $param['member_name'] = isset($value['member_name']) ? $value['member_name'] : '';
            $data['list'][] = $param;
        }
        echo Tpl::flexigridXML($data);exit();
    }
    ...
    public function get_bind_class_applay_xmlOp() {
        ...
        $order = '';
        $param = array('bid', 'store_id', 'commis_rate', 'class_1', 'class_2', 'class_3','class_4', 'state');
        if (in_array($_POST['sortname'], $param) && in_array($_POST['sortorder'], array('asc', 'desc'))) {
            $order = $_POST['sortname'] . ' ' . $_POST['sortorder'];
        }
        ...
        foreach ($store_bind_class_list as $value) {
            $param = array();
            if($value['state'] == '0') {
                $operation = "<a class='btn orange' href=\"javascript:if(confirm('确认审核吗？'))window.location = 'index.php?controller=store&action=store_bind_class_applay_check&bid=".$value['bid']."&store_id=".$value['store_id']."'\"><i class=\"fa fa-check-circle\"></i>审核</a>";
            } else {
                $operation = "<a class='btn red' href=\"javascript:if(confirm('".($value['state'] == '1' ? '该类目已经审核通过，删除它可能影响到商家的使用，' : null)."确认删除吗？'))window.location = 'index.php?controller=store&action=store_bind_class_applay_del&bid=".$value['bid']."&store_id=".$value['store_id']."'\"><i class=\"fa fa-trash-o\"></i>删除</a>";
            }
            $param['operation'] = $operation;
            $param['store_id'] = $value['store_id'];
            ...
            if ($value['class_4'] > 0) {
                $param['class'] .= '   > ' . $goods_class[$value['class_4']]['gc_name']. '(ID:' . $value['class_4'] . ')';
            }
    ...
    public function store_joinin_detailOp(){
        $model_store_joinin = Model('store_joinin');
        $joinin_detail = $model_store_joinin->getOne(array('bind_mobile'=>$_GET['bind_mobile']));
    ...
    public function store_joinin_verifyOp() {
        $model_store_joinin = Model('store_joinin');
        $joinin_detail = $model_store_joinin->getOne(array('bind_mobile'=>$_POST['bind_mobile']));
    ...
    private function store_joinin_verify_pass($joinin_detail) {
        ...
        $model_store_joinin = Model('store_joinin');
        $model_store_joinin->modify($param, array('bind_mobile'=>$_POST['bind_mobile']));
    ...
    private function store_joinin_verify_open($joinin_detail) {
        // @TODO 开通店铺的时候需要在member表中生成一个供应商的账户，但是这个账号在前台是不能登录的
        $model_store_joinin = Model('store_joinin');
        $model_store    = Model('store');
        ...
        if($_POST['verify_type'] === 'pass') {
            // @TODO 在member表中生成一个供应商账号
            $member_data['member_number'] = $member_model->createMemberId();
            $member_data['member_type'] = $member_model::MEMBER_TYPE_SUPPLIER;
            $member_data['member_name'] = $joinin_detail['bind_mobile'];
            $member_data['member_passwd'] = $joinin_detail['login_password'];
            $member_data['member_email'] = $joinin_detail['contacts_email'];
            $member_data['member_time'] = TIMESTAMP;
            $member_data['member_login_time'] = TIMESTAMP;
            $member_data['member_old_login_time'] = TIMESTAMP;
            $member_data['member_login_ip'] = getIp();
            $member_data['member_old_login_ip'] = $member_data['member_login_ip'];
            $member_data['member_truename'] = $joinin_detail['company_name'];
            $member_data['member_mobile'] = $joinin_detail['bind_mobile'];
            $member_data['member_mobile_bind'] = 1;
            $member_data['is_buy'] = 0;

            $insert_id = $member_model->insert($member_data);
            $member_model->table('member_common')->insert(array('member_id' => $insert_id));


            $store_joinin_save_data['member_id'] = $insert_id;
            $store_joinin_save_data['member_name']  = $member_data['member_name'];
            $model_store_joinin->modify($store_joinin_save_data, array('bind_mobile'=>$_POST['bind_mobile']));

            //开店
            $shop_array     = array();
            $shop_array['member_id']    = $insert_id;
            $shop_array['member_name']  = $member_data['member_name'];
            ...
            if($store_id) {
                ...
                $seller_array['member_id'] = $insert_id;
            ...
            if($state) {
                ...
                for($i=0, $length=count($store_bind_class); $i<$length; $i++) {
                    list($class1, $class2, $class3, $class4) = explode(',', $store_bind_class[$i]);
                    $store_bind_class_array[] = array(
                        'store_id' => $store_id,
                ...
                DB::commit();
                showMessage('店铺开店成功','index.php?controller=store&action=store_joinin');
            } else {
                showMessage('店铺开店失败','index.php?controller=store&action=store_joinin');
            }
        } else {
            showMessage('店铺开店拒绝','index.php?controller=store&action=store_joinin');
        }
    }
