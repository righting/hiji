...
class stat_consumptionControl extends SystemControl
{
    private $links = [
    ...
    public function indexOp()
    {
        ...
        $lists         = $user_consumption_sale_log_day_model->where($where)->field($field)->page($page)->group('user_id')->order('created_at desc')->select();
        // 获取这一页的用户信息
        $new_list = [];
        if (!empty($lists)) {
        ...
        if ($_GET['exporttype'] == 'excel') {
            ...
            $excel_obj->addWorksheet($excel_obj->charset('个人消费统计', CHARSET));
            $excel_obj->generateXML($excel_obj->charset('个人消费统计', CHARSET) . date('Y-m-d-H', time()));
            exit();
        }
        Tpl::showpage('stat/index');
    }
}
