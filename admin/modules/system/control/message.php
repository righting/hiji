...
{
    private $links = [
        ['url' => 'controller=message&action=email', 'lang' => 'email_set'],
        ['url' => 'controller=message&action=email_tpl', 'lang' => 'email_tpl']
    ];
    ...
    public function email_tpl_editOp()
    {
        ...
        if (chksubmit()) {
            ...
            $obj_validate->validateparam = [
                ["input" => $_POST["code"], "require" => "true", "message" => L('mailtemplates_edit_no_null')],
                ["input" => $_POST["title"], "require" => "true", "message" => L('mailtemplates_edit_title_null')],
                ["input" => $_POST["content"], "require" => "true", "message" => L('mailtemplates_edit_content_null')],
            ];
            ...
            if ($error != '') {
                ...
                $result                  = $model_templates->editTpl($update_array, ['code' => $_POST['code']]);
                if ($result === true) {
                    $this->log(L('nc_edit,email_tpl'), 1);
                    showMessage(L('mailtemplates_edit_succ'), 'index.php?controller=message&action=email_tpl');
                } else {
        ...
        $templates_array = $model_templates->getTplInfo(['code' => $_GET['code']]);
