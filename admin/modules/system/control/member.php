...
class memberControl extends SystemControl{
	...
	private $links = array(
	    array('url'=>'controller=member&action=index','lang'=>'member_index_manage'),
		array('url'=>'controller=member&action=predeposit_add','lang'=>'member_index_predeposit'), 
    );
    ...
    public function get_xmlOp() {
        ...
        $all_user_id_arr = $model_member->getAllUserId();
        ...
        foreach ($member_list as $value) {
            $this_user_point = '未认证';
            if(in_array($value['member_id'],$all_user_id_arr)){
                $this_user_location = array_keys($all_user_id_arr, $value['member_id']); // 当前用户在所有用户中所处位置
                $this_later_count = bcsub(count(array_slice($all_user_id_arr, $this_user_location[0])), 1);  // 该用户之后注册加入的所有新会员用户
                $this_user_point = intval(floor(bcdiv($this_later_count, 100)));   // 用户垂直阵列下面的点位人数
            }

            $param = array();
            $param['operation'] = "<a class='btn blue' href='index.php?controller=member&action=member_edit&member_id=" . $value['member_id'] . "'><i class='fa fa-pencil-square-o'></i>编辑</a>
			<a href='javascript:submit_delete(". $value['member_id'] .");' class='btn red'><i class='fa fa-trash-o'></i>删除</a>";
            $param['member_id'] = $value['member_id'].'(点位数：'.$this_user_point.')';
            ...
            unset($this_user_location);
            unset($this_later_count);
            unset($this_user_point);
        }
    ...
    public function getUserForExcelOp(){
        ...
        foreach ($arr_chunk_user_id_arr as $k => $user_id_arr){
            foreach ($user_id_arr as $key=>$user_id){
                $excel_data[$k][$key] = array('data'=>'（会员ID：）'.$user_id);
            }
        }
	...
	public function predeposit_addOp(){
		if (chksubmit()){
			...
			if (!is_array($member_info) || count($member_info)<=0){
				showMessage(Language::get('admin_points_userrecord_error'),'index.php?controller=member&action=predeposit_add','','error');
			}
			...
			if ($_POST['operatetype'] == 2 && $money > $available_predeposit){
				showMessage(('预存款不足，会员当前预存款').$available_predeposit,'index.php?controller=member&action=predeposit_add','','error');
			}
			if ($_POST['operatetype'] == 3 && $money > $available_predeposit){
				showMessage(('可冻结预存款不足，会员当前预存款').$available_predeposit,'index.php?controller=member&action=predeposit_add','','error');
			}
			if ($_POST['operatetype'] == 4 && $money > $freeze_predeposit){
				showMessage(('可恢复冻结预存款不足，会员当前冻结预存款').$freeze_predeposit,'index.php?controller=member&action=predeposit_add','','error');
			}
			...
			switch ($_POST['operatetype'])
			{
				...
				case 4:
					...
					break;
				default:
					showMessage('操作失败','index.php?controller=member&action=predeposit_add');
					break;  
			}
			try {
				...
				$model_pd->commit();
				$this->log($log_msg,1);
				showMessage('操作成功','index.php?controller=member&action=predeposit_add');
			} catch (Exception $e) {
				$model_pd->rollback();
				$this->log($log_msg,0);
				showMessage($e->getMessage(),'index.php?controller=member&action=predeposit_add','html','error');
			}
		}else{
		   //输出子菜单
