...
class storeControl extends SystemControl{
    ...
    private $_links = array(
        array('url'=>'controller=store&action=store','text'=>'店铺管理'),
        array('url'=>'controller=store&action=distribution_list','text'=>'分销申请')
    );
    ...
    public function get_xmlOp() {
        ...
        foreach ($store_list as $value) {
            ...
            $operation = "<a class='btn green' href='index.php?controller=store&action=store_joinin_detail&member_id=".$value['member_id']."'><i class='fa fa-list-alt'></i>查看</a><span class='btn'><em><i class='fa fa-cog'></i>" . L('nc_set') . " <i class='arrow'></i></em><ul><li><a href='index.php?controller=store&action=store_edit&store_id=" . $value['store_id'] . "'>编辑店铺信息</a></li><li><a href='index.php?controller=store&action=del&store_id=" . $value['store_id'] . "'>删除该店铺</a></li><li><a href='index.php?controller=store&action=store_bind_class&store_id=" . $value['store_id'] . "'>修改经营类目</a></li>";
    ...
    public function get_distribution_xmlOp() {
        ...
        foreach ($distribution_list as $value) {
            ...
            $param['distri_end_time'] = $value['distri_end_time']?date('Y-m-d',$value['distri_end_time']):null;
    ...
    public function distribution_checkOp() {
        ...
        if ($id > 0) {
            ...
            if ($update) {
                if (empty($distribution_info['member_id'])){
                    //更新店铺有效期
                ...
                if (empty($distribution_info['distri_store_id'])){
                    Db::beginTransaction();
                    try{
                        Model('member')->editMember(array('member_id'=>$distribution_info['member_id']),array('is_distribution'=>1));//更新会员分销状态
                        $model_store_distribution-> editStoreDistribution(['distri_end_time'=>strtotime("+1 year")],['distri_id'=>$distribution_info['distri_id']]);;//设置分销有效期至1年后
                        Db::commit();
                    }catch (Exception $e){
                        Db::rollback();
                    }
