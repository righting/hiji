...
class helpControl extends SystemControl{
    ...
    public function helpOp(){
        ...
        //分类列表
        $model_class = Model('help_class');
        $parent_list = $model_class->getTreeClassList(10);
        ...
        Tpl::showpage('help.index');
    }

    public function deleteOp() {
        $model_help = Model('newhelp');
    ...
    public function get_xmlOp(){
        $type = isset($_GET['type']) ? intval(trim($_GET['type'])) : 0;
        ...
        !empty($type) && $condition['ac_id'] = $type;
        $condition['order'] = ltrim($condition['order'].',article_id desc',',');
        $page   = new Page();
        $page->setEachNum(intval($_POST['rp']));
        $page->setStyle('admin');
        $article_list = $model_help->getArticleList($condition,$page);
        $data = array();
        ...
        if (is_array($article_list)){
            ...
            foreach ($article_list as $k => $v){
                $list = array();
                $list['operation'] = "<a class='btn red' onclick=\"fg_delete({$v['article_id']})\"><i class='fa fa-trash-o'></i>删除</a><a class='btn blue' href='index.php?controller=help&action=help_edit&article_id={$v['article_id']}'><i class='fa fa-pencil-square-o'></i>编辑</a>";
    ...
    public function get_article_type(){
        return array('1'=>'商家','2'=>'消费者');
    }
    /**
     * 文章添加
     */
    public function help_addOp(){
        $lang   = Language::getLangContent();
        $model_help = Model('newhelp');
        /**
         * 保存
        ...
        if (chksubmit()){
            ...
            $obj_validate->validateparam = array(
                array("input"=>$_POST["article_title"], "require"=>"true", "message"=>$lang['help_add_title_null']),
                array("input"=>$_POST["ac_id"], "require"=>"true", "message"=>$lang['help_add_class_null']),
...
//                 array("input"=>$_POST["article_content"], "require"=>"true", "message"=>$lang['help_add_content_null']),
                array("input"=>$_POST["article_sort"], "require"=>"true", 'validator'=>'Number', "message"=>$lang['help_add_sort_int']),
            );
            ...
            if ($error != ''){
                ...
                $insert_array['article_type'] = trim($_POST['article_type']);
                $insert_array['common'] = $_POST['common']?$_POST['common']:'0';
                $insert_array['is_concerns'] = $_POST['isConcerns']?$_POST['isConcerns']:'0';
                $insert_array['is_wtsj'] = $_POST['isWtsj']?$_POST['isWtsj']:'0';
                $result = $model_help->add($insert_array);
                if ($result){
                    /**
                     * 更新图片信息ID
                     */
                    ...
                    $url = array(
                        array(
                            'url'=>'index.php?controller=help&action=help',
                            'msg'=>"{$lang['help_add_tolist']}",
                        ),
                        array(
                            'url'=>'index.php?controller=article&action=article_add&ac_id='.intval($_POST['ac_id']),
        ...
        $article_type = $this->get_article_type();
...
//                 array("input"=>$_POST["article_content"], "require"=>"true", "message"=>$lang['help_add_content_null']),
            ...
            if ($error != ''){
                ...
                $update_array['article_type'] = trim($_POST['article_type']);
                $update_array['common'] = $_POST['common']?$_POST['common']:'0';
                $update_array['is_concerns'] = $_POST['isConcerns']?$_POST['isConcerns']:'0';
                $update_array['is_wtsj'] = $_POST['isWtsj']?$_POST['isWtsj']:'0';
                $result = $model_help->updates($update_array);
                if ($result){
                    /**
                     * 更新图片信息ID
                     */
                    ...
                    $url = array(
                        ...
                        array(
                            'url'=>'index.php?controller=article&action=article_edit&article_id='.intval($_POST['article_id']),
        ...
        $article_array = $model_help->getOneArticle(intval($_GET['article_id']));
