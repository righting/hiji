...
class articleControl extends SystemControl{
     ...
     * 文章管理
     */
    public function articleOp($type){
        //分类列表
    ...
    public function get_xmlOp(){
        ...
        if(!empty($type)){
            ...
            foreach ($s_list as $sk=>$sv){
                $article_type[] = $sv['ac_id'];
            }
            $condition['ac_id'] = implode(",", $article_type);
        }
        ...
        if (is_array($article_list)){
            ...
            foreach ($article_list as $k => $v){
                $list = array();
                $list['operation'] = "<a class='btn red' onclick=\"fg_delete({$v['article_id']})\"><i class='fa fa-trash-o'></i>删除</a><a class='btn blue' href='index.php?controller=article&action=article_edit&article_id={$v['article_id']}'><i class='fa fa-pencil-square-o'></i>编辑</a>";
...
//                 array("input"=>$_POST["article_content"], "require"=>"true", "message"=>$lang['article_add_content_null']),
            ...
            if ($error != ''){
                ...
                $insert_array['article_image'] = trim($_POST['articleImage']);
                ...
                if ($result){
                    ...
                    $url = array(
                        array(
                            'url'=>'index.php?controller=article&action=article',
...
//                 array("input"=>$_POST["article_content"], "require"=>"true", "message"=>$lang['article_add_content_null']),
            ...
            if ($error != ''){
                ...
                $update_array['article_image'] = trim($_POST['articleImage']);
