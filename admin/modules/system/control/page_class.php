...
class page_classControl extends SystemControl{
    public function __construct(){
        parent::__construct();
        Language::read('page_class');
    }
    ...
    public function page_classOp(){
        ...
        Tpl::showpage('page_class.index');
    }

    /**
     * 文章分类 新增
     */
    public function page_class_addOp(){
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result){
                    $url = array(
                        array(
                            'url'=>'index.php?controller=page_class&action=page_class_add&ac_parent_id='.intval($_POST['ac_parent_id']),
                            'msg'=>$lang['article_class_add_class'],
                        ),
                        array(
                            'url'=>'index.php?controller=page_class&action=page_class',
                            'msg'=>$lang['article_class_add_back_to_list'],
                        )
                    );
                    $this->log(l('nc_add,page_class_index_class').'['.$_POST['ac_name'].']',1);
		...
		Tpl::setDirquna('system');
        Tpl::showpage('page_class.add');
    }

    /**
     * 文章分类编辑
     */
    public function page_class_editOp(){
        $lang   = Language::getLangContent();
        $model_class = Model('page_class');
...
//              $update_array['ac_parent_id'] = intval($_POST['ac_parent_id']);
                ...
                if ($result){
                    $url = array(
                        ...
                        array(
                            'url'=>'index.php?controller=page_class&action=page_class_edit&ac_id='.intval($_POST['ac_id']),
                            'msg'=>$lang['article_class_edit_again'],
                        ),
                    );
                    $this->log(l('nc_edit,page_class_index_class').'['.$_POST['ac_name'].']',1);
                    showMessage($lang['article_class_edit_succ'],'index.php?controller=page_class&action=page_class');
                }else {
		...
		Tpl::setDirquna('system');
        Tpl::showpage('page_class.edit');
    }

    /**
     * 删除分类
     */
    public function page_class_delOp(){
        $lang   = Language::getLangContent();
        $model_class = Model('page_class');
        if (intval($_GET['ac_id']) > 0){
            ...
            $this->log(l('nc_add,page_class_index_class').'[ID:'.intval($_GET['ac_id']).']',1);
            showMessage($lang['article_class_index_del_succ'],'index.php?controller=page_class&action=page_class');
        }else {
            showMessage($lang['article_class_index_choose'],'index.php?controller=page_class&action=page_class');
        }
    }
    /**
     * ajax操作
