  ...
  <form id="article_form" method="post" name="articleForm">
    ...
    <div class="ncap-form-default">
                ...
                <select name="ac_id" id="ac_id">
                    ...
                    <?php if(!empty($output['parent_list']) && is_array($output['parent_list'])){ ?>
                        <?php foreach($output['parent_list'] as $k => $v){ ?>
                            <option <?php if($output['info']['c_id']==$v['id']){echo 'selected';}?> value="<?php echo $v['id'];?>"><?php echo $v['cate_name'];?></option>
                        <?php } ?>
                    <?php } ?>
                </select>
                <span class="err"></span>
        ...
        <dl class="row">
            ...
            <dd class="opt">
                <input type="date" value="<?php echo substr($output['info']['start_time'],0,10);?>"  style="width:140px;" name="start_time"> -- <input type="date" style="width:140px;" name="end_time" value="<?php echo substr($output['info']['end_time'],0,10);?>"  >
                <span class="err"></span>
        ...
        <dl class="row">
            ...
            <input type="button" name="button" id="button" value="选择上传..." class="type-file-button" />
                ...
                <p class="notic">如需修改图片请直接上传新的图片</p>
            </dd>
        </dl>

        <input type="hidden" value="<?php echo $output['info']['img_url']?>" name="img" id="img"/>
        <input type="hidden" value="<?php echo $output['info']['id']?>" name="id" id="id"/>
      ...
      <dl class="row">
        ...
        <dd class="opt">
          <input type="text" value="<?php echo $output['info']['img_link']?>" name="img_link" id="img_link" class="input-txt">
          <span class="err"></span>
        ...
        <dl class="row">
            ...
            <dd class="opt">
                <input type="text" value="<?php echo $output['info']['sort']?>" name="sort" id="sort" class="input-txt">
                <span class="err"></span>
        ...
        <dl class="row">
            ...
            <dd class="opt">
                <div class="onoff">
                    <label for="status1" class="cb-enable <?php if($output['info']['status']==1){echo 'selected';}?>" ><?php echo $lang['nc_yes'];?></label>
                    <label for="status0" class="cb-disable <?php if($output['info']['status']==0){echo 'selected';}?>" ><?php echo $lang['nc_no'];?></label>
