...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?controller=goods&action=get_xml&type=<?php echo $output['type'];?>',
        colModel : [
            ...
            {display: '供货价格(元)', name : 'goods_costprice', width : 100, sortable : true, align: 'center'},
...
function fg_lonkup(ids) {
    _uri = "index.php?controller=goods&action=goods_lockup&id=" + ids;
    CUR_DIALOG = ajax_form('goods_lockup', '违规下架理由', _uri, 640);
}
//分销设置
function fg_invite(ids) {
    _uri = "index.php?controller=goods&action=goods_invite&id=" + ids;
    CUR_DIALOG = ajax_form('goods_invite', '分销设置', _uri, 640);
}
//添加评论
function fg_pinglun(gids) {
    _uri = "index.php?controller=goods&action=add&goods_id=" + gids;
    CUR_DIALOG = ajax_form('goods_pinlun', '为此商品添加评论', _uri, 640);
}

function fg_sku(commonid) {
    CUR_DIALOG = ajax_form('login','商品"' + commonid +'"的SKU列表','index.php?controller=goods&action=get_goods_sku_list&commonid=' + commonid, 580);
}
// 删除
function fg_del(id) {
