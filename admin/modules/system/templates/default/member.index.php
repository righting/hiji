...
$(function(){
    $("#flexigrid").flexigrid({
        ...
        buttons : [
            ...
            {display: '<i class="fa fa-file-excel-o"></i>导出矩阵数据', name : 'excel', bclass : 'excel', title : '将矩阵数据导出excel文件', onpress : fg_operation }
            ],
        searchitems : [
...
function fg_operation(name, bDiv) {
    ...
    if (name == 'excel') {
        ...
        window.location.href = $("#flexigrid").flexSimpleSearchQueryString()+'&action=getUserForExcel';
    }
}
...
function submit_delete(id){
	...
	if(confirm('删除后将不能恢复，确认删除这 ' + id.length + ' 项吗？')){
		id = id.join(',');
        window.location.href = 'index.php?controller=member&action=member_del&member_id='+id;
    }
}
