...
<div class="page">
    <div class="fixed-bar">
        <div class="item-title"><a class="back" href="index.php?controller=navigation&action=navigation" title="返回列表"><i
    ...
    <form id="navigation_form" method="post">
        ...
        <div class="ncap-form-default">
            <dl class="row">
              ...
              <input type="radio"
                     <?php if ($output['navigation_array']['nav_type'] == '0'){ ?>checked="checked"<?php } ?> value="0"
                     name="nav_type" id="diy" onclick="showType('diy');">
              <label for="diy"><?php echo $lang['navigation_add_custom']; ?></label>
              </span></li>
                        <li class="left "><span class="radio">
              <input type="radio"
                     <?php if ($output['navigation_array']['nav_type'] == '1'){ ?>checked="checked"<?php } ?> value="1"
                     name="nav_type" id="goods_class" onclick="showType('goods_class');">
              <label for="goods_class"><?php echo $lang['navigation_add_goods_class']; ?></label>
                            ...
                            <select name="goods_class_id" id="goods_class_id" style="display: none;">
                                <?php if (is_array($output['goods_class_list'])) { ?>
                                    <?php foreach ($output['goods_class_list'] as $k => $v) { ?>
                                        <option
                                            <?php if ($output['navigation_array']['item_id'] == $v['gc_id']){ ?>selected="selected"<?php } ?>
                        ...
                        <li class="left "><span class="radio">
              <input type="radio"
                     <?php if ($output['navigation_array']['nav_type'] == '2'){ ?>checked="checked"<?php } ?> value="2"
                     name="nav_type" id="article_class" onclick="showType('article_class');">
              <label for="article_class"><?php echo $lang['navigation_add_article_class']; ?></label>
                            ...
                            <select name="article_class_id" id="article_class_id" style="display: none;">
                                <?php if (is_array($output['article_class_list'])) { ?>
                                    <?php foreach ($output['article_class_list'] as $k => $v) { ?>
                                        <option
                                            <?php if ($output['navigation_array']['item_id'] == $v['ac_id']){ ?>selected="selected"<?php } ?>
                                            value="<?php echo $v['ac_id']; ?>"><?php echo $v['ac_name']; ?></option>
                                    <?php } ?>
                                <?php } ?>
                            </select>
                        </li>
                        <li class="left "><span class="radio">
              <input type="radio"
                     <?php if ($output['navigation_array']['nav_type'] == '3'){ ?>checked="checked"<?php } ?> value="3"
                     name="nav_type" id="activity" onclick="showType('activity');">
                            ...
                            <select name="activity_id" id="activity_id" style="display: none;">
                                <?php if (is_array($output['activity_list'])) { ?>
                                    <?php foreach ($output['activity_list'] as $k => $v) { ?>
                                        <option
                                            <?php if ($output['navigation_array']['item_id'] == $v['activity_id']){ ?>selected="selected"<?php } ?>
                                            value="<?php echo $v['activity_id']; ?>"><?php echo $v['activity_title']; ?></option>
                                    <?php } ?>
                                <?php } ?>
                            </select>
                        </li>
            ...
            <dl class="row">
                <dt class="tit">
                    <label for="type"><?php echo $lang['navigation_index_modue']; ?></label>
                </dt>
                <dd class="opt">
                    <!--导航所属模型 0=全局 1=商城 2=农耕优品 3=全球跨境 4=积分商城 5=消费资本 6=线下加盟 7=分享阵地-->
                    <input type="radio" checked="checked" value="0" name="nav_modue">
                    <label><?php echo $lang['navigation_index_modue_a']; ?></label>
                    <input type="radio" value="1" name="nav_modue">
                    <label><?php echo $lang['navigation_index_modue_b']; ?></label>
                    <input type="radio" value="2" name="nav_modue">
                    <label><?php echo $lang['navigation_index_modue_c']; ?></label>
                    <input type="radio" value="3" name="nav_modue">
                    <label><?php echo $lang['navigation_index_modue_d']; ?></label>
                    <input type="radio" value="4" name="nav_modue">
                    <label><?php echo $lang['navigation_index_modue_e']; ?></label>
                    <input type="radio" value="5" name="nav_modue">
                    <label><?php echo $lang['navigation_index_modue_f']; ?></label>
                    <input type="radio" value="6" name="nav_modue">
                    <label><?php echo $lang['navigation_index_modue_g']; ?></label>
                    <input type="radio" value="7" name="nav_modue">
                    <label><?php echo $lang['navigation_index_modue_h']; ?></label>
                    <span class="err"></span>
                    <p class="notic"></p>
                </dd>
            </dl>
            ...
            <dl class="row">
                <dt class="tit">
                    <label for="parent_id"><?php echo $lang['navigation_add_sup_class'];?></label>
                </dt>
                <dd class="opt">
                    <select name="nav_pid" id="nav_pid">
                        <option value="0"><?php echo $lang['nc_please_choose'];?></option>
                        <?php if(!empty($output['navigation_type_list']) && is_array($output['navigation_type_list'])){ ?>
                            <?php foreach($output['navigation_type_list'] as $k => $v){ ?>
                                <option <?php if(isset($output['nav_pid']) && $output['nav_pid'] == $v['nav_id']){ ?>selected='selected'<?php } ?> value="<?php echo $v['nav_id'];?>"><?php echo $v['nav_title'];?></option>
                            <?php } ?>
                        <?php } ?>
                    </select>
                    <span class="err"></span>
                    <p class="notic"><?php echo $lang['navigation_add_sup_class_notice'];?></p>
                </dd>
            </dl>
            <!--选择所属父级-----结束-->
