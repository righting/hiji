        ...
        <ul class="tab-base nc-row">
            <li>
                <select class="category">
                    ...
                    <?php foreach ($output['parent_list'] as $key=>$v){ ?>
                        <option value ="<?php echo $v['ac_id'];?>" <?php if($output['type']==$v['ac_id']){ echo 'selected';}?>><?php echo $v['ac_name']; ?></option>
                    <?php } ?>
                </select>
            </li>
        </ul>
...
$(function(){
    ...
    $("#flexigrid").flexigrid({
    	url: 'index.php?controller=article&action=get_xml&type=<?php echo $output['type'];?>',
    	colModel : [
        ...
        searchitems : [
            {display: '文章标题', name : 'article_title', isdefault: true}
        ],
...
function fg_operate(name, bDiv) {
    if (name == 'del') {
    	...
    	window.location.href = 'index.php?controller=article&action=article_add';
    }
}
$('.category').on('change',function(){
    type = $.trim($(this).children('option:selected').val());
    window.location.href = 'index.php?controller=article&action=index&type='+type;
});
function fg_delete(id) {
	...
	$.ajax({
        type: "GET",
        dataType: "json",
        url: "index.php?controller=article&action=delete",
        data: "del_id="+id,
