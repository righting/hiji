...
final class Base
{
    ...
    private static function parse_conf(&$setting_config)
    {
        ...
        $setting['ccynet_version'] = '<span class="vol"><font class="b">CCY</font><font class="o">NET</font></span>';
    ...
    {
        //二级域名
        if ($GLOBALS['setting_config']['enabled_subdomain'] == '1' && $_GET['controller'] == 'index' && $_GET['action'] == 'index') {
            $store_id = subdomain();
            if ($store_id > 0) $_GET['controller'] = 'show_store';
        }
        $act_file   = realpath(BASE_PATH . '/control/' . $_GET['controller'] . '.php');
        $class_name = $_GET['controller'] . 'Control';
        if (!@include($act_file)) {
            if (C('debug')) {
    ...
    private static function controladmin($admin)
    {
        ...
        $act_file = realpath(BASE_PATH . '/modules/' . $admin . '/control/' . $_GET['controller'] . '.php');
    ...
    public static function autoload($class)
    {
        ...
        if (ucwords(substr($class, -5)) == 'Class') {
            if (!@include_once(BASE_PATH . DS . 'framework' . DS . 'libraries' . DS . substr($class, 0, -5) . '.class.php')) {
                exit("Class Error: {$class}.isn't exists!");
            }
        } elseif ((ucwords(substr($class, -5)) == 'Model') && (strlen($class)) > 5) {
            if (!@include_once(BASE_DATA_PATH . DS . 'model' . DS . substr($class, 0, strlen($class) - 5) . '.model.php')) {
                exit("Class Error: {$class}.isn't exists!");
            }
        } elseif ((ucwords(substr($class, -5)) == 'Logic') && (strlen($class)) > 5) {
            if (!@include_once(BASE_DATA_PATH . DS . 'logic' . DS . substr($class, 0, strlen($class) - 5) . '.logic.php')) {
                exit("Class Error: {$class}.isn't exists!");
            }
        } elseif (ucwords(substr($class, 0, 5)) == 'Cache' && $class != 'cache') {
            if (!@include_once(BASE_CORE_PATH . DS . 'framework' . DS . 'cache' . DS . substr($class, 0, 5) . '.' . substr($class, 5) . '.php')) {
                exit("Class Error: {$class}.isn't exists!");
            }
        } elseif ($class == 'db') {
            if (!@include_once(BASE_CORE_PATH . DS . 'framework' . DS . 'db' . DS . strtolower(DBDRIVER) . '.php')) {
                exit("Class Error: {$class}.isn't exists!");
            }
        } else {
            if (!@include_once(BASE_CORE_PATH . DS . 'framework' . DS . 'libraries' . DS . $class . '.php')) {
                exit("Class Error: {$class}.isn't exists!");
            }
        }
    }
    ...
    private static function cp()
    {
        ...
        if (strpos(self::CPURL, '||') !== false) {
            ...
            header('location: http://www.ccynet.cn/');
            exit();
        } else {
