 ...
 *
 *
 * @package    function* www.ccynet.cn海吉壹佰技术交流中心为你提供售后服务 以便你更好的了解
 */
...
function cthumb($file, $type = '', $store_id = false) {
    ...
    $fname = basename($file);
    $image_pathinfo = pathinfo($file);
    if($image_pathinfo['dirname'].'/' == C('oss.img_url')){
        // 如果文件是oss域名开头的
//        return $file.'?x-oss-process=image/resize,m_pad,h_'.$type.',w_'.$type.',color_FFFFFF';
        return $file;
    }
    return $file;
}
...
defined("ByCCYNet") || exit("Access Invalid!");
