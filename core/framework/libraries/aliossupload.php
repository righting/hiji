...
class AliOssUpload{
    ...
    public function upfile($field){
        ...
        $oss_sdk_service = new ALIOSS(C('oss.access_id'),C('oss.access_key'),'haijishop.oss-cn-shenzhen.aliyuncs.com');

        //设置是否打开curl调试模式
        $oss_sdk_service->set_debug_mode(true);
        $upload_response = $this->upload_by_file($oss_sdk_service,'haijishop',$this->file_name,$this->upload_file['tmp_name']);
        if($upload_response){
    ...
    //通过路径上传文件
    public function upload_by_file($obj,$bucket,$object,$file_path){
