...
class Sms
{
    ...
    {
        $ccynet_sms_type = C('ccynet_sms_type');
        if ($ccynet_sms_type == 1) {
            return $this->mysend_smsbao($mobile, $content);// 短信宝
        }
        if ($ccynet_sms_type == 2) {
            return $this->mysend_yunpian($mobile, $content);// 云片
        }
        if ($ccynet_sms_type == 3) {
            return $this->_sendAPI($mobile, $content);// 互亿无线短信，需要更改模板
        }
        if ($ccynet_sms_type == 4) {
            return $this->smsAliYun($mobile, $content);// 阿里云
        }
    }
    ...
    {
        $user_id = urlencode(C('ccynet_sms_zh')); // 这里填写用户名
        $pass    = urlencode(C('ccynet_sms_pw')); // 这里填登陆密码
     ...
     * @return bool
     */
    private function smsAliYun($mobile, $content)
    {
        $aliyun = 'aliyun';
        $plugin = str_replace('\\', '', str_replace('/', '', str_replace('.', '', $aliyun)));
        if (!empty($aliyun)) {
            define('PLUGIN_ROOT', BASE_DATA_PATH . DS . 'api/smsapi');
            require_once(PLUGIN_ROOT . DS . $plugin . DS . 'Send.php');
