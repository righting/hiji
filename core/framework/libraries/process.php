...
class process
{
    ...
    {
        return str_replace(['login', 'commit', 'reg', 'forget', 'admin'], [1, 2, 3, 4, 5], $type);
    }
    ...
    {
        return str_replace(['login', 'commit', 'reg', 'forget', 'admin'], [self::MAX_LOGIN, self::MAX_COMMIT, self::MAX_REG, self::MAX_FORGET, self::MAX_ADMIN], $type);
    }
}
...
class lock
{
    ...
    {
        $info = $this->model->where(['pid' => $key])->find();
        if ($info) {
            $this->model->where(['pid' => $key])->update(['pvalue' => $value, 'expiretime' => TIMESTAMP + $ttl]);
        } else {
            $this->model->insert(['pid' => $key, 'pvalue' => $value, 'expiretime' => TIMESTAMP + $ttl]);
        }
    }
    ...
    {
        return $this->model->where(['pid' => $key])->delete();
    }
}